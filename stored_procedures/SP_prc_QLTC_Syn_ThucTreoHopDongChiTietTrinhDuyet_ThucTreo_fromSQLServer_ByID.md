# Stored Procedure: `prc_QLTC_Syn_ThucTreoHopDongChiTietTrinhDuyet_ThucTreo_fromSQLServer_ByID`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2024-02-23 17:57:17.387000
- **Ngày sửa cuối**: 2024-02-23 17:57:17.387000
...
*/
CREATE PROCEDURE [dbo].[prc_QLTC_Syn_ThucTreoHopDongChiTietTrinhDuyet_ThucTreo_fromSQLServer_ByID] 	
...
BEGIN
	...
	WHERE ThucTreoHopDongChiTietTrinhDuyetID  IN (SELECT [Name] FROM dbo.STRING_SPLIT_QLTC(@ThucTreoIds))
	...
	CREATE TABLE #ThucTreoHopDongChiTietTrinhDuyet_Thuctreo (
	  ...
	  record_status SMALLINT
		...
		SELECT tc.Id AS ThucTreoHopDongChiTietTrinhDuyetID,
               ...
               tc.DeletedStatus,
			   '' AS linkBai,
			   (select top (1) tca.NhanVien_Id from  [asdag2].ThucTreo.dbo.[ThucTreo_ChiPhi_Account] tca where tca.thuctreo_chiphi_REF = tc.id and tca.deleted_status = 0) AS ListNhanVienREF,
               0 status_table FROM [asdag2].ThucTreo.dbo.ThucTreo_ChiPhi tc
		WHERE tc.ID IN (SELECT [Name] FROM dbo.STRING_SPLIT_QLTC(@ThucTreoIds))
		...
		where dc.ThucTreoHopDongChiTietTrinhDuyetID IN (SELECT [Name] FROM dbo.STRING_SPLIT_QLTC(@ThucTreoIds))
	...
	and B.ThucTreoHopDongChiTietTrinhDuyetID IN (SELECT [Name] FROM dbo.STRING_SPLIT_QLTC(@ThucTreoIds))
	...
	and dchdct.ThucTreoHopDongChiTietTrinhDuyetID IN (SELECT [Name] FROM dbo.STRING_SPLIT_QLTC(@ThucTreoIds))
