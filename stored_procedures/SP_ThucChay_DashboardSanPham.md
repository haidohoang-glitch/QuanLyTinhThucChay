# Stored Procedure: `ThucChay_DashboardSanPham`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2013-06-14 13:08:23.340000
- **Ngày sửa cuối**: 2014-11-19 12:16:43.543000
...
```sql
CREATE PROCEDURE [dbo].[ThucChay_DashboardSanPham]
	@FromDate datetime, 
	@ToDate datetime,
...
BEGIN
	...
	IF @DonViThoiGian >= 0
		Begin
			...
			-- Set GroupBy
			SET @GroupBy = ' A.DmSanPhamREF,A.TenSanPham,dbo.GetDonViThoiGian(NgayThucHien,' + CONVERT(nvarchar(50), @DonViThoiGian) + ')'
			-- Set OrderBy
			SET @OrderBy = ' dbo.GetDonViThoiGian(NgayThucHien,' + CONVERT(nvarchar(50), @DonViThoiGian) + '), A.TenSanPham'
		End
	ELSE IF @DonViThoiGian = -1
		Begin 
			-- Set GroupBy
			SET @GroupBy = ' A.DmSanPhamREF,A.TenSanPham'
			-- Set OrderBy
			SET @OrderBy = ' A.TenSanPham'
		End
	...
	IF (@ListUserName <> '0')
		SET @sql = @sql + ' AND A.TenDangNhap IN (N' + CONVERT(nvarchar(2000), @ListUserName) + ')'
