# Stored Procedure: `rptThucChay_GetDataBaoCaoChiTiet`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-03-27 17:43:53.273000
- **Ngày sửa cuối**: 2015-03-27 17:43:53.273000
...
--EXEC [rptThucChay_GetDataBaoCaoChiTiet] '01',1,100,'TenSanPham','2013-01-01','2013-01-31','','','','','','','','Phuongvtt','','','',-1,0,0,0,'','',''
CREATE PROCEDURE [dbo].[rptThucChay_GetDataBaoCaoChiTiet]
...
BEGIN
 ...
 --   SET @ToUserName = (SELECT ToUserName FROM MappingUser A WHERE A.FromUserName = @TenDangNhap)
    ...
    SELECT 
		...
		T.GroupFieldID,
		T.GroupFieldName,
		T.TenDonViTinh AS DonViTinh,
		T.GiaTriThayDoiTC + T.GiaTriThayDoiNB AS GTTD,
		...
		--T.SoLuongNoiBoPhatSinhTrongKy - T.SoLuongNoiBoThayDoiTrongKy AS SoLuongThucChayNoiBo,
		--T.SoLuongKhuyenMaiPhatSinhTrongKy - T.SoLuongKhuyenMaiThayDoiTrongKy AS  SoLuongThucChayKhuyenMai,
		--T.SoLuongPhatSinhTrongKy - T.SoLuongThayDoiTrongKy AS SoLuongThucChayThucThu,
		T.SoLuongNoiBoPhatSinhTrongKy  AS SoLuongThucChayNoiBo,
		T.SoLuongKhuyenMaiPhatSinhTrongKy  AS  SoLuongThucChayKhuyenMai,
		T.SoLuongPhatSinhTrongKy  AS SoLuongThucChayThucThu,
    ...
    (
       SELECT 
			...
			SUM(T1.SoLuongPhatSinhTrongKy) AS SoLuongPhatSinhTrongKy,
    		SUM(T1.SoLuongKhuyenMaiPhatSinhTrongKy) AS SoLuongKhuyenMaiPhatSinhTrongKy,
    		SUM(T1.SoLuongNoiBoPhatSinhTrongKy) AS SoLuongNoiBoPhatSinhTrongKy,
    		SUM(T1.SoLuongThayDoiTrongKy) AS SoLuongThayDoiTrongKy,
    		SUM(T1.SoLuongKhuyenMaiThayDoiTrongKy) AS SoLuongKhuyenMaiThayDoiTrongKy,
    		SUM(T1.SoLuongNoiBoThayDoiTrongKy) AS SoLuongNoiBoThayDoiTrongKy,
			ROW_NUMBER() OVER (ORDER BY GroupFieldName) AS num
		FROM @TempTable T1 
        GROUP BY GroupFieldName,GroupFieldID,T1.TenDonViTinh
        HAVING ROUND(SUM(T1.GiaTriThayDoiTC + T1.GiaTriThayDoiNB),0) <> 0 OR ROUND(SUM(T1.ThanhTienNoiBoThucThu),0) <> 0 OR ROUND(SUM(T1.ThanhTienKhuyenMaiThucThu),0) <> 0 OR ROUND(SUM(T1.ThanhTienThucThu),0) <> 0
		)T
