# Stored Procedure: `ThucChay_BaoCaoLechTreoHaGetListHopDong`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-07-17 08:28:14.327000
- **Ngày sửa cuối**: 2014-11-19 12:16:58.147000
...
-- =============================================
CREATE PROCEDURE [dbo].[ThucChay_BaoCaoLechTreoHaGetListHopDong]
	@PageIndex				INT,
	@RecordCount			INT,
	@StartDate				datetime,
	@EndDate				datetime,
...
BEGIN
	...
	SET @SqlCommand = '
		SELECT 
			B.DmSanPhamREF, B.TenSanPham, B.DonViTinh, B.SoHopDong,
		...
		('
			+ dbo.ThucChay_GenSQLCommandForBaoCaoLechTreoHaAllSanPham(@StartDate,
												...
												@IsShowNoiBo) +
		')B
		GROUP BY B.DmSanPhamREF, B.TenSanPham, B.DonViTinh, B.SoHopDong ';
	...
	SELECT 
		DmSanPhamREF, TenSanPham, dbo.FormatText(DonViTinh) AS DonViTinh, SoHopDong,
		dbo.FormatNumber(SoLuongHopDong) AS SoLuongHopDong, 
		...
		dbo.FormatNumber(SoLuongLechTreoHa) AS SoLuongLechTreoHa, 
		dbo.FormatNumber(ThanhTienLechTreoHa) AS ThanhTienLechTreoHa
	FROM
	(
		SELECT 	
			DmSanPhamREF, TenSanPham, DonViTinh, SoHopDong,
			...
			ROW_NUMBER() OVER (ORDER BY (TenSanPham) ASC) AS num 
		FROM #TempTable
		GROUP BY
			DmSanPhamREF, TenSanPham, DonViTinh, SoHopDong
	)T
