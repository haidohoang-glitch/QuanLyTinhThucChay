# Stored Procedure: `prc_asd_Admarket_TinhGiaTri_Online_PhanBo`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2024-02-06 10:58:48.753000
- **Ngày sửa cuối**: 2024-02-28 11:33:41.873000
...
CREATE PROCEDURE [dbo].[prc_asd_Admarket_TinhGiaTri_Online_PhanBo]
...
BEGIN
	...
	SELECT 
      ...
      ,LTRIM(RTRIM(STR(SUM(convert(money,[domain_tt_money])/1.08) - ISNULL(( SELECT   SUM(GiaTriThayDoi + ThanhTienSauTrietKhauThucChay)
  ...
  group by 
	...
	SELECT 
      ...
      ,SUM(convert(money,[domain_tt_click])) [domain_tt_click]
      ,SUM(convert(money,[domain_tt_view])) [domain_tt_view]
      , LTRIM(RTRIM(STR(SUM(convert(money,[domain_tt_money])/1.08)
  ...
  group by 
	...
	SELECT 
      ...
      ,SUM(convert(money,[domain_tt_click])) [domain_tt_click]
      ,SUM(convert(money,[domain_tt_view])) [domain_tt_view]
      ,LTRIM(RTRIM(STR(SUM(convert(money,[domain_money])/1.08) - ISNULL(( SELECT   SUM(GiaTriThayDoi + ThanhTienSauTrietKhauThucChay)
                                                   FROM     ThucChayDaTinhAdmarket c
                                                   WHERE    SoHopDong = a.contract_number
