# Stored Procedure: `ThucChayDaTinh_CheckVaUpdateGiaTriThayDoi_ThanhTien_GGFB_ByHopDongChiTietREF`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-10-10 10:25:11.367000
- **Ngày sửa cuối**: 2021-11-26 14:29:16.720000
...
--	  AND NgayThucHien = @NgayThucHien
	...
	EXEC [[ThucChayDaTinh_CheckVaUpdateGiaTriThayDoi_ThanhTien_GGFB_ByHopDongChiTietREF]] '2021-06-18',13678
*/
CREATE PROCEDURE [dbo].[ThucChayDaTinh_CheckVaUpdateGiaTriThayDoi_ThanhTien_GGFB_ByHopDongChiTietREF]
...
BEGIN
	...
	(
		...
		AND (hdct.DmSanPhamREF in (306,423) OR DmViTriREF in (100093,100478) )
	...
	(SELECT ISNULL(OD.Contract_Id,0) Contract_Id, OD.Contract_Number, OD.Id AS Operating_Order_Id
		...
		AND ISNULL(OD.Contract_Detail_Id,0) = @HopDongChiTietREF
	)OD 
	OUTER APPLY
	...
	*/
	...
	SELECT CONTRACT_ID 
		...
		, IS_PROCESSED  
	FROM @Table_ThucChay_ThanhTien_GGFB
	WHERE CONTRACT_DETAIL_ID = @HopDongChiTietREF
	--where OPERATING_ORDER_ID = 164
	...
	BEGIN
		...
		BEGIN
			...
			IF(EXISTS(SELECT TOP(1) tcdt.HopDongID FROM ThucChayDaTinh tcdt
				...
				AND (tcdt.DmSanPhamREF in (306,423,5160,5188)  OR tcdt.DmViTriREF in (100093,100478))
			...
			BEGIN
				...
				IF(EXISTS(SELECT TOP (1) OPERATING_ORDER_ID FROM @Table_ThucChay_ThanhTien_GGFB TC
				WHERE (TC.DATATYPE IN (0) AND TC.IS_PROCESSED = 0)
		...
		BEGIN
			...
			BEGIN
				...
				IF(EXISTS(SELECT TOP (1) OPERATING_ORDER_ID FROM @Table_ThucChay_ThanhTien_GGFB TC
				WHERE (TC.DATATYPE IN (1) AND TC.IS_PROCESSED = 0)
				AND TC.CONTRACT_ID = @CONTRACT_ID
				AND TC.CONTRACT_DETAIL_ID = @CONTRACT_DETAIL_ID
		...
		BEGIN
			...
			IF(EXISTS(SELECT TOP (1) OPERATING_ORDER_ID FROM @Table_ThucChay_ThanhTien_GGFB TC
				WHERE (TC.DATATYPE IN (1,2,3) AND TC.IS_PROCESSED = 0)
				AND TC.CONTRACT_ID = @CONTRACT_ID
				AND TC.CONTRACT_DETAIL_ID = @CONTRACT_DETAIL_ID
		...
		BEGIN
			...
			IF(EXISTS(SELECT TOP (1) OPERATING_ORDER_ID FROM @Table_ThucChay_ThanhTien_GGFB TC
				WHERE (TC.DATATYPE IN (1,2,3,4,5) AND TC.IS_PROCESSED = 0)
				AND TC.CONTRACT_ID = @CONTRACT_ID
				AND TC.CONTRACT_DETAIL_ID = @CONTRACT_DETAIL_ID
				AND TC.OPERATING_ORDER_ID = @OPERATING_ORDER_ID
		...
		BEGIN
			...
			IF(EXISTS(SELECT TOP (1) OPERATING_ORDER_ID FROM @Table_ThucChay_ThanhTien_GGFB TC
				WHERE (TC.DATATYPE IN (1,2,3,4,5,6,7) AND TC.IS_PROCESSED = 0)
				AND TC.CONTRACT_ID = @CONTRACT_ID
				AND TC.CONTRACT_DETAIL_ID = @CONTRACT_DETAIL_ID
				AND TC.OPERATING_ORDER_ID = @OPERATING_ORDER_ID 
				AND TC.ADS_OPERATING_RESULT_MAP_ORDER_OR_QUANTITY_ID = @ADS_OPERATING_RESULT_MAP_ORDER_OR_QUANTITY_ID
