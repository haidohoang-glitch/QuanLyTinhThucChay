# Stored Procedure: `ThucChayDaTinh_MuaNgoai_InsertThucChayLai_SanLuongChot_GTTD_ThanhTien_GGFB`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-09-09 16:40:27.467000
- **Ngày sửa cuối**: 2024-11-26 11:19:28.973000
...
CREATE PROCEDURE [dbo].[ThucChayDaTinh_MuaNgoai_InsertThucChayLai_SanLuongChot_GTTD_ThanhTien_GGFB]
...
BEGIN
	...
	@DmChienDichREF INT = @Operating_Order_Id --Chien dich chay cho GGFB va tinh tien theo san luong chot
	, @Brand_id NVARCHAR(100) = ''
	, @DmWebsiteREF INT = 265

	SELECT TOP 1 @Brand_id = ISNULL(OD.Brand_id,0) , @DmWebsiteREF = ISNULL(OD.DmWebsiteREF,0)
	...
	SET @Brand_id = ISNULL((@Brand_id),0)
    ...
    SELECT
           ...
           ,[GiaTriThayDoiLaiSauCK] = IIF((HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100) , 0, @TongTienLaiThucChaySCK)
           ,[SoLuongThayDoi] = IIF ((HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100), 0, @SoLuongThucChay)
           ,[SoLuongKMThayDoi] = IIF((HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100), @SoLuongThucChay, 0)
           ,[GiaTriKMLaiThayDoi] = IIF((HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100), @TongTienLaiThucChaySCK, 0)  -- thực chạy lãi của TH Khuyến mại 100%
           ,[GhiChu] = @GhiChu
           ,[CreatedAt] = GETDATE()
           ,[LastModifiedAt] = GETDATE()
		FROM 
