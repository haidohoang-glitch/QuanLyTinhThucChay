# Stored Procedure: `sp_TC_DoiTruVaTinhLai_CPM_V2`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2023-08-15 16:22:11.033000
- **Ngày sửa cuối**: 2024-10-14 16:30:26.173000
...
CREATE  PROCEDURE [dbo].[sp_TC_DoiTruVaTinhLai_CPM_V2]
    @StartDate DATETIME
  , @EndDate DATETIME
...
AS
    BEGIN
        ...
        --1. THUC HIEN DOI TRU TOAN BO GROUP THEO BANNER,...
		...
		BEGIN
			...
			-- Tính lại
					DELETE  FROM dbo.[ThucChayTemp_TinhLai]

					INSERT  INTO dbo.[ThucChayTemp_TinhLai]
							( ThucChayID
							, SoHopDong
						...
						--Luu log dl thucchaytemp haidh 20241004
							   ...
							   ,[LastModifiedAt])
								...
								SELECT 0 AS HopDongChiTietREF,0 AS HopDongFK, DmSanPhamREF, getdate(),0 LoaiLog, 
								N'[sp_TC_DoiTruVaTinhLai_CPM_V2]: ' + SoHopDong + ',sobanghi: ' 
								+ Convert(nvarchar(10),count(SoHopDong)) 
								+ ',hopdongchitietref :' +  Convert(nvarchar(10),@pHopDongChiTietID)
								 AS [ContentLog], 
								N'haidh' [CreatedBy],
								GETDATE() [CreatedAt],
								N'haidh' [LastModifiedBy],
								GETDATE() [LastModifiedAt]
								FROM [ThucChayTemp_TinhLai]
								group by SoHopDong, DmSanPhamREF

					DECLARE Record_Cursor_v9 CURSOR
					FOR
						SELECT DISTINCT
								A.SoHopDong
							  , A.TypeProduct
						...
						FROM    ( SELECT    tct.SoHopDong
										  ...
										  , tct.DmBannerREF
										  , 'Y' HDLechGiaYN
								  FROM      dbo.[ThucChayTemp_TinhLai] tct
								) A
						ORDER BY A.SoHopDong
							  , A.TypeProduct	

					OPEN Record_Cursor_v9

			-- Perform the first fetch.
					FETCH NEXT FROM Record_Cursor_v9 INTO @SoHopDong, @TypeProduct, @DmWebsiteREF, @TenWebsite, @DmBannerREF, @HDLechGiaYN
			
					WHILE @@FETCH_STATUS = 0
						BEGIN
							 EXEC [dbo].[ThucChay_InsertThucChayDaTinh_DoiTruVaTinhLai_CPM_V2] 
					...
					CLOSE Record_Cursor_v9
					DEALLOCATE Record_Cursor_v9
		...
		BEGIN
           ...
           ,[LastModifiedAt])
			...
			DECLARE Record_Cursor_v10 CURSOR
			FOR
				SELECT DISTINCT
						A.SoHopDong
						, A.TypeProduct
			...
			OPEN Record_Cursor_v10

			-- Perform the first fetch.
			FETCH NEXT FROM Record_Cursor_v10 INTO @SoHopDong, @TypeProduct, @DmWebsiteREF, @TenWebsite, @DmBannerREF, @HDLechGiaYN
			...
			WHILE @@FETCH_STATUS = 0
				BEGIN
						...
						,[LastModifiedBy]
						,[LastModifiedAt])
		   				SELECT @pHopDongChiTietID AS HopDongChiTietREF,0 AS HopDongFK, @DmSanPhamREF as DmSanPhamREF, getdate(),0 LoaiLog, 
						N'[sp_TC_DoiTruVaTinhLai_CPM_V2-> [ThucChay_InsertThucChayDaTinh_DoiTruVaTinhLai_CPM_V2]]: ' + @pSoHopDong 
						+ ',@TypeProduct :' +  Convert(nvarchar(10),@TypeProduct)
						+ ',@DmWebsiteREF :' +  Convert(nvarchar(10),@DmWebsiteREF)
						+ ',@TenWebsite :' +  Convert(nvarchar(10),@TenWebsite)
						+ ',hopdongchitietref :' +  Convert(nvarchar(10),@pHopDongChiTietID)
						+ ',@NgayTinh :' +  Convert(nvarchar(10),@NgayThucHien)
							AS [ContentLog], 
						N'haidh' [CreatedBy],
						GETDATE() [CreatedAt],
						N'haidh' [LastModifiedBy],
			...
			CLOSE Record_Cursor_v10
			DEALLOCATE Record_Cursor_v10
