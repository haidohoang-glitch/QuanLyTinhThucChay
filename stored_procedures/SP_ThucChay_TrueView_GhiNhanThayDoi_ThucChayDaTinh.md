# Stored Procedure: `ThucChay_TrueView_GhiNhanThayDoi_ThucChayDaTinh`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-04-15 10:06:55.163000
- **Ngày sửa cuối**: 2025-04-15 10:07:58.467000
...
CREATE  PROCEDURE [dbo].[ThucChay_TrueView_GhiNhanThayDoi_ThucChayDaTinh]
    @NgayGhiNhan DATE,
...
BEGIN
	...
	WHERE	CONVERT(date,NgayThucHien) = @NgayGhiNhan
			...
			AND DotChayHopDong IN ( N'Đối trừ TrueView', N'Tính lại TrueView')
    ...
    CREATE TABLE #DmPBThayDoi (
					...
					ChietKhau_BF FLOAT,
					SoluongHD_BF FLOAT,
					SoluongGhiNhanDaTinh INT,
					SoluongLechTreoHaDaTinh INT,
					SoLuongKMDaTinh INT,
					TongSoLuongThucChayDenHT INT,
					ThanhTienSauCKThayDoi FLOAT,
					ThanhTienLechTreoHaThayDoi FLOAT,
					ThanhTienKhuyenMaiThayDoi FLOAT,
	...
	BEGIN
		...
		OUTER APPLY (SELECT TOP 1 SoLuong 
		...
		WHERE hdct.soluong = ISNULL(l.SoLuong, hdct.SoLuong) AND dm.LoaiThayDoi = 1
		...
		BEGIN
			...
			FROM ABM_Data_ThucChay.dbo.HopDongChiTiet hdct
			INNER JOIN ABM_Data_ThucChay.dbo.HopDong hd ON hdct.HopDongFK = hd.HopDongID 
			WHERE    hdct.DmSanPhamREF IN (240,733) AND
					 NOT ( hdct.DmLoaiREF IN ( 13, 42 ) OR hdct.DmLoaiBannerREF IN (17, 18)) AND
					 hdct.DonViTinhREF IN (31, 32) AND 
	...
	WHERE   CONVERT(DATE,tc.NgayThucHien) < @NgayGhiNhan
...
SELECT	  NEWID()
		...
		, DmViTriREF
		, TenViTri
		, N'Đối trừ TrueView' AS DotChayHopDong
...
GROUP BY  tcdt.HopDongID
	...
	DECLARE @Thucchay_TrueView_DmTinhlai DataType_Thucchay_CPMthuan_DmTinhlai
	INSERT INTO @Thucchay_TrueView_DmTinhlai
	(
	    HopDongChiTietREF,
		NgayThucHien,
		DmBannerREF,
	...
	EXEC [dbo].[ThucChay_TrueView_DoiTruTinhLai_ThucChayDaTinh] @Thucchay_TrueView_DmTinhlai
