# Function: `GetThucChayLabelFilterString_v2`

- **Loại**: SQL_SCALAR_FUNCTION
- **Ngày tạo**: 2014-07-10 23:06:57.257000
- **Ngày sửa cuối**: 2014-10-14 10:39:35.107000
...
-- =============================================
CREATE FUNCTION [dbo].[GetThucChayLabelFilterString_v2]
(
...
BEGIN	
	...
	set @FilterSQLCommand = 'CONVERT(DATE,tcdt.NgayThucHien) Between ' + @DauNhay + Convert(nvarchar(50),@StartDate) + @DauNhay + ' and '+ @DauNhay + Convert(nvarchar(50),@EndDate)+@DauNhay
	...
	IF (@TenKhachHangList IS NOT NULL AND @TenKhachHangList <> '' )
		SET @FilterSQLCommand += ' AND hd.DmKhachHangREF IN (' + @TenKhachHangList + ')'		
		
	IF @DonViTinhList <> '' AND @DonViTinhList IS NOT NULL
		SET @FilterSQLCommand += ' AND tcdt.DonViTinh IN (' + @DonViTinhList + ')';	 
			
	-- Kiểm tra Admin trong bảng AdminGroup
	-- 2, 34			Admin Thực chạy
	-- 106				thunguyenthi
	-- 103 ,151 ,152	Quản lý nhãn
	IF (NOT EXISTS(SELECT UserName FROM AdminPermistionUsers WHERE UserName = @TenDangNhap))
	BEGIN			
				...
				IF(@IsViewAll = 0)
					BEGIN
						SET @FilterSQLCommand += ' AND dbo.ReplaceStringEmpty(hd.NhanHopDong) IN 
													(SELECT dbo.ReplaceStringEmpty(dnh.TenNhanHang) FROM PhanQuyenNhanHang pqnh
														INNER JOIN AdminUser au ON pqnh.OxUserREF = au.OxUserREF
														INNER JOIN DmNhanHang dnh ON dnh.DmNhanHangID = pqnh.DmNhanHangREF
														WHERE 
														au.Username = ' + @DauNhay + @TenDangNhap + @DauNhay + ')'		
					END							
			END			 	
	END
