# Stored Procedure: `ThucChay_UpdateGiaTriThayDoiThucChayDaTinh_TMDTByHopDongChiTiet`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-07-01 11:41:41.567000
- **Ngày sửa cuối**: 2014-11-19 12:16:59.500000
...
-- =============================================
--exec [dbo].[ThucChay_UpdateGiaTriThayDoiThucChayDaTinh_TMDTByHopDongChiTiet] 25282,'QC1560414',56778,'2014-06-06'

CREATE PROCEDURE [dbo].[ThucChay_UpdateGiaTriThayDoiThucChayDaTinh_TMDTByHopDongChiTiet] 
...
BEGIN
	...
	DECLARE @ChietKhau FLOAT, @SoLuongHT INT, @DonGia FLOAT, @DonGiaHT FLOAT, @HopDongChiTietThayDoiGia INT, @HopDongChiTietThayDoiCK INT, @HopDongChiTietThayDoiSL INT
	...
	SELECT @SoLuongHT = hdct.SoLuong, @DonViTinh = hdct.DonViTinh, @DonGiaHT = hdct.DonGia
	...
	SET @DonGiaTheoDVTinhHT = ISNULL(dbo.ThucChay_GetDonGiaChuanTheoDonViTinh_TinVIP(@SoLuongHT,@DonViTinh,@DonGiaHT,@NgayThucHien, @NgayThucHien, @HopDongChiTietID),0)
	...
	--NEU CO THAY DOI VE CHIET KHAU
	IF(@HopDongChiTietThayDoiSL <>0)
	BEGIN
		SET @CONTENT_LOG = @CONTENT_LOG + N'(HĐ Thay đổi số lượng: ' + CONVERT(NVARCHAR(30),@SoLuong) + '->' + CONVERT(NVARCHAR(30),@SoLuongHT) + ');'
		SET @NGUON_LOG = @NGUON_LOG + 'Table:HopDongChiTiet'+ CONVERT(NVARCHAR(30),@HopDongChiTietID)
	END
	
	--CHECK HD CO THAY DOI SOLUONGTHUCCHAY HD
	SET @SoLuongThucChayHT =  isnull(dbo.[ThucChay_GetSoLuongThucChay_TMDT](@SoLuongHT ,@DonViTinh ,@HopDongChiTietID ,@NgayThucHien ),0)
	...
	BEGIN
			...
			BEGIN
				...
				VALUES
				  (NEWID(),
					...
					,@NGUON_LOG,'TMDT',	'ThucChay',	GETDATE(),
					'ThucChay',GETDATE(),0,
					0,0
				  );
		...
		ELSE
			BEGIN
				...
				EXEC [dbo].[ThucChay_InsertThucTreoThayDoi_TMDT] @HopDongChiTietID,@NgaythucHien,@GiaTriThayDoi
			END	
	END
	SELECT 1
END
