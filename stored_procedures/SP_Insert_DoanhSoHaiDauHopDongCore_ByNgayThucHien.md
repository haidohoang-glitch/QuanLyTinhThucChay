# Stored Procedure: `Insert_DoanhSoHaiDauHopDongCore_ByNgayThucHien`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-04-22 16:42:58.227000
- **Ngày sửa cuối**: 2015-04-22 16:42:58.227000
...
-- =============================================
--EXEC [dbo].[Insert_DoanhSoHaiDauHopDongCore_ByNgayThucHien] '2013-01-01'

CREATE PROCEDURE [dbo].[Insert_DoanhSoHaiDauHopDongCore_ByNgayThucHien]
...
BEGIN
	...
	, hdct.DmBannerREF, hdct.DonViTinh, hdct.DeletedStatus, hdct.DmChuyenMucREF
	 
	FROM HopDong hd
		INNER JOIN HopDongChiTiet hdct ON hd.HopDongID = hdct.HopDongFK
		INNER JOIN KhachHangThongTinChung khttc ON hd.DmKhachHangREF = khttc.KhachHangThongTinChungID
		WHERE (
			       (convert(date,hdct.LastModifiedAt) = @NgayThucHien)
	...
	, @DmBannerREF_New, @DonViTinh_New, @DeletedStatus_HDCT_New,@DmChuyenMuc_New
			
	WHILE @@FETCH_STATUS = 0
		BEGIN
			PRINT @HopDongChiTietID
			SET @IsPhatSinhGiaTriTD = [dbo].[fn_IsPhatSinhGiaTriTD_DoanhSoHaiDauHopDongCore]
			(
				@HopDongID,
				@HopDongChiTietID ,
				@SoHopDong_New ,
			...
			BEGIN
				...
				EXEC [Insert_DoanhSoHaiDauHopDongCore_PhatSinhTrongKy] @NgayThucHien, @HopDongID,@HopDongChiTietID ,@DmMaHopDongREF_New 
			END
			--@IsPhatSinhGiaTriTD <> 0 PHAT SINH GIA TRI THAY DOI
			ELSE
				BEGIN
				IF @IsPhatSinhGiaTriTD = 2 SET @LyDoThayDoi ='Hop dong bi xoa'
				IF @IsPhatSinhGiaTriTD = 3 SET @LyDoThayDoi ='Trang thai isBanCung bi thay doi'
					--PRINT @HopDongChiTietID
					EXEC [dbo].[Insert_DoanhSoHaiDauHopDongCore_PhatSinhGiaTriThayDoi] @NgayThucHien, @HopDongID, @HopDongChiTietID, @DmMaHopDongREF_New,@LyDoThayDoi
				END
		FETCH NEXT FROM Record_Cursor INTO @DmMaHopDongREF_New, @HopDongID, @SoHopDong_New, @DmNhanVienREF_New, @PhongBanREF_New
	, @BoPhanREF_New, @NhomREF_New, @DmKhachHangREF_New, @NgayDanhSo_New, @TrangThaiHopDong_New
	, @HopDongChiTietID, @DmHinhThucQuangCaoREF_New, @DmSanPhamREF_New, @DmWebsiteREF_New
