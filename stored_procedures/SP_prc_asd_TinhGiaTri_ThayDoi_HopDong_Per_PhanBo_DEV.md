# Stored Procedure: `prc_asd_TinhGiaTri_ThayDoi_HopDong_Per_PhanBo_DEV`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2024-03-11 16:04:32.037000
- **Ngày sửa cuối**: 2024-10-25 11:50:48.823000
...
EXEC [dbo].[prc_asd_TinhGiaTri_ThayDoi_HopDong_Per_PhanBo_dev]
	@NgayThucHien = '2024-10-24',
	@HopDong_ID = 1060012,
	@PhanBo_ID = 729831,
	@DmSanPham_ID = 585,
	@TenSanPham = N'AdX',
	@GiaTriTaiThoiDiemTinhThucChay = 18365741,
	@GiaTriGanNhatThayDoi = 26000000
*/
CREATE PROCEDURE [dbo].[prc_asd_TinhGiaTri_ThayDoi_HopDong_Per_PhanBo_dev]
...
BEGIN
	...
	PRINT 'SP'
	--SELECT @GiaTriTaiThoiDiemTinhThucChay
	--SELECT @GiaTriGanNhatThayDoi
	...
	BEGIN
		...
		IF(EXISTS( SELECT top 1 HopDongChiTietREF  from [dbo].ThucChayAdmarket_HopDong_online 
			...
			BEGIN
				PRINT N'START TANG GIA TRI PHAN BO'
					----PRINT 'START'
	...
	BEGIN
		...
		BEGIN
			PRINT N'START GIAM GIA TRI PHAN BO'
			DECLARE @DmWebsiteREF INT, @TenWebsite NVARCHAR(50),@v_TenViTri nvarchar(50)
				...
				SET @GiaTriThayDoi = @GiaTriThucChayPhanBo - @GiaTriTaiThoiDiemTinhThucChay
			...
			select @HopDong_ID		
			select @PhanBo_ID -- cần sửa
			select @DmSanPham_ID  -- cần sửa ADX: 585, Viewplus 628
			select @User_Name  -- cần sửa
			select @v_DmViTriREF
			select @v_TenViTri
			select @DmWebsiteREF
			select @TenWebsite
			select @GiaTriThayDoi  -- cần sửa
			select @v_GhiChu
