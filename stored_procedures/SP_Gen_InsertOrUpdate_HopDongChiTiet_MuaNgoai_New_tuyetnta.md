# Stored Procedure: `Gen_InsertOrUpdate_HopDongChiTiet_MuaNgoai_New_tuyetnta`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-11-02 18:44:50.847000
- **Ngày sửa cuối**: 2020-11-03 09:35:41.583000
...
```sql
/*
EXEC [dbo].[Gen_InsertOrUpdate_HopDongChiTiet_MuaNgoai_New_tuyetnta]
*/
CREATE PROCEDURE [dbo].[Gen_InsertOrUpdate_HopDongChiTiet_MuaNgoai_New_tuyetnta]
@hdct int 
As 	
BEGIN
	...
	, '' AS AttachFile
	, ct.CreationTime
	, ISNULL((SELECT TOP (1) t.Username FROM [192.168.23.150].PMS.dbo.V_NhanVien_DangNhap t WHERE t.ID = ct.CreatorUserId ORDER BY t.ID),'') Created_By
	...
	, ISNULL((SELECT TOP (1) t.Username FROM [192.168.23.150].PMS.dbo.V_NhanVien_DangNhap t WHERE t.ID = ct.LastModifierUserId ORDER BY t.ID),'') AS LastModified_By
	, ct.IsDeleted
	, ISNULL(ct.ThanhTien,0) AS ThanhTienBanSauCK
	, ISNULL(ct.ChenhLech,0) AS ThanhTienLaiSauCK
	, 0 Statuss
	 FROM (
		SELECT dt.Id, dt.LastModificationTime FROM [192.168.23.150].PMS.dbo.B_DuToan dt 
	 ...
	 (
		...
		FROM [192.168.23.150].PMS.dbo.B_DuToan_ChiTiet ct WHERE ct.IsDeleted = 0 AND  ISNULL(ct.PhanBoId,0) <> 0
		--AND ct.LastModificationTime >= @NgayThucHien
		and ct.PhanBoID = @hdct
	 ...
	 (SELECT cth.IsDeleted, cth.id, cth.ThanhTienSauCK,cth.Vat, cth.PhaiTraNhaCungCap,
			 ...
			 FROM [192.168.23.150].PMS.dbo.B_DuToan_ChiTiet_HopDong cth WHERE 1=1  
		AND cth.IsDeleted = 0
    ...
    CreationTime,
    ISNULL((SELECT TOP (1) t.Username FROM [192.168.23.150].PMS.dbo.V_NhanVien_DangNhap t WHERE t.ID = CreatorUserId ORDER BY t.ID),'') AS Created_By,
    LastModificationTime,
    ISNULL((SELECT TOP (1) t.Username FROM [192.168.23.150].PMS.dbo.V_NhanVien_DangNhap t WHERE t.ID = LastModifierUserId ORDER BY t.ID),'') AS LastModified_By,
	TrangThai, --0 : mới,1: gửi duyệt;2 duyệt thanh toán, 3 -- gửi duyệt thực chạy, --4 duyệt thực chạy
    IsDeleted,
	NgayDuyet,
	ISNULL((SELECT TOP (1) t.tendangnhap FROM [192.168.23.150].PMS.[dbo].[V_NguoiDung] t WHERE t.OxUserID = NguoiDuyet ORDER BY t.OxUserID),'') AS NguoiDuyet,
	NgayChot,
	ISNULL((SELECT TOP (1) t.tendangnhap FROM [192.168.23.150].PMS.[dbo].[V_NguoiDung] t WHERE t.OxUserID = NguoiChot ORDER BY t.OxUserID),'') AS NguoiChot,
	0 AS Statuss FROM [192.168.23.150].PMS.dbo.B_QuanLyThucChay tc
	--WHERE LastModificationTime >= @NgayThucHien_ThucChayMN
	where tc.PhanboId= @hdct
	--select top 1 * from [192.168.23.150].PMS.dbo.B_QuanLyThucChay
