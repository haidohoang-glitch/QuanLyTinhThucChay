# Stored Procedure: `ThucChayDaTinh_SponsorPost_UpdateGiaTriThayDoi_ManualByHopDongChiTietID`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-11-12 11:50:14.203000
- **Ngày sửa cuối**: 2015-04-10 11:13:10.010000
...
-- =============================================
-- 
-- EXEC [dbo].[ThucChayDaTinh_SponsorPost_UpdateGiaTriThayDoi_ManualByHopDongChiTietID] '2015-03-26', 'DT3161014',67738,381

CREATE PROCEDURE [dbo].[ThucChayDaTinh_SponsorPost_UpdateGiaTriThayDoi_ManualByHopDongChiTietID] 
...
BEGIN
	...
	--FROM ThucChayDaTinhSponsorPost
	FROM ThucChayDaTinh
	WHERE
		SoHopDong =@SoHopDong
	...
	BEGIN
		...
		FROM	ThucChayDaTinh AS tcdt
		    	--ThucChayDaTinhSponsorPost AS tcdt
		...
		--SELECT @GiaTriHopDong =(SELECT ISNULL(SUM(tcdtsp.ThanhTienSauTrietKhauThucChay + tcdtsp.GiaTriThayDoi),0) 
		--                          FROM ThucChayDaTinhSponsor tcdtsp WHERE tcdtsp.SoHopDong = @SoHopDong
		--AND tcdtsp.HopDongChiTietREF = @HopDongChiTietREF AND tcdtsp.NgayThucHien <= @NgayThucHien
		--)	
...
ThucChayDaTinh AS tcdt
		...
		BEGIN
			...
			--FROM ThucChayDaTinhSponsorPost AS tcdt
			FROM ThucChayDaTinh AS tcdt
			WHERE tcdt.SoHopDong = @SoHopDong
				AND tcdt.DmSanPhamREF = @DmSanPhamREF
...
--DELETE FROM ThucChay_LogNNTinhGiaTriThayDoi WHERE HopDongChiTietREF = 49895
--AND NgayThucHien = '2014-12-31'
--AND CreatedAt>'2015-01-15 15:00'

--DELETE FROM ThucChayDaTinhMobile WHERE HopDongChiTietREF = 49895
--AND NgayThucHien = '2014-12-31'
--AND CreatedAt>'2015-01-15 15:00'
```
