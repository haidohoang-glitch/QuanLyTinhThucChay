# Stored Procedure: `ThucChay_GGFB_GhiNhanThayDoi_dev`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-12-24 11:27:14.483000
- **Ngày sửa cuối**: 2025-12-24 15:02:06.763000
...
--EXEC [dbo].[ThucChay_GGFB_GhiNhanThayDoi_dev]
...
/*
exec [dbo].[ThucChay_GGFB_GhiNhanThayDoi_dev]
	@NgayGhiNhan = N'2025-12-23',
	@NgayCheckThayDoi  = N'2025-12-23',
	@NgayDanhSoGioiHan = N'2022-04-18'
	--@SoHopDong  = N'QC3191225',
	--@HopDongChiTietID = 773808
*/



CREATE PROCEDURE [dbo].[ThucChay_GGFB_GhiNhanThayDoi_dev]
	@NgayGhiNhan DATE,
 ...
 --         AND (@SoHopDong IS NULL OR tcdt.SoHopDong = @SoHopDong) 
	...
	BEGIN
		...
		WHERE CONVERT(date,OD.LastModificationTime) = @NgayCheckThayDoi
			  ...
			  and od.Contract_Detail_Id = 773808
...
select 1
	select * from #DmPBThayDoi d
	where d.HopDongChiTietID = 773808
		...
		WHERE dm.LoaiThayDoi = 3 AND dm.Operator_Type = 1 AND 
			 (dm.IsActive_hdct = 0 OR							-- TH phân bổ sau không active
			  dm.HopDongChiTietID = dm2.HopDongChiTietID OR		-- TH phân bổ trước và sau không TĐ
			  dm.IsActive_Order = 0 OR 							-- TH tại thời điểm hiện tại, order bị hủy	
			  dm.IsActive_Operator = 0)	
		and dm.HopDongChiTietID = 773808
	...
	select * from #DmPBThayDoi dm
	where dm.HopDongChiTietID = 773808
	...
	BEGIN
		...
		WHERE @SoHopDong IS NULL 
			  ...
			  AND L.IsDeleted IS NOT NULL
		select 'TH0'
		select * from #DmPBThayDoi g
		where g.HopDongChiTietID = 773808
	...
	select * from #DmTinhLai_TheoOrder
