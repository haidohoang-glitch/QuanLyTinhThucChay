# Stored Procedure: `ThucChay_GoogleFacebookInsertGTTD4_Doannv`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-07-16 14:53:29.093000
- **Ngày sửa cuối**: 2016-10-25 10:28:25.313000
...
-- =============================================
CREATE PROCEDURE [dbo].[ThucChay_GoogleFacebookInsertGTTD4_Doannv]
...
BEGIN
    ...
    -- Số lượng input > Số lượng thực chay, Thành tiền input < thành tiênf thực chạy
    -- Update giá trị thay đôi cho số lượng
   ...
   --- Kiểm tra đơn vị tính
	...
	-- xoa du lieu
	DELETE FROM ThucChayDaTinh where 
	...
	ORDER BY hdct.HopDongChiTietID ASC
	...
	BEGIN   
		  ...
		  SET @SoLuongHopDong = ISNULL((SELECT SUM(SoLuong)
		                                 FROM HopDongChiTiet WHERE HopDongFK = @HopDongID
										  AND TenSanPham = @DmSanPhamREF
										  AND DonViTinh = @DonViTinhThucChay),0)
	...
	IF --@SoLuongThucChay > @SoLuongThucChayHopDong --OR  
	   round(@ThanhTienThucChay,-1) > round(@ThanhTienThucChayHopDong,-1) 
	-- kiem tra theo phan bo 	   
    IF --@SoLuongPhanBo > @SoLuongPhanBoThucChay --OR 
       round(@ThanhTienPhanBo,-1) > round(@ThanhTienPhanBoThucChay,-1)
    BEGIN
    	--SET @SoLuongCheck = @SoLuongThucChay - @SoLuongThucChayHopDong
    	SET @ThanhTienCheck = @ThanhTienThucChay - @ThanhTienThucChayHopDong
    	IF @ThanhTienCheck < (@ThanhTienPhanBo - @ThanhTienPhanBoThucChay)
    	   SET @ThanhTienThucChayInsert = @ThanhTienCheck
    	ELSE  SET @ThanhTienThucChayInsert = (@ThanhTienPhanBo - @ThanhTienPhanBoThucChay)
    	
    	--IF @SoLuongCheck < (@SoLuongPhanBo - @SoLuongPhanBoThucChay)
    	--   SET @SoLuongThucChayInsert = @SoLuongCheck
    	--ELSE  SET @SoLuongThucChayInsert = (@SoLuongPhanBo - @SoLuongPhanBoThucChay)
    	-- Insert dữ liệu
	...
	--- câp nhâtj thành tiên
	DECLARE db_cursorinsert1 CURSOR FOR  
	SELECT 
	hd.HopDongID,
	hdct.HopDongChiTietID,
	hdct.SoLuong,
	...
	OPEN db_cursorinsert1   
	
	FETCH NEXT FROM db_cursorinsert1 INTO @HopDongID,@HopDongChiTietID,@SoLuongPhanBo,@ThanhTienPhanBo
	...
	BEGIN   
		  ...
		  SET @ThanhTienThucChayHopDong = ISNULL((SELECT SUM(ThanhTienSauTrietKhauThucChay + GiaTriThayDoi)
		                                 ...
		                                 AND DmLoaiBannerREF = @DmLoaiBannerREF
										  AND TrangThaiHopDong <> 3),0)
	-- kiem tra theo hop dong
	IF --@SoLuongThucChay > @SoLuongThucChayHopDong 
	    @SoLuongPhanBoThucChay > 0
	-- kiem tra theo phan bo 	   
    IF --@SoLuongPhanBo > @SoLuongPhanBoThucChay 
       @SoLuongThucChay < @SoLuongThucChayHopDong
    BEGIN
	...
	SELECT DISTINCT
		...
		'' PheDuyetBy,
		'' PheDuyetAt,
		CASE WHEN @TrangThai = 1 THEN
		-@SoLuongThucChayInsert
		ELSE -1  END SoLuongThayDoi,
		0 as SoLuongKMThayDoi,
		0 as GiaTriKMThayDoi,
		'' as GhiChu
	FROM
	...
	CLOSE db_cursorinsert1   
	DEALLOCATE db_cursorinsert1
END
