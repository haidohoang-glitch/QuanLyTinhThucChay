# Stored Procedure: `ThucChay_InsertThucChayDaTinh_CPM_DonViBai_Admatic_ThucTreo`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2021-06-23 11:24:00.430000
- **Ngày sửa cuối**: 2023-05-29 16:08:27.223000
...
```sql
--EXEC [ThucChay_InsertThucChayDaTinh_CPM_DonViBai_Admatic_ThucTreo]
-------------------------------------------------------------
CREATE PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_CPM_DonViBai_Admatic_ThucTreo] 
...
SET @NgayGioiHanTinh = '2021-01-01'
   ...
   OUTPUT inserted.ThucChayDaTinhID, inserted.HopDongID, inserted.HopDongChiTietREF, inserted.DotChayBooking, inserted.DmBannerREF INTO @Table_ouput
	...
	ISNULL(TD.DonGia*(100-TD.ChietKhau)/100,0) AS ThanhTienSauTrietKhauThucChay,	
	...
	(CASE when ((TD.IsKhuyenMai=1) OR (TD.ChietKhau = 100)) then TD.SoLuongThucChay
		else 0
	  END
	...
	0 TongClickThucChay,
	0 TongSoBaiViet,
	(CASE when ((c.IsKhuyenMai=0) OR (c.ChietKhau <> 100)) then 1
		else 0
	  END
	) AS SoLuongThucChay, --MOI BANNER TINH LA 1 DV SO LUONG
	@NgayThucHien AS NgayThucHien,
	0 as GiaTriThayDoi,
	C.DonGia AS ThanhTienThucChayTruocTrietKhau
	FROM 
	(
