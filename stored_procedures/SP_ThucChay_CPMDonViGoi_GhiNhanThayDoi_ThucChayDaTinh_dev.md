# Stored Procedure: `ThucChay_CPMDonViGoi_GhiNhanThayDoi_ThucChayDaTinh_dev`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-08-25 15:20:39.623000
- **Ngày sửa cuối**: 2025-11-05 10:15:08.717000
...
```sql
/*
EXEC dbo.[ThucChay_CPMDonViGoi_GhiNhanThayDoi_ThucChayDaTinh_dev] @NgayGhiNhan = '2025-09-30',
                                                            @SoHopDong = N'P0710725',            -- nvarchar(50)
                                                            @HopDongChiTietID = 760787              -- int

*/
     

--DECLARE @DmInterger TABLE (ID INT)
CREATE PROCEDURE [dbo].[ThucChay_CPMDonViGoi_GhiNhanThayDoi_ThucChayDaTinh_dev]
    @NgayGhiNhan DATE,
...
AS
BEGIN
	DECLARE @GhiChuDoiTru NVARCHAR(2000) =  N'Đối trừ: SP tối ưu [dbo].[ThucChay_CPMDonViGoi_GhiNhanThayDoi_ThucChayDaTinh] do '
	...
	BEGIN
		...
		INNER JOIN  dbo.ThucChayHopDongChiTietAndBanner_Native_Ads tchdctab ON b.DmBannerID = tchdctab.DmBannerID 
		INNER JOIN  dbo.HopDong hd ON hd.HopDongID = tchdctab.HopDongREF
		WHERE tchdctab.HopDongChiTietREF NOT IN (SELECT HopDongChiTietREF FROM #DmPBThayDoi dm )
			 ...
			 AND NOT EXISTS(SELECT top (1) iv.HopDongChiTietREF FROM  dbo.DmThongTinHopDongBanInventory iv 
						    WHERE iv.HopDongChiTietREF = tchdctab.HopDongChiTietREF )  
			 AND NOT EXISTS (SELECT top (1) tl.HopDongChiTietID FROM  dbo.GhiNhanThanhLy tl 
	...
	INNER JOIN  dbo.ThucChayHopDongChiTietAndBanner_Native_Ads tchdctab ON tchdctab.DmBannerID = CONVERT(NVARCHAR(50),tc.DmBannerID) 
																					  AND tchdctab.DeletedStatus = 0
	...
	SELECT tchdct.ThucChayHopDongChiTietID, tchdct.HopDongChiTietREF, tchdct.DmWebsiteREF, tchdct.TenWebsite,
						tchdct.DmSanPhamREF, tchdct.SoLuongThucChay,tchdct.DmBannerREF,
	                    hd.NgayDanhSoHopDong,hd.SoHopDong, 
						DmDonvitinhREF = (CASE WHEN tchdct.DmDonViTinhREF =1 THEN N'VIEW'
											   WHEN tchdct.DmDonViTinhREF =2 THEN N'CLICK'
											   WHEN tchdct.DmDonViTinhREF =32 THEN N'TRUE VIEW'
				 ...
				 WHERE hd.TrangThaiHopDong != 3 AND 
					   ...
					   tchdct.DmDonViTinhREF = 7 AND --Don vi cua treo = BAI
	...
	UPDATE temp
	SET  	temp.SoLuongThucChay_GhiNhan = IIF((temp.ChietKhau <> 100 AND temp.DonGiaSauCK <> 0), temp.ThanhTienTC_GhiNhan/temp.DonGiaSauCK, 0),
			temp.SoLuongKM_GhiNhan = IIF(temp.ChietKhau = 100 AND temp.DonGiaKM <> 0 , temp.ThanhTienKM_GhiNhan/temp.DonGiaKM, 0),
			temp.SoLuongLechTreoHa = (CASE WHEN temp.ChietKhau = 100 AND temp.DonGiaKM <> 0 THEN temp.SoLuongKM - temp.ThanhTienKM_GhiNhan/temp.DonGiaKM
									WHEN temp.ChietKhau <> 100 AND temp.DonGiaSauCK <> 0 THEN temp.SoLuongThucChay - temp.ThanhTienTC_GhiNhan/temp.DonGiaSauCK
									ELSE 0
									END),----IIF(ChietKhau = 100, SoLuongKM - ThanhTienKM_GhiNhan/DonGiaKM, SoLuongThucChay - ThanhTienTC_GhiNhan/DonGiaSauCK) ,
										--ThanhTienLechTreoHa = IIF(ChietKhau = 100, ThanhTienKM - ThanhTienKM_GhiNhan, ThanhTienSauCK - ThanhTienTC_GhiNhan)
			temp.ThanhTienLechTreoHa = IIF(temp.ChietKhau = 100, temp.ThanhTienKM - temp.ThanhTienKM_GhiNhan, (temp.ThanhTienSauCK/(100-temp.ChietKhau))*100 - (temp.ThanhTienTC_GhiNhan/(100-temp.ChietKhau))*100)
	FROM #DmTinhLai temp
...
SELECT	  NEWID()
		...
		,@GhiChuDoiTru + dm.LyDo AS GhiChu
...
GROUP BY  tcdt.HopDongID
	...
	SELECT * FROM @Thucchay_CPMDonViGoi_DmTinhlai
