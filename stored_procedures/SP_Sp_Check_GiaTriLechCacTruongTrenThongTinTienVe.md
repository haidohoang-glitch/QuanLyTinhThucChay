# Stored Procedure: `Sp_Check_GiaTriLechCacTruongTrenThongTinTienVe`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-02-23 11:15:00.393000
- **Ngày sửa cuối**: 2017-03-11 09:42:40.590000
...
```sql
--[Sp_Check_GiaTriLechCacTruongTrenThongTinTienVe] '2014-01-01',''
CREATE PROCEDURE [dbo].[Sp_Check_GiaTriLechCacTruongTrenThongTinTienVe]
...
AS
    BEGIN
        ...
        SET @SQL = 'select A.id,hd_id,giathanhtoan,ngaythanhtoan,CAST(IFNULL(LogTime,''''2010-01-01'''') AS DATETIME) AS LastModifiedAt from hdcn_congno A
					WHERE 1=1 AND  ((`ngaythanhtoan` <> ''''0000-00-00'''') AND (`ngaythanhtoan` <> ''''0001-01-01'''')) AND CAST(IFNULL(LogTime,''''2010-01-01'''') AS DATE) between '''''
        ...
        SET @TotalRows = ( SELECT   MAX(STT)
                           ...
                           WHERE    DoiTuong = N'Thông tin tiền về'
                         )
        ...
        WHILE @x <= @TotalRows
            BEGIN
                ...
                WHERE   STT = @x
                        AND DoiTuong = N'Thông tin tiền về'
