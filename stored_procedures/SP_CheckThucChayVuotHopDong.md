# Stored Procedure: `CheckThucChayVuotHopDong`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-11-03 12:14:54.490000
- **Ngày sửa cuối**: 2018-05-04 16:08:58.853000
...
-- =============================================
/*
SELECT NhanHang, NgayThucHien, HopDongChiTietREF, DmSanPhamREF, SoHopDong FROM dbo.ThucChayDaTinh WHERE ISNUMERIC(NhanHang) = 0
AND NgayThucHien >='2017-01-01' AND HopDongID <> 0 AND NhanHang <> ''
ORDER BY NhanHang

check website
SELECT DmWebsiteREF, tc.TenWebsite, wb.DmWebsiteReportingdbID, SUM(tc.ThanhTienSauTrietKhauThucChay+tc.GiaTriThayDoi)   
FROM dbo.ThucChayDaTinh tc
LEFT JOIN  DmWebsiteReportingdb wb ON tc.DmWebsiteREF = tc.DmWebsiteREF
AND tc.TenWebsite = wb.TenWebsite
where tc.DmWebsiteREF <> 0
--AND tc.NgayThucHien >='2016-01-01'
AND DmWebsiteREF <> DmWebsiteReportingdbID
AND tc.TenWebsite NOT LIKE '%http%'  AND tc.TenWebsite <> 'googleusercontent.com'
GROUP BY DmWebsiteREF, tc.TenWebsite, wb.DmWebsiteReportingdbID
HAVING round( SUM(tc.ThanhTienSauTrietKhauThucChay+tc.GiaTriThayDoi),0) <> 0
...
--DELETE FROM dbo.ThucChayDaTinh WHERE DmSanPhamREF =585 AND NgayThucHien BETWEEN '2015-12-01' AND '2015-12-31'AND CreatedAt > '2017-01-01'

--1: check vuot theo hop dong QC790213,QC1170913,DT311013
--7: check sai, vuot theo tat ca thong tin hd sp cpd
--8: check sai, vuot theo tat ca thong tin hd sp tru cpd, admarket, cpm, google
--9: check sai, vuot theo tat ca thong tin hd sp admarket
/*
SELECT SoHopDong, NhanHang FROM dbo.ThucChayDaTinh WHERE ISNUMERIC(NhanHang) = 0 AND NgayThucHien = '2016-06-02'
AND SoHopDong <> '-'

SELECT SoHopDong, NhanHang FROM dbo.ThucChayDaTinhAdmarket WHERE ISNUMERIC(NhanHang) = 0 AND NgayThucHien = '2016-06-01'
AND SoHopDong <> '-'
*/
--INSERT INTO thucchaydatinh SELECT * FROM [ASD-SQLSVR_UUTP].ABM_Data_Release.dbo.ThucChayDaTinh WHERE HopDongID = 39191 AND DmSanPhamREF = 140 and hopdongchitietref = 87128 AND NgayThucHien = '2016-12-28'
CREATE PROCEDURE [dbo].[CheckThucChayVuotHopDong] 
...
BEGIN
	...
	FULL OUTER JOIN
	(
	SELECT B.HopDOngID, B.SoHopDong, B.tttc FROM( 
		SELECT B.HopDOngID, B.SoHopDong, SUM(B.tttc) tttc FROM
		(
		SELECT tcdt.HopDongID, tcdt.SoHopDong,round(SUM(tcdt.ThanhTienSauTrietKhauThucChay+ tcdt.GiaTriThayDoi),0) tttc
		...
		GROUP BY tcdt.HopDongID, tcdt.SoHopDong
		UNION ALL
		SELECT tcdt.HopDongID,tcdt.SoHopDong,round(SUM(tcdt.ThanhTienSauTrietKhauThucChay+ tcdt.GiaTriThayDoi),0) tttc
		  FROM ThucChayDaTinhAdmarket tcdt  where 1=1 AND tcdt.NgayThucHien <=@NgayThucHien
		  --AND tcdt.DmSanPhamREF NOT IN (247)
		    AND tcdt.TrangThaiHopDong <> 3
		GROUP BY tcdt.HopDongID, tcdt.SoHopDong
		)B GROUP BY B.HopDongID, B.SoHopDong
	)B)B
	ON A.HopDongID = B.HopDongID	
	AND A.SoHopDong = B.SoHopDong
	WHERE-- (abs(A.ThanhTien - B.tttc) > 5 AND A.ThanhTien < (B.tttc-5)) 
	--OR NOT (A.ThanhTien IS NOT NULL AND B.tttc IS NULL)
...
AND DmSanPhamREF NOT IN (141,637,305) 
 ...
 AND tcdt.Nam >=2014
  --tcdt.SoHopDong NOT IN ('QC800113','QC2190913','QC1811013','QC1041113','DT1180913','DT151113')
 -- and HopDongChiTietREF in (SELECT distinct HopDongChiTietREF FROM ThucChayDaTinhAdmarket WHERE YEAR(NgayThucHien)= 2014)
 GROUP BY --sohopdong,
...
AND DmSanPhamREF NOT IN (141,637,305) 
 ...
 --AND a.TenLoaiBanner = b.TenLoaiBanner
 ) 
