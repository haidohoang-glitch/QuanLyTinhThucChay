# Stored Procedure: `sp_TC_UpdateGiaTriThayDoiThucChayDaTinh_DonViGoi_v2_ByHopDongChiTiet`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2023-08-04 10:49:56.550000
- **Ngày sửa cuối**: 2023-08-04 14:41:36.790000
...
-- =============================================
/*
exec [dbo].[sp_TC_UpdateGiaTriThayDoiThucChayDaTinh_DonViGoi_v2_ByHopDongChiTiet] @NgayThucHien = '2021-06-30' , @HopDongChiTietREF = 123556
--NOTE: Ngaythuchien = Ngay co thay doi thong tin , khong ton tai du lieu phat sinh thuc chay cua phan bo nay sau ngay thuc hien
*/
CREATE  PROCEDURE [dbo].[sp_TC_UpdateGiaTriThayDoiThucChayDaTinh_DonViGoi_v2_ByHopDongChiTiet] 
...
AS
    BEGIN
        ...
        DECLARE  @count_HDCT INT , @SoLuongThucChayBF INT   , @GhiChu NVARCHAR(1000) = N''
        ...
        SET @SoLuongThucChayBF = 0
			...
			(
				...
				(
					...
					WHERE 1=1
					AND hdct.DmSanPhamREF IN (339, 240, 598, 342, 5056,821, 5133, 5299)
					AND convert(date,hdct.LastModifiedAt) = @NgayThucHien
				)tt
				INNER JOIN 
				(
						...
						AND hdcttd.DmSanPhamREF IN (339, 240, 598, 342, 5056, 733, 5299)---haidh comment them san pham 733 - nhieu sanpham cho donvigoi 2021-10-29)
						AND NOT (hdcttd.DmLoaiBannerREF = 18 OR hdcttd.DmLoaiREF IN (42,13))
						AND hdcttd.DeletedStatus = 0
				)hdct ON tt.HopDongChiTietREF = hdct.HopDongChiTietID AND (tt.DmSanPhamREF = hdct.DmSanPhamREF OR hdct.DmSanPhamREF = 733)
        ...
        WHILE @@FETCH_STATUS = 0
            BEGIN
                ...
                WHERE   CONVERT(DATE, NgayThucHien) = @NgayThucHien
						...
						AND DmSanPhamREF IN (339, 240, 598, 342, 5056,821, 5133, 5299)--CPM va Native Ads, OnImage
						--AND DotChayHopDong = N'CPM_DonViGoi' --comment vi chay nhieu san pham donvigoi
				...
				BEGIN
                    ...
                    END  
					--XU LY CHO NHOM SAN PHAM NATIVE ADS, ONIMAGE
					IF(EXISTS(SELECT tcdt.HopDongChiTietREF
						FROM dbo.ThucChayDaTinh tcdt
						WHERE convert(date,tcdt.NgayThucHien) <= @NgayThucHien
						AND tcdt.HopDongID = @HopDongREF
						AND tcdt.DmSanPhamREF IN (821, 5133)
						AND tcdt.HopDongChiTietREF = @HopDongChiTietID
						--AND tcdt.DotChayHopDong = N'CPM_DonViGoi'
						GROUP BY tcdt.HopDongChiTietREF HAVING sum(tcdt.SoLuongThucChay + tcdt.SoLuongThayDoi) <> 0
					))
					BEGIN
							...
							set @GhiChu = N'Native ads, ' + @GhiChu
							EXEC [dbo].[ThucChay_DoiTruVaTinhLai_CPM_Native_Ads_ByHopDongChiTiet] 
							@pSoHopDong = @SoHopDong,
							@pHopDongChiTietID = @HopDongChiTietID,
							@pStartDate =  @MinNgayThucChay,
				...
				BEGIN
					...
					BEGIN
						...
						SET @MinNgayThucChay = ISNULL((SELECT MIN(NgayThucHien) FROM dbo.Thucchaydatinh tcdt
						WHERE tcdt.HopDongChiTietREF = @HopDongChiTietID),'2001-01-01')
