# Stored Procedure: `ThucChay_GetDetailFilterCondition_v1`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-02-25 11:30:37.963000
- **Ngày sửa cuối**: 2014-10-14 10:39:54.813000
...
--exec [ThucChay_GetDetailFilterCondition]
--@PageIndex = 1,
--	@RecordCount = 10,
--	@GroupFieldName = N'TenWebsite',
...
CREATE PROCEDURE [dbo].[ThucChay_GetDetailFilterCondition_v1] 
	...
	BEGIN
		...
		SET @Sql = '
			SELECT TOP (' + CONVERT(NVARCHAR,@RecordCount) + ') 
				T.'+ @GroupFieldName+',T.ID,T.DonViTinh,T.HopDongChiTietREF,T.LechBooking,T.NgayThucHien,T.IsKhuyenMai,
				T.GiaTriThayDoi,
			...
			(
				SELECT
					T2.'+ @GroupFieldName+',T2.ID, T2.DonViTinh, SUM(0) AS HopDongChiTietREF,
					T2.LechBooking,
					T2.NgayThucHien,T2.IsKhuyenMai,
				...
				(
					SELECT
						T1.'+ @GroupFieldName+','+@GroupByFildID+'T1.DonViTinh,SoHopDong AS SHD,T1.HopDongChiTietREF,
					...
					('
						+ dbo.ThucChay_GenSQLCommandForQuery(@StartDate,
															...
															@ChucDanhID) +
					')T1
					WHERE T1.DonViTinh = ' +@DauNhay + @DonViTinh + @DauNhay + ' 
				...
				GROUP BY T2.'+ @GroupFieldName+',T2.ID,T2.DonViTinh,T2.LechBooking,T2.NgayThucHien,T2.IsKhuyenMai
			) AS T
		...
		BEGIN			
			...
			WHILE @@FETCH_STATUS = 0
			   BEGIN
	   				...
	   				SET @Sql = '
	   					SELECT
							T2.'+ @GroupFieldName+',T2.ID, T2.DonViTinh,T2.HopDongChiTietREF,T2.LechBooking,
							T2.NgayThucHien,T2.IsKhuyenMai,
							SUM(T2.GiaTriThayDoi) GiaTriThayDoi,
							SUM(T2.SoLuongHopDongNoiBo) SoLuongHopDongNoiBo,
							SUM(T2.SoLuongHopDongKhuyenMai) SoLuongHopDongKhuyenMai,
						...
						GROUP BY T2.'+ @GroupFieldName+',T2.ID,T2.DonViTinh,T2.HopDongChiTietREF,T2.LechBooking,T2.NgayThucHien,T2.IsKhuyenMai
	   					'
	   				PRINT @Sql
			...
			SELECT 
				T.GroupFileName,T.GroupFileID,T.DonViTinh,T.HopDongChiTietREF,T.LechBooking,T.NgayThucHien,T.IsKhuyenMai,T.GiaTriThayDoi,
			...
			(
				SELECT 
					...
					SUM(0) AS HopDongChiTietREF,T1.LechBooking,T1.NgayThucHien,T1.IsKhuyenMai,
				...
				FROM @TableResult T1 
				GROUP BY T1.GroupFileName,T1.GroupFileID,T1.DonViTinh,T1.LechBooking,T1.NgayThucHien,T1.IsKhuyenMai
			)T
