# Function: `fn_IsPhatSinhGiaTriTD_DoanhSoXuatHoaDonNganhHangCore`

- **Loại**: SQL_SCALAR_FUNCTION
- **Ngày tạo**: 2015-06-12 10:46:58.807000
- **Ngày sửa cuối**: 2015-06-12 10:46:58.807000
...
CREATE FUNCTION [dbo].[fn_IsPhatSinhGiaTriTD_DoanhSoXuatHoaDonNganhHangCore]
(
	@HopDongID INT,
...
BEGIN
	...
	DECLARE @DmMaHopDongREF_Bef INT , @SoLuong_Bef INT, @DonGia_Bef FLOAT, @ChietKhau_Bef FLOAT;
	...
	ELSE
		...
		SELECT TOP 1  @SoHopDong_Bef = dskc.SoHopDong
					...
					,@DonGia = dskc.DonGia
					,@ChietKhau = dskc.ChietKhau
		FROM DoanhSoXuatHoaDonNganhHangCore dskc
		...
		IF((@SoHopDong <> @SoHopDong_Bef) OR (@DmNhanVienREF_Bef <> @DmNhanVienREF) 
			...
			OR(@SoLuong <> @SoLuong_Bef) OR (@DonGia <> @DonGia_Bef) OR (@ChietKhau <> @ChietKhau_Bef)
	...
	IF (SELECT COUNT(hdct.HopDongChiTietID) FROM HopDongChiTiet hdct WHERE hdct.HopDongFK = @HopDongID AND hdct.DeletedStatus =0 ) <>
	   (SELECT COUNT(A.HopDongChiTietREF) FROM (SELECT DISTINCT dsxhdc.HopDongChiTietREF AS HopDongChiTietREF
	      FROM DoanhSoXuatHoaDonNganhHangCore dsxhdc WHERE dsxhdc.HopDongID = @HopDongID) A
	   )
	SET @IsPhatSinhGiaTriTD = 1
	IF(SELECT COUNT(DoanhSoXuatHoaDonNganhHangCoreID) FROM DoanhSoXuatHoaDonNganhHangCore dsxhdc WHERE dsxhdc.ThongTinHoaDonREF = @ThongTinHoaDonID) > 0
	SET @IsPhatSinhGiaTriTD = 1
	END 
