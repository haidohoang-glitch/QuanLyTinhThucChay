# Stored Procedure: `ThucChay_Publish_Data`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2013-11-09 11:47:06.210000
- **Ngày sửa cuối**: 2014-11-19 12:16:53.803000
...
```sql
CREATE proc [dbo].[ThucChay_Publish_Data]
...
BEGIN
	...
	SET @Pamrams = N'@SoHopDongParam nvarchar(50),
					@DmSanPhamREFParam int,
					@DmWebsiteREFParam int,
					@IsPheDuyetParam int, 
	...
	IF @SoHopDong <> ''
		SET @FilterSing += ' AND SoHopDong = ' + @DauNhay + @SoHopDong + @DauNhay
	
	IF @DmSanPhamREF > 0 
		SET @FilterSing += ' AND DmSanPhamREF = ' + CONVERT(NVARCHAR(50), @DmSanPhamREF)
	IF @DmWebsiteREF > 0
		SET @FilterSing += ' AND DmWebsiteREF = ' + CONVERT(NVARCHAR(50), @DmWebsiteREF)
	...
	SET @TenBaoCao = N'Duyệt dữ liệu theo hợp đồng'
