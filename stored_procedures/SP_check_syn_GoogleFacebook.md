# Stored Procedure: `check_syn_GoogleFacebook`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2022-04-27 11:25:57.733000
- **Ngày sửa cuối**: 2022-04-27 11:25:57.733000
...
CREATE PROCEDURE [dbo].[check_syn_GoogleFacebook]
AS
BEGIN
	SET NOCOUNT ON;

	BEGIN
		SELECT 'Operating_Order'
		-- Bước 1 check syn_ADS_Operating_Order
		/*select top 1 * from [asdag].ADS.dbo.Operating_Order -- nguồn
		select top 1 * from ADS_Operating_Order -- đích
		select Max(LastModificationTime) from ADS_Operating_Order 
		*/
		SELECT A.*,B.* FROM (
		...
		ORDER BY A.LastModificationTime, A.CreationTime--,B.LastModificationTime DESC

		--select top 1 * from [asdag].ADS.dbo.Operating_Order 
		--Money_turnover: giá trị order 

		--select max(LastModificationTime) from ADS_Operating_Order
	END

	BEGIN
		SELECT 'Operating_Result'
		-- Check  syn_ADS_Operating_Result
		/*ADS_Operating_Result_Map_Order DONE
		ADS_Operating_Order DONE
		ADS_Operating_Result DONE
		...
		*/
		...
		/*
		select max(LastModificationTime) from ADS_Operating_Result
		*/
	END

	BEGIN
		SELECT 'Operating_Result_Map_Order'
		-- check syn_Operating_Result_Map_Order
		...
		order by A.LastModificationTime 
		/*
		select max(LastModificationTime) from ADS_Operating_Result_Map_Order
		select * from [asdag2].ADS.dbo.Operating_Result_Map_Order  where operating_order_id in (385)
		select * from ADS_Operating_Result_Map_Order  where id in (5392,
		5494,
		5495,
		5496,
		5497,
		5498)
		--update ADS_Operating_Result_Map_Order set  IsDeleted = 1, LastModificationTime = '2020-09-15 11:18:54.6566667' where id in (5392,5494,5495,5496,5497,5498)
		*/
	END
    
    BEGIN
		SELECT 'Operating_Result_Quantity'

		--syn_ADS_Operating_Result_Quantity
		/*ADS_Operating_Result_Map_Order
		ADS_Operating_Order
		ADS_Operating_Result
		ADS_Operating_Result_Quantity*/
