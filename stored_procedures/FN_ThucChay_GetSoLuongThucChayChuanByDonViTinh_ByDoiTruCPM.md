# Function: `ThucChay_GetSoLuongThucChayChuanByDonViTinh_ByDoiTruCPM`

- **Loại**: SQL_SCALAR_FUNCTION
- **Ngày tạo**: 2023-08-15 15:52:28.370000
- **Ngày sửa cuối**: 2023-08-15 15:56:28.547000
...
-- =============================================
CREATE  FUNCTION [dbo].[ThucChay_GetSoLuongThucChayChuanByDonViTinh_ByDoiTruCPM] 
(
...
BEGIN
	...
	FROM dbo.[ThucChayDaTinh_DoiTruVaTinhLai_CPM] tcdt INNER JOIN dbo.HopDongChiTiet hdct
	ON tcdt.HopDongID = hdct.HopDongFK
	AND tcdt.DmSanPhamREF = hdct.DmSanPhamREF
	WHERE hdct.HopDongChiTietID = @HopDongChiTietREF
	...
	BEGIN
		...
		SET @IsKM =
		(
			SELECT count(hdct.HopDongChiTietID) FROM dbo.HopDongChiTiet hdct
			WHERE hdct.HopDongChiTietID = @HopDongChiTietREF
		...
		BEGIN
			...
			(
				SELECT SUM(ISNULL(tc.SoLuongThucChay,0) + ISNULL(tc.SoLuongThayDoi,0)) FROM dbo.[ThucChayDaTinh_DoiTruVaTinhLai_CPM] tc
		...
		ELSE
			BEGIN
				SET @v_tongviewthucchay =
				(
					SELECT SUM(ISNULL(tc.SoLuongThucChayKM,0) + ISNULL(tc.SoLuongKMThayDoi,0)) FROM dbo.[ThucChayDaTinh_DoiTruVaTinhLai_CPM] tc
					WHERE tc.HopDongChiTietREF = @HopDongChiTietREF
					--AND tc.NgayThucHien BETWEEN @MinDate AND @NgayThucHien
					AND tc.NgayThucHien <= @NgayThucHien
				)	
