# Stored Procedure: `prc_asd_TinhGiaTri_ThayDoi_HopDong_Per`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-09-11 15:02:52.513000
- **Ngày sửa cuối**: 2021-06-29 11:47:44.303000
...
-- =============================================
CREATE PROCEDURE [dbo].[prc_asd_TinhGiaTri_ThayDoi_HopDong_Per]
...
BEGIN
	...
	where 
	HopDongChiTietREF = @PhanBo_ID and DmSanPhamREF = @DmSanPham_ID),0)
	...
	begin
		...
		begin
			...
			update [dbo].[ThucChayAdmarket_ViewPlus_HopDong_online] set trangthai = 1 where contract_number = @SoHopDong and DmSanPhamREF = @DmSanPham_ID and user_id = @User_id
	...
	begin
		...
		begin
		 if @GiaTriOnline > 0
			...
			-- update đã sử dụng dl online
			if @DmSanPham_ID = 144
			update [dbo].ThucChayAdmarket_ADX_CPC_HopDong_online set trangthai = 1 where contract_number = @SoHopDong and DmSanPhamREF = @DmSanPham_ID and user_id = @User_id
			-- để phần thừa vào online
			if @GiaTriOnline > (@GiaTriTaiThoiDiemTinhThucChay - @GiaTriGanNhatThayDoi)
			begin
			if @DmSanPham_ID = 144
			exec prc_insert_ThucChayAdmarket_ADX_CPC_HopDong_online 
																@User_id,
																@User_Name,
																'0',
																@SoHopDong,
																...
																0
			-- insert thực chạy
			declare @GiaTriTinhThucChay2 float = @GiaTriTaiThoiDiemTinhThucChay - @GiaTriGanNhatThayDoi
			...
			begin
				declare @GiaTriTinhThucChay3 float = @GiaTriOnline
