# Stored Procedure: `ThucChayDaTinh_InsertThucChaySanLuongChot_GTTD_ThanhTien_GGFB`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-09-09 16:27:46.780000
- **Ngày sửa cuối**: 2024-11-26 11:13:51.270000
...
```sql
CREATE PROCEDURE [dbo].[ThucChayDaTinh_InsertThucChaySanLuongChot_GTTD_ThanhTien_GGFB]
	@NgayThucHien						DATETIME,
	@ADS_Operating_Result_QuantityId	INT,
	@HopDongREF							INT,
	@HopDongChiTietREF					INT,
	@Operating_Order_Id					INT,
	@MaxTienThucChayBanOrder			FLOAT,
	@TongTienThucChaySauChietKhauOder	FLOAT,
	@GhiChuDotChayHopDong				NVARCHAR(2000),
	@ghiChu								NVARCHAR(512),
	@ThucChayDaTinhID_output			NVARCHAR(50) OUTPUT
AS
BEGIN
	DECLARE @NgayGioiHanTinh DATETIME  = '2020-01-01',
	@DmChienDichREF int = @Operating_Order_Id --Chien dich cho GGFB va dc tinh theo sanluongchot
	...
	OUTPUT INSERTED.ThucChayDaTinhID, INSERTED.HopDongID INTO @Table_thucchaydatinhid
	...
	SELECT HopDongID    = HD.HopDongID,         
			...
			DotChayHopDong = @GhiChuDotChayHopDong	,		
			SoLuongDotChayHD = 0	,		
			DotChayBooking = @ADS_Operating_Result_QuantityId	,			
			SoLuongDotChayBooking = @ADS_Operating_Result_QuantityId ,   
			SoLuong = HDCT.SoLuong       ,             
			DonViTinh = [dbo].[FormatDonViTinh_ThanhTien_GGFB](ISNULL(TC.DonViTinhTC,''))	,				
			DonGia = HDCT.DonGia,						
			DonGiaTheoDonVi = 0 ,            
			...
			DmWebsiteREF = dbo.GetDmWebsiteReportingdbIDByDmWebsiteID(TC.DmWebsiteREF) ,				
			TenWebsite = dbo.GetWebsiteLinkByDmWebsiteID(TC.DmWebsiteREF,N'') ,			
			TongViewThucChay = 0,		
			TongClickThucChay = 0	,			
			TongSoBaiViet = TC.SoLuongTC	,	
			SoLuongThucChay = (CASE WHEN (HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100) THEN 0
								ELSE [dbo].[ThucChay_GetSoLuongThucChay_ThanhTien_GGFB] ( TC.SoLuongTC, TC.Sell_Money_VND,
																						 ...
																						 HDCT.ThanhTien, HDCT.ChietKhau, @MaxTienThucChayBanOrder,	@TongTienThucChaySauChietKhauOder )
							END),
			NgayThucHien = @NgayThucHien,				
			GiaTriThayDoi = 0,			
	 ...
	 FROM 
		(	  SELECT OD.Contract_Number, OD.Contract_Id, OD.Contract_Detail_Id
			  , ISNULL(OD.UNITS,'') AS DonViTinhTC--DONVITINH
			  ...
			  , ISNULL(TC.Result,0) AS SoLuongTC --SOLUONG
			  , ISNULL(TC.Sell_Money_VND,0) AS Sell_Money_VND --THANH TIEN THUC CHAY BAN
			  , OD.DmWebsiteREF
			  FROM 
			  (SELECT TC.id, TC.Operating_Order_Id, 0 AS operating_Result_Id, ISNULL(TC.[Quantity],0) AS Result, ISNULL(TC.[TotalMoney],0) AS Sell_Money_VND
					FROM [dbo].[ADS_Operating_Result_Quantity] TC 
					WHERE TC.ID = @ADS_Operating_Result_QuantityId 
					AND ISNULL(TC.[TotalMoney],0) <> 0
					--AND  CONVERT(DATE,TC.[LastModificationTime]) = @NgayThucHien
				) TC
			  INNER JOIN 
			  (SELECT OD.Contract_Number, OD.Contract_Id, OD.Contract_Detail_Id, OD.Id  , OD.UNITS , OD.Product_Id , OD.Brand_id
			  , OD.DmWebsiteREF
	...
	SET @ThucChayDaTinhID_output = ISNULL((SELECT top (1) ISNULL(ThucChayDaTinhID,'') FROM @Table_ThucChayDaTinhID),'')
