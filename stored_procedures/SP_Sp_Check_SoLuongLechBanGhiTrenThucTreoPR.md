# Stored Procedure: `Sp_Check_SoLuongLechBanGhiTrenThucTreoPR`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-02-23 11:43:54.520000
- **Ngày sửa cuối**: 2017-03-11 09:48:04.030000
...
```sql
--Sp_Check_SoLuongLechBanGhiTrenThucTreoPR '',''
CREATE PROCEDURE [dbo].[Sp_Check_SoLuongLechBanGhiTrenThucTreoPR]
...
AS
    BEGIN
        ...
        SET @SQL = 'select A.id,CAST(IF (A.modifieddate = ''''0000-00-00'''' OR A.modifieddate = ''''0001-01-01'''' OR A.modifieddate IS NULL,IFNULL(A.CreatedDate, ''''2010-01-01''''), A.modifieddate) AS DATETIME) AS LastModifiedAt from hdcn_thucchay_pr A
        ...
        INSERT  INTO CheckThongTinDauVao
                SELECT  N'Thiếu bản ghi' NhomVanDe ,
                        N'Thông tin thực treo PR' DoiTuong ,
                        A.ID ,
                        '' DuLieuTrenSQL ,
                        '' DuLieuTrenMySQL ,
                ...
                FROM    ( SELECT    N'Thiếu bản ghi trên SQL' ThongTinLech ,
                          ...
                          FROM      dbo.ThucChayHopDongChiTietPR
                          WHERE     LastModifiedAt BETWEEN @FromDate AND @LastSynTime
                                    AND ThucChayHopDongChiTietPRID NOT IN (
                                    SELECT  ID
                                    FROM    #Temp )
                                    AND DeletedStatus = 0
                        ) A
