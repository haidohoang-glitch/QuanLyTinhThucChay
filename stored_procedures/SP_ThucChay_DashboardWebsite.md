# Stored Procedure: `ThucChay_DashboardWebsite`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2013-06-14 13:08:23.320000
- **Ngày sửa cuối**: 2014-11-19 12:16:43.537000
...
```sql
CREATE PROCEDURE [dbo].[ThucChay_DashboardWebsite] 
	@FromDate datetime, 
	@ToDate datetime,
...
BEGIN
	...
	IF @DonViThoiGian >= 0
		Begin
			...
			-- Set GroupBy
			SET @GroupBy = ' A.DmWebsiteREF,A.TenWebsite,dbo.GetDonViThoiGian(NgayThucHien,' + CONVERT(nvarchar(50), @DonViThoiGian) + ')'
			-- Set OrderBy
			SET @OrderBy = ' dbo.GetDonViThoiGian(NgayThucHien,' + CONVERT(nvarchar(50), @DonViThoiGian) + '), A.TenWebsite'
		End
	ELSE IF @DonViThoiGian = -1
		Begin
			-- Set GroupBy 
			SET @GroupBy = ' A.DmWebsiteREF,A.TenWebsite'
			-- Set OrderBy
			SET @OrderBy = ' A.TenWebsite'
		End
