# Stored Procedure: `prc_asd_TinhThucChay_Admarket_With_HopDong_NgayDauThang_ThangDuGP_ByHopDongChiTiet`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2024-12-23 09:42:49.773000
- **Ngày sửa cuối**: 2025-07-10 16:54:35.370000
...
*/
CREATE PROCEDURE [dbo].[prc_asd_TinhThucChay_Admarket_With_HopDong_NgayDauThang_ThangDuGP_ByHopDongChiTiet]
...
BEGIN
	...
	(
	 ...
	 , tc.DmViTriREF, tc.TenViTri, tc.NgayGhiNhanThayDoi, tc.CreatedAt, tc.CreatedBy, tc.LastModifiedAt, tc.LastModifiedBy,ISNULL(tc.RecordStatus,0) AS RecordStatus
	 , (
	 ...
	 , ISNULL(tc.LoaiGhiNhan,0) as LoaiGhiNhan
	  ...
	  WHERE Convert(date,tc.LastModifiedAt) = @NgayXuly
	...
	BEGIN   
		...
		BEGIN
			PRINT N'TH2'
			--GHI NHAN THEO REQUEST CHO NHOM SAN PHAM MKT-FEE
			...
			,@TienThucChay_GhiNhan = @TienThucChay_GhiNhan
		END
		--THUC HIEN UPDATE LAI THONG TIN NGAY GHI NHAN THUC CHAY
		IF (EXISTS(SELECT TOP(1) tcdta.HopDongID FROM dbo.ThucChayDaTinhAdmarket tcdta
					WHERE tcdta.[HopDongID] = @HopDongID
					AND tcdta.[HopDongChiTietREF] = @HopDongChiTietID
					AND tcdta.NgayThucHien = @NgayXuly
					AND tcdta.SoLuongDotChayBooking = @ThucChay_PerformanceBase_ThayDoi_ID
					AND tcdta.DmSanPhamREF = @DmSanPhamID
					))
		BEGIN
			--CHO TABLE THUCCHAYDATINHADMARKET
			UPDATE tcdta
			SET tcdta.NgayThucHien = @NgayGhiNhanThucChay
			, tcdta.GhiChu = N'Xl dl theo yeu cau hopdongchitiet , ' + tcdta.GhiChu
			...
			--CHO TABLE THUCCHAYDATINH
			UPDATE tcdta
			SET tcdta.NgayThucHien = @NgayGhiNhanThucChay
			, tcdta.GhiChu =  N'Xl dl theo yeu cau hopdongchitiet , ' + tcdta.GhiChu
			FROM dbo.ThucChayDaTinh tcdta
			WHERE 1=1 
			--AND tcdta.[HopDongID] = @HopDongID
			--AND tcdta.[HopDongChiTietREF] = @HopDongChiTietID
			AND tcdta.NgayThucHien = @NgayXuly
			...
			--CHO TABLE THUCCHAYDATINH_MUANGOAI
			UPDATE tcdta
			SET tcdta.NgayThucHien = @NgayGhiNhanThucChay
			, tcdta.GhiChu =  N'Xl dl theo yeu cau hopdongchitiet , ' + tcdta.GhiChu
			FROM dbo.ThucChayDaTinh_MuaNgoai tcdta
			WHERE tcdta.[HopDongREF] = @HopDongID
			AND tcdta.[HopDongChiTietREF] = @HopDongChiTietID
			AND tcdta.NgayThucHien = @NgayXuly
			AND tcdta.[ThucChayMuaNgoaiChiTietREF] = @ThucChay_PerformanceBase_ThayDoi_ID
			AND tcdta.DmSanPhamREF = @DmSanPhamID
