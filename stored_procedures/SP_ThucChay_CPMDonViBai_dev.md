# Stored Procedure: `ThucChay_CPMDonViBai_dev`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-08-25 15:52:34.240000
- **Ngày sửa cuối**: 2025-12-29 09:46:59.917000
...
/*
EXEC dbo.[ThucChay_CPMDonViBai_dev]
    @NgayGhiNhan = '2025-12-27',  -- date
	--@NgayCheckThayDoi  = '2025-12-26',
    --@NgayDanhSoGioiHan = '2022-12-28',  -- date
    @SoHopDong = N'QC0520725',  -- nvarchar
    @HopDongChiTietID = 774189  -- int
*/

CREATE PROCEDURE [dbo].[ThucChay_CPMDonViBai_dev]
	@NgayGhiNhan DATE,
...
AS
    BEGIN
		...
		--		AND ((DmSanPhamREF = 598 AND DmHinhThucQuangCao = 5001 --AND DmVitriREF = 9198 --HAIDH COMMENT 27122025 Bo dieu kien banner vi hien co mot so banner khac tinh banner = 100222
		--		) OR (DmSanPhamREF = 735))
		--		AND DonViTinh = N'BÀI'
  --              AND DotChayHopDong IN (N'Tính mới CPM donvibai', N'Tính lại CPM donvibai', N'Đối trừ CPM donvibai')
		...
		*/
		BEGIN
			...
			SELECT  DISTINCT
					...
					ThanhTienKhuyenMaiTreo = IIF(hdct.ChietKhau <> 100 OR hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1, 0,hdct.DonGia),
					ThanhTienSauCKTreo =  IIF(hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1, 0, hdct.DonGia*(1-hdct.ChietKhau/100)),
					ThanhTienKhuyenMaiPhanBo = IIF(hdct.ChietKhau <> 100 OR hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1, 0, hdct.SoLuong*hdct.DonGia),
					ThanhTienSauCKPhanBo = IIF(hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1, 0, hdct.SoLuong*hdct.DonGia*(1-hdct.ChietKhau/100)),
					LoaiThayDoi = IIF(hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1 OR tchdct.DeletedStatus = 1 , 1, 2),
					LyDo =	IIF(hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1  OR tchdct.DeletedStatus = 1 , 
								IIF(@SoHopDong IS NULL, '', N'xử lý tay ') + N'hợp đồng hoặc phân bổ xóa, hủy', 
								IIF(@SoHopDong IS NULL, '', N'xử lý tay ') + N'phân bổ thay đổi thông tin'),
					LoaiXuLy = IIF(hdct.DeletedStatus = 1 OR hd.TrangThaiHopDong = 3 OR hd.DeletedStatus = 1  OR tchdct.DeletedStatus = 1 , 1, NULL)
			...
			WHERE (@NgayCheckThayDoi IS NULL OR 
				   ...
				   AND ((hdct.DmSanPhamREF = 598 --AND hdct.DmViTriREF = 9198 --HAIDH COMMENT 27122025 Bo dieu kien banner vi hien co mot so banner khac tinh banner = 100222
						) OR (hdct.DmSanPhamREF = 735)) AND hdct.DonViTinhREF = 7
				   AND NOT ( hdct.DmLoaiREF = 13 OR hdct.DmLoaiBannerREF = 18 )
				   AND hdct.DmLoaiREF = 5001
				   AND (@SoHopDong IS NULL OR hd.SoHopDong = @SoHopDong)
				   AND (@HopDongChiTietID IS NULL OR hdct.HopDongChiTietID = @HopDongChiTietID)

				   AND tchdct.DmSanPhamREF in( 598,735)
				   AND tchdct.DmHinhThucQuangCaoREF = 5001
				   AND tchdct.RecordStatus = 1
				   AND CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) < @NgayGhiNhan
			...
			SELECT DISTINCT
					...
					ThanhTienSauCKPhanBo = hdct.SoLuong*hdct.DonGia*(1-hdct.ChietKhau/100),
					LoaiThayDoi = CASE WHEN tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0 AND 
											(CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayGhiNhan or @SoHopDong IS NOT NULL )
									   THEN 3
									   WHEN tchdct.RecordStatus = 1 AND tchdct.DeletedStatus = 1 AND 
											CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayCheckThayDoi
									   THEN 4
									   WHEN tchdct.RecordStatus = 1 AND tchdct.DeletedStatus = 0 AND 
											CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayCheckThayDoi
									   THEN 5
									   ELSE 0
								  END,
					LyDo       = CASE  WHEN tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0 AND 
											(CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayGhiNhan OR @SoHopDong IS NOT NULL)
									   THEN IIF(@SoHopDong IS NULL, '', N'xử lý tay ') + N'ghi nhận phát sinh treo mới'
									   WHEN tchdct.RecordStatus = 1 AND tchdct.DeletedStatus = 1 AND 
											CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayCheckThayDoi
									   THEN N'treo hủy'
									   WHEN tchdct.RecordStatus = 1 AND tchdct.DeletedStatus = 0 AND 
											CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayCheckThayDoi
									   THEN N'treo thay đổi phân bổ'
									   ELSE N''
								 END,
					LoaiXuLy = IIF (tchdct.RecordStatus = 1 AND tchdct.DeletedStatus = 1, 1, NULL)
			FROM ABM_Data_ThucChay.dbo.ThucChayHopDongChiTiet tchdct
			INNER JOIN ABM_Data_ThucChay.dbo.HopDongChiTiet hdct ON tchdct.HopDongChiTietREF = hdct.HopDongChiTietID
			INNER JOIN ABM_Data_ThucChay.dbo.HopDong hd ON hd.HopDongID = hdct.HopDongFK
			WHERE  ((@NgayCheckThayDoi IS NULL AND tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0) OR 
					(CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayGhiNhan AND tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0) OR 
				    (CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayCheckThayDoi AND tchdct.RecordStatus = 1 ))
                   --AND hdct.DmSanPhamREF = 598 AND hdct.DmViTriREF = 9198 AND hdct.DonViTinhREF = 7
				   AND ((hdct.DmSanPhamREF = 598 --AND hdct.DmViTriREF = 9198 --HAIDH COMMENT 27122025 Bo dieu kien banner vi hien co mot so banner khac tinh banner = 100222
						) OR (hdct.DmSanPhamREF = 735)) AND hdct.DonViTinhREF = 7
				   AND NOT ( hdct.DmLoaiREF = 13 OR hdct.DmLoaiBannerREF = 18 )
				   AND hdct.DmLoaiREF = 5001
				   ...
				   AND tchdct.ThucChayHopDongChiTietID NOT IN (SELECT DISTINCT ThucChayHopDongChiTietID FROM #DmThayDoi)
			...
			OUTER APPLY (SELECT  TOP 1 l.HopDongChiTietREF
						 ...
						 ORDER BY CONVERT(DATE, ISNULL(l.LastModifiedAt, l.CreatedAt)) desc
					     ) l
			WHERE dm.LoaiThayDoi = 5 AND 
			      dm.HopDongChiTietID = ISNULL(l.HopDongChiTietREF, dm.HopDongChiTietID)
			...
			UPDATE dm
			SET dm.ThanhTienKhuyenMaiDaTinh = IIF(hdct.ChietKhau = 100, ISNULL(tchdct.SoLuongDaTinh,0)*hdct.DonGia, 0),
			    dm.ThanhTienSauCKDaTinh = ISNULL(tchdct.SoLuongDaTinh,0)*hdct.DonGia*(1-hdct.ChietKhau/100)
			FROM #DmThayDoi dm
			INNER JOIN ABM_Data_ThucChay.dbo.HopDongChiTiet hdct ON hdct.HopDongChiTietID = dm.HopDongChiTietID
			OUTER APPLY (SELECT COUNT(1) SoLuongDaTinh
						 FROM ABM_Data_ThucChay.dbo.ThucChayHopDongChiTiet tchdct
		...
		BEGIN
			...
			SELECT   NEWID() 
					...
					,tcdt.[DmViTriREF]
					,tcdt.[TenViTri]
					,[DotChayHopDong] = N'Đối trừ CPM donvibai'
					,[SoLuongDotChayHD]
					,[DotChayBooking]
					,[SoLuongDotChayBooking]
					,tcdt.[SoLuong]
					...
					, N'Đối trừ: SP tối ưu [dbo].[ThucChay_CPMDonViBai] do ' + dm.LyDo
					, dm.NgayThucHien
			...
			WHERE		tcdt.NgayThucHien <= dm.NgayThucHien 
						...
						AND ((tcdt.DmSanPhamREF = 598 --AND tcdt.DmVitriREF = 9198 --HAIDH COMMENT 27122025 Bo dieu kien banner vi hien co mot so banner khac tinh banner = 100222
								) OR (tcdt.DmSanPhamREF = 735))
   ...
   --         SET     tchdct.RecordStatus = 0
		...
		BEGIN
			...
			SELECT  NEWID(),
					...
					hdct.DmViTriREF ,
					hdct.TenViTri ,
					DotChayHopDong = IIF(dm.LoaiXuLy = 2, N'Tính lại CPM donvibai', N'Tính mới CPM donvibai'),   
					...
					IIF(dm.LoaiXuLy = 2 OR hdct.ChietKhau = 100, 0, (dm.ThanhTienSauCKTreo/(1-ISNULL(hdct.ChietKhau, 0)/100)) * ISNULL(hdct.ChietKhau, 0) / 100) AS GiaTriTrietKhauThucChay ,
					IIF(dm.LoaiXuLy = 2 OR  hdct.ChietKhau = 100, 0, dm.ThanhTienSauCKTreo) AS ThanhTienSauTrietKhauThucChay ,
					...
					IIF(dm.LoaiXuLy = 2, N'Tính lại: ', N'Tính mới: ') + N'SP tối ưu [dbo].[ThucChay_CPMDonViBai] do ' + dm.LyDo
