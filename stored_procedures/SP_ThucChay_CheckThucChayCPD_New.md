# Stored Procedure: `ThucChay_CheckThucChayCPD_New`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-03-04 09:09:12.807000
- **Ngày sửa cuối**: 2014-11-19 12:16:59.133000
...
-- =============================================
--EXEC [dbo].[ThucChay_CheckThucChayCPD_New] '2014-05-23', '2014-05-25'
CREATE PROCEDURE [dbo].[ThucChay_CheckThucChayCPD_New]
	    ...
	    FROM   (
	               SELECT --ID Hop Dong
	                     ...
	                     --Thong tin San pham
	                      ...
	                      c.TenWebsite,
	                      --A.SoHopDong,	                      
	                      ISNULL(dbo.ThucChay_GetSoLuongThucChayCPD_Test(@NgayThucHien, C.HopDongChiTietID),0) 
								AS SoLuongThucChay,
	                      --Thanhuc Tien Thuc Chay
	                      @NgayThucHien AS NgayThucHien,	                      
	                      ISNULL(
	                          dbo.[ThucChay_GetThanhTienChuanThucChay_CPD_Test](
	                              C.SoLuong,
	                              C.DonViTinh,
	                              C.DonGia,
	                              --D.NgayKyHopDong,
	               ...
	               FROM   (
	                          SELECT *
	                          FROM   HopDongChiTiet
	                          WHERE  DmSanPhamREF IN (140, 228,241, 564, 549)
	                          --AND DmLoaiREF = 5	                          
	                          AND (DonViTinh = N'NGÀY'
									OR DonViTinh = N'TUẦN'
									OR DonViTinh = N'THÁNG'
									OR DonViTinh = N'NĂM'
									)
				...
				WHERE dbo.ThucChay_GetSoLuongThucChayCPD_Test(@NgayThucHien, C.HopDongChiTietID) > 0
				) TD
	...
	FROM   (
	       ...
	       FULL OUTER JOIN (
	                ...
	                WHERE  CONVERT(date, ngaythuchien) BETWEEN @StartDate AND @EndDate
	                       AND tcdt.DmSanPhamREF IN (140, 228,241, 564,549)
