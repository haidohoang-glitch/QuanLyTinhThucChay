# Stored Procedure: `ThucChay_CPMDonViBai`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-03-07 17:20:10.737000
- **Ngày sửa cuối**: 2025-12-29 09:51:01.027000
...
--EXEC [dbo].[ThucChay_CPMDonViBai] @NgayGhiNhan = '2025-04-10',
--                                  @SoHopDong = N'dfawdfa',
--								  @HopDongChiTietID = 12645

CREATE PROCEDURE [dbo].[ThucChay_CPMDonViBai]
	@NgayGhiNhan DATE,
		...
		BEGIN
			...
			SELECT DISTINCT
					...
					LoaiThayDoi = CASE WHEN tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0 AND 
											--CONVERT(DATE, ISNULL(tchdct.LastModifiedAt, tchdct.CreatedAt)) = @NgayGhiNhan
			...
			INNER JOIN ABM_Data_ThucChay.dbo.HopDong hd ON hd.HopDongID = hdct.HopDongFK
			WHERE  (
					(@NgayCheckThayDoi IS NULL AND tchdct.RecordStatus = 0 AND tchdct.DeletedStatus = 0) OR 
