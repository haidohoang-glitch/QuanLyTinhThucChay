# Stored Procedure: `ThucChay_Insert_TinhLai_ThucChayDaTinh_ByHD_ThanhTien_Admatic`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-06-24 15:45:48.057000
- **Ngày sửa cuối**: 2021-06-11 10:27:54.767000
...
--EXEC [ThucChay_Insert_TinhLai_ThucChayDaTinh_ByHD_ThanhTien_Admatic]


CREATE  PROCEDURE [dbo].[ThucChay_Insert_TinhLai_ThucChayDaTinh_ByHD_ThanhTien_Admatic] 
	...
	(		
		SELECT  NEWID() AS ThucChayDaTinhID,
			...
			(
				...
				ISNULL((SELECT TOP (1) sp.TenSanPham FROM DmSanPham sp WHERE sp.DmSanPhamID = tc.DmSanPhamREF ORDER BY sp.DmSanPhamID),'') as TenSanPham,  
				hd.DmNhomWebsiteREF, 
				hd.TenNhomWebsite, 
				hd.DmChuyenMucREF, 
				hd.TenChuyenMuc, 
			...
			(
				...
				(	SELECT HopDongREF, HopDongChiTietREF, DmSanPhamREF , DmBannerREF as DmBannerID, TiLeThucChayHDCTSoVoiBanner 
					...
					AND HopDongChiTietREF = @HopDongChiTietID AND DmBannerREF = @DmBannerID
					AND DeletedStatus = 0
			...
			(SELECT hd.HopDongID,
				...
				(SELECT * FROM dbo.HopDongChiTiet hdct 
					...
					AND NOT hdct.DonViTinhREF IN (3, 4) --KHONG PHAI DON VI TINH NGAY
