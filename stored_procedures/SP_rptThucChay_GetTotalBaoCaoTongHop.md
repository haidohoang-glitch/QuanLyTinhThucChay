# Stored Procedure: `rptThucChay_GetTotalBaoCaoTongHop`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-03-26 11:49:29.837000
- **Ngày sửa cuối**: 2015-03-26 11:49:29.837000
...
-- =============================================
--EXEC [rptThucChay_GetDataBaoCaoTongHop] '01',1,100,'TenSanPham','2013-01-01','2014-12-31','','','','','','','','Phuongvtt','5,10','','',' AND 1=1'
CREATE PROCEDURE [dbo].[rptThucChay_GetTotalBaoCaoTongHop]
...
BEGIN
 ...
 --   SET @ToUserName = (SELECT ToUserName FROM MappingUser A WHERE A.FromUserName = @TenDangNhap)
    ...
    ELSE
    	SET @sWhereSecurity = ' AND ' + [dbo].[fn_getsWhereSecurity](@TenDangNhap,@Security,@PhongBanSecurity,@BoPhanSecurity,@NhomSecurity,@ListSanPhamSecurity,@ListWebsiteSecurity)
     PRINT(@sGroupBy)
    ...
    BEGIN
    	 ...
    	 SET @Sql = @Sql + @sWhereChay + @sWhereSecurity + @sGroupBy
    ...
    SELECT 
		...
		SUM(T.GiaTriThayDoiTC + T.GiaTriThayDoiNB) AS GiaTriThayDoi,
		SUM(T.ThanhTienNoiBo - T.GiaTriThayDoiNB) as ThanhTienNoiBo,
		SUM(T.ThanhTienKhuyenMai - T.GiaTriThayDoiKM) as ThanhTienKhuyenMai,
		SUM(T.ThanhTienThucThu - T.GiaTriThayDoiTC) as ThanhTienThucChaySauChietKhau,
		SUM(T.ThanhTienThucThu + T.ThanhTienNoiBo) AS ThanhTienThucThu,
		SUM(T.GiaTriThayDoiNB) AS GiaTriThayDoiNB,
		SUM(T.GiaTriThayDoiTC) AS GiaTriThayDoiTC,
		SUM(T.ThanhTienNoiBo) AS ThanhTienThucThuNB,
		SUM(T.ThanhTienThucThu) AS ThanhTienThucThuTC
    FROM
    (
       SELECT 
