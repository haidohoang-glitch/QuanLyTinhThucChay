# Stored Procedure: `ThucChay_ExecInsertThucChayDaTinh_Admatic_Adx`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-05-25 17:40:35.343000
- **Ngày sửa cuối**: 2020-07-14 11:41:38.467000
...
*/
CREATE  PROCEDURE [dbo].[ThucChay_ExecInsertThucChayDaTinh_Admatic_Adx] 
...
BEGIN
	...
	DECLARE @HopDongChiTietID INT, @DmSanPhamREF int, @ThanhTien_HDCT FLOAT, @ThanhTienThucChay FLOAT, @SoLuongThucChay BIGINT
	...
	, @COUNT INT = 0
	, @HopDongID INT =0
	, @PreFromDate DATETIME
	, @NgayDanhSoGioiHan Datetime = '2020-07-20'

	SET @DmSanPhamREF = 585
	SET @PreFromDate = DATEADD(DAY,-1,@NgayThucHien)
	...
	INNER JOIN (SELECT hd.SoHopDong FROM dbo.HopDong hd WHERE hd.NgayDanhSoHopDong < @NgayDanhSoGioiHan) hd
	on hd.SoHopDong = tc.contract_number
	...
	BEGIN
		--SET @COUNT = @COUNT +1
		--PRINT @COUNT
		SET @HopDongID = ISNULL((SELECT TOP (1) HopDongID FROM dbo.HopDong 
					WHERE SoHopDong = @contract_number AND NgayDanhSoHopDong < @NgayDanhSoGioiHan ORDER BY HopDongID),0)
		...
		BEGIN
			...
			EXEC [dbo].[ThucChay_Update_AdmaticThuTuChayHopDongChiTiet_ByHopDongID] @HopDongID
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						SET @DonViTinh = (CASE WHEN (@DonViTinh = N'CLICK' OR @DonViTinh = N'VIEW') THEN @DonViTinh
							ELSE N'VIEW'
						END)
						--SOLUONG THUC CHAY
					...
					BEGIN
						...
						ELSE SET @SoLuongThucChay = @SoLuong*1000 - @SoLuongThucChayDaTinh
						...
						BEGIN
							...
							ELSE SET @ClickBalance = @ViewBalance - @SoLuongThucChay
