# Stored Procedure: `Insert_DoanhSoTienVeNganhHangCore_NoiBo_History_Update`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-06-12 10:46:59.760000
- **Ngày sửa cuối**: 2015-06-12 10:46:59.760000
...
| `@HopDongID_in` | `int(4)` | No |
| `@HopDongChiTietID_int` | `int(4)` | No |
| `@ThongTinTienVeID_in` | `int(4)` | No |
...
-- =============================================
CREATE PROCEDURE [dbo].[Insert_DoanhSoTienVeNganhHangCore_NoiBo_History_Update]
	-- Add the parameters for the stored procedure here
	@NgayThucHien DATETIME,
	@HopDongID_in INT,
	@HopDongChiTietID_int INT,
	@ThongTinTienVeID_in int
AS
BEGIN
	...
	DECLARE @DmMaSoHopDong INT,@NgayTienVe DATETIME, @SoHoaDon NVARCHAR(50), @ThongTinTienVeID INT;
	...
	FROM HopDong hd
		...
		INNER JOIN 
		(SELECT  tthd.HopDongREF,CONVERT(DATE,tthd.NgayThanhToan) AS NgayTienVe, tthd.ThongTinTienVeID, tthd.GiaTri
		   FROM ThongTinTienVe tthd WHERE  tthd.DeletedStatus = 0 AND tthd.ThongTinTienVeID = @ThongTinTienVeID_in) A
		...
		AND hd.HopDongID = @HopDongID_in
		AND hdct.HopDongChiTietID = @HopDongChiTietID_int
    ...
    , @NgayTienVe,@ThongTinTienVeID
	WHILE @@FETCH_STATUS = 0   
	BEGIN   
		SET @ThanhTienHoaDon = (SELECT dbo.fn_GetThanhTienByNgay_DoanhSoTienVeCore(@NgayTienVe,@HopDongID,@ThongTinTienVeID,@ThanhTien))
		    SET @DmNganhHangREF = 0
			SET @TenNganhHang = ''
			SET @CountNhan = 0;
			SELECT @SoLuongNganh = count(a.DmNganhHang) from
			(
				SELECT distinct dbo.FormatString(item) DmNganhHang
				FROM dbo.ArrayToTable(dbo.Array(@DmListNganhHangREF,','))
			)a
			IF @DmListNganhHangREF ='' SET @SoLuongNganh = -1
	IF(@SoLuongNganh >0)
	BEGIN
			...
			BEGIN   
				   SET @CountNhan +=1;
				    IF(@CountNhan <> @SoLuongNganh) 
						BEGIN
							IF(@SoLuong <> 0)
								begin
				    				SET @ChiSoSLNhanM =  @SoLuong/@SoLuongNganh
				    				SET @ChiSoTTNhanM = @ChiSoSLNhanM * CAST(@ThanhTienHoaDon AS BIGINT )/@SoLuong
								END
							ELSE
								BEGIN
									SET @ChiSoSLNhanM =  0
				    				SET @ChiSoTTNhanM = CAST(@ThanhTienHoaDon AS BIGINT )/@SoLuongNganh
								END
						END
				    ...
				    BEGIN
				    	IF(@SoLuong <> 0)
				    	    begin
				    			SET @ChiSoSLNhanM =  @SoLuong - (@SoLuongNganh-1) * CAST(@SoLuong/@SoLuongNganh AS INT)
				    			SET @ChiSoTTNhanM =  CAST(@ThanhTienHoaDon AS BIGINT) - (@SoLuongNganh-1) * CAST(@SoLuong/@SoLuongNganh AS int) * CAST(@ThanhTienHoaDon AS BIGINT)/@SoLuong
				    	    END
				    	ELSE
				    		BEGIN
				    			SET @ChiSoSLNhanM =  0
				    			SET @ChiSoTTNhanM =  CAST(@ThanhTienHoaDon AS BIGINT) - (@SoLuongNganh-1)  * CAST(@ThanhTienHoaDon AS BIGINT)/@SoLuongNganh
				    		END
