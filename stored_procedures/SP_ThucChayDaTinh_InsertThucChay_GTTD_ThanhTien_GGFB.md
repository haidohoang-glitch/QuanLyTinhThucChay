# Stored Procedure: `ThucChayDaTinh_InsertThucChay_GTTD_ThanhTien_GGFB`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-09-01 15:55:11.243000
- **Ngày sửa cuối**: 2024-11-26 11:11:56.460000
...
| `@GhiChuDotChayHopDong` | `nvarchar(2000)` | No |
...
```sql
CREATE PROCEDURE [dbo].[ThucChayDaTinh_InsertThucChay_GTTD_ThanhTien_GGFB]
	@NgayThucHien						DATETIME,
	@ADS_Operating_Result_Map_OrderId	INT,
	@HopDongREF							INT,
	@HopDongChiTietREF					INT,
	@Operating_Order_Id					INT,
	@GhiChuDotChayHopDong				NVARCHAR(1000),
...
BEGIN
	...
	SELECT NEWID() AS ThucChayDaTinhID, TD.HopDongID, TD.SoHopDong, TD.DmMaHopDongREF, TD.TenMaHopDong, TD.NgayDanhSoHopDong, TD.NgayKyHopDong, 
		...
		(CASE WHEN TD.ChietKhau = 100 THEN (TD.SoLuongTC - TD.SoLuongKMThayDoi)
				ELSE (TD.SoLuongTC - TD.SoLuongThayDoi)
			END
		)AS SoLuongLechTreoHa,
		(CASE WHEN TD.ChietKhau = 100 THEN (TD.Sell_Money_VND- TD.GiaTriKMThayDoi)
				ELSE ((TD.Sell_Money_VND - TD.GiaTriThayDoi)/(100-TD.ChietKhau))*100
			END
	...
	SELECT HopDongID    = HD.HopDongID,         
			...
			SoLuongThucChay = 0,
			NgayThucHien = @NgayThucHien,				
			GiaTriThayDoi = (CASE WHEN (HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100)  THEN 0
			...
			PheDuyetBy = '',    
			PheDuyetAt = GETDATE(),   
			SoLuongThayDoi = (CASE WHEN (HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100) THEN 0
								ELSE [dbo].[ThucChay_GetSoLuongThucChay_ThanhTien_GGFB] ( TC.SoLuongTC, TC.Sell_Money_VND,
																						 ...
																						 HDCT.ThanhTien, HDCT.ChietKhau, @MaxTienThucChayBanOrder,@TongTienThucChaySauChietKhauOder )
							END),   
			SoLuongKMThayDoi =  (CASE WHEN (HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100) THEN [dbo].[ThucChay_GetSoLuongThucChay_ThanhTien_GGFB] ( TC.SoLuongTC, TC.Sell_Money_VND,
																						 TC.SoLuongTC, TC.Sell_Money_VND,
																						 @NgayThucHien, HDCT.HopDongChiTietID,
																						 HDCT.SoLuong, HDCT.DonGia,
																						 HDCT.ThanhTien, HDCT.ChietKhau,  @MaxTienThucChayBanOrder,@TongTienThucChaySauChietKhauOder )
								ELSE 0
								END),   
			GiaTriKMThayDoi = (CASE WHEN (HDCT.IsKhuyenMai = 1 OR HDCT.ChietKhau = 100)  THEN [dbo].[ThucChay_ThanhTienThucChay_ThanhTien_GGFB] ( TC.Sell_Money_VND, TC.Sell_Money_VND,
																										@NgayThucHien , HDCT.HopDongChiTietID, 
																										HDCT.SoLuong, HDCT.DonGia,
																										HDCT.ThanhTien, HDCT.ChietKhau,  @MaxTienThucChayBanOrder,@TongTienThucChaySauChietKhauOder )
												ELSE 0
	 ...
	 FROM 
		(	  SELECT OD.Contract_Number, OD.Contract_Id, OD.Contract_Detail_Id
			  ...
			  (SELECT TC.id, TC.Operating_Order_Id, TC.operating_Result_Id, ISNULL(TC.Result,0) Result, TC.Sell_Money_VND 
					...
					AND TC.Id = @ADS_Operating_Result_Map_OrderId
				) TC
			  INNER JOIN 
