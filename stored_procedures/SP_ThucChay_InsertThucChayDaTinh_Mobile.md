# Stored Procedure: `ThucChay_InsertThucChayDaTinh_Mobile`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2016-03-17 09:21:20.603000
- **Ngày sửa cuối**: 2016-03-17 09:21:20.603000
...
CREATE PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_Mobile] 
...
AS
BEGIN
	DECLARE @v_DmWebsiteREF INT, @v_DsNhanHangREF NVARCHAR(200)
	SET @v_DmWebsiteREF = 0
	SET @v_DsNhanHangREF = ''
	
	SET @v_DmWebsiteREF =
	(
		SELECT TOP 1 DmWebsiteReportingdbID 
		 FROM DmWebsiteReportingdb 
		WHERE DmWebsiteReportingdb.TenWebsite = @TenWebsite
	)
	SET @v_DsNhanHangREF = [dbo].[f_ReturnListConcatNhanHangREF_MobileSingleBanner] 
	(
		@SoHopDong,
		@TypeProduct,
		@HopDongChiTietID ,
...
SELECT TC.* FROM (
	...
	@BannerType,@NgayThucHien)as DonGia,
	dbo.ThucChay_GetDonGiaTheoDonViTruocChietKhau(C.HopDongChiTietID,--@ProductUnitName,
	(CASE WHEN C.DonViTinh IN ('CPC', 'CPM') THEN C.DonViTinh
		  WHEN C.DonViTinh = N'Gói' AND C.TenLoai = 'CPC' THEN 'CPC'
		  WHEN C.DonViTinh = N'Gói' AND C.TenLoai = 'CPM' THEN 'CPM'
	END),
	@BannerType,@NgayThucHien)
	...
	0 as GiaTriThayDoi,
	ISNULL(dbo.ThucChay_GetThanhTienChuanThucChay_Mobile(
		0,--@SoLuong 
	@ProductUnitName,-- 
	0,--@DonGia,
	'',--@NgayKyHopDong,
	...
	@BannerType ,
	@NgayThucHien,
	@HopDongChiTietID),0) as ThanhTienThucChayTruocTrietKhau	
