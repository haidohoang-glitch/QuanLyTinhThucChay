# Stored Procedure: `Sp_Check_GiaTriLechCacTruongTrenThucTreoPR`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-02-23 15:15:26.297000
- **Ngày sửa cuối**: 2017-03-11 09:43:07.250000
...
```sql
--[Sp_Check_GiaTriLechCacTruongTrenThucTreoPR] '2016-01-01',''
CREATE PROCEDURE [dbo].[Sp_Check_GiaTriLechCacTruongTrenThucTreoPR]
...
AS
    BEGIN
        ...
        SET @SQL = 'select id,
					...
					giatien	,
					chietkhau	,
					CAST(IF (A.thoigianbd = ''''0000-00-00'''' OR A.thoigianbd =''''0001-01-01'''' OR A.thoigianbd IS NULL,''''2010-01-01'''', A.thoigianbd) AS DATETIME) ThoiGianBatDau,
        ...
        SET @TotalRows = ( SELECT   MAX(STT)
                           ...
                           WHERE    DoiTuong = N'Thông tin thực treo PR'
                         )
        ...
        WHILE @x <= @TotalRows
            BEGIN
                ...
                WHERE   STT = @x
                        AND DoiTuong = N'Thông tin thực treo PR'
			...
			) B ON A.id=B.ThucChayHopDongChiTietPRID
			WHERE 1=1 
