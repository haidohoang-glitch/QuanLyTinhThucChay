# Stored Procedure: `sp_TC_CheckHopDongCoThayDoi_TRUEVIEW`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-09-24 15:33:01.377000
- **Ngày sửa cuối**: 2021-06-14 16:00:53.170000
...
CREATE  PROCEDURE [dbo].[sp_TC_CheckHopDongCoThayDoi_TRUEVIEW] 
...
BEGIN
	...
	SELECT @StartDate = MIN(tv.NgayThucHien),
	@EndDate = MAX(tv.NgayThucHien) 
	FROM dbo.ThucChayTrueView TV
	WHERE TV.SoHopDong = @SoHopDong
	AND TV.DmSanPhamREF = @DmSanPhamREF
	...
	IF(
		((@SoLuongHD <> @SoLuongHDBF)OR (@ChietKhau <> @ChietKhauBF) OR (@ThanhTienBF <> @ThanhTien)) 
		AND (@SoLuongThucChay > 0)
	)
	BEGIN
		--TH CO THAY DOI VE GIA TRI, THUC HIEN DOI TRU DI VA TINH LAI
		EXEC [dbo].[sp_TC_DoiTruVaTinhLai_CPM_TrueView]
		@StartDate = @StartDate
