# Stored Procedure: `Insert_DoanhSoKyNganhHangCore_PhatSinhGiaTriThayDoi`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2015-04-22 16:42:52.997000
- **Ngày sửa cuối**: 2015-04-22 16:42:52.997000
...
-- =============================================
CREATE PROCEDURE [dbo].[Insert_DoanhSoKyNganhHangCore_PhatSinhGiaTriThayDoi]
...
BEGIN
	...
	SELECT @CountBefor = COUNT(dskc.DoanhSoKyNganhHangCoreID)
	 FROM DoanhSoKyNganhHangCore dskc WHERE dskc.HopDongID = @HopDongID
	...
	dsknhc.DmWebsiteREF
	 ...
	 , dsknhc.DmMaSoHopDongREF
	FROM DoanhSoKyNganhHangCore dsknhc 
	WHERE dsknhc.HopDongID = @HopDongID
	AND dsknhc.HopDongChiTietREF = @HopDongChiTietREF
	AND dsknhc.TypeRecordStatus IN (0,1,3)
	AND CONVERT(date,NgayThucHien) <= @NgayThucHien
	AND dsknhc.NgayThucHien = (SELECT MAX(NgayThucHien)FROM DoanhSoKyNganhHangCore WHERE HopDongID = @HopDongID AND HopDongChiTietREF = @HopDongChiTietREF)
	ORDER BY NgayThucHien DESC
    -- Insert statements for procedure here
	SELECT @CountAffter = COUNT(dskc.DoanhSoKyNganhHangCoreID)
	 FROM DoanhSoKyNganhHangCore dskc WHERE dskc.HopDongID = @HopDongID
	AND dskc.HopDongChiTietREF = @HopDongChiTietREF
