# Stored Procedure: `sp_TC_InsertThucChayDaTinh_ChiPhiKhac`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-06-12 09:21:31.113000
- **Ngày sửa cuối**: 2024-11-14 10:16:33.877000
...
```sql
--EXEC  [dbo].[sp_TC_InsertThucChayDaTinh_ChiPhiKhac] '2018-08-29','2018-08-29', NULL
-------------------------------------------------------------
CREATE PROCEDURE [dbo].[sp_TC_InsertThucChayDaTinh_ChiPhiKhac]
    @StartDate DATETIME ,
    @EndDate DATETIME ,
...
AS
    BEGIN
        ...
        WHILE ( @NgayThucHien <= @EndDate )
            BEGIN	 
			PRINT @NgayThucHien
					--Record_Cursor_SPChiPhi_1
					DECLARE Record_Cursor_SPChiPhi_1 CURSOR FOR 

					SELECT distinct  tchdctp.ThucChayHopDongChiTietID, ct.HopDongChiTietID, ct.ChietKhau, ct.ThanhTien, ct.DonGia, ct.SoLuong
                        FROM
                            (SELECT hdct.* FROM dbo.HopDongChiTiet hdct
								...
								AND NOT (hdct.DmViTriREF in (100093,100478))	--banner của GGFB 28/02/2021
								AND ( @pSoHopDong IS NULL OR hdct.HopDongFK = @v_HopDongID )
					...
					OPEN Record_Cursor_SPChiPhi_1

					-- Perform the first fetch.
					FETCH NEXT FROM Record_Cursor_SPChiPhi_1 into @v_ThucChayHopDongChiTietID, @v_HopDongChiTietID, @v_ChietKhau, @v_thanhTienHDCN, @v_DonGiaHDCN, @v_SoLuongHDCN
					...
					WHILE @@FETCH_STATUS = 0
						BEGIN
									...
									FROM    ( SELECT 
											  ...
											  FROM      ( SELECT    *
														  FROM      ( SELECT
																	  ...
																	  FROM
																		  (SELECT hdct.* FROM dbo.HopDongChiTiet hdct WHERE hdct.HopDongChiTietID = @v_HopDongChiTietID
																			AND NOT (hdct.DmLoaiREF = 42 AND hdct.DmLoaiNenTangREF = 9 AND hdct.DmSanPhamREF = 817)
																		  ) ct
																		  INNER JOIN 
							...
							--HAIDH COMMENT 04/08/2022 TH NEU TON TAI THUC TREO CHUA DUOC TINH CUA HOPDONGCHITIET THI TINH LUON
							IF(EXISTS(SELECT top(1) tc.ThucChayHopDongChiTietID FROM dbo.ThucChayHopDongChiTiet tc
							WHERE tc.HopDongChiTietREF = @v_HopDongChiTietID
							 AND ( CASE
											WHEN tc.CreatedAt >= tc.LastModifiedAt
											THEN CONVERT(DATE, tc.CreatedAt)
											ELSE CONVERT(DATE, tc.LastModifiedAt)
											END 
									) < CONVERT(DATE, @NgayThucHien)
									AND tc.DeletedStatus = 0
									AND tc.RecordStatus = 0
									AND CONVERT(DATE, ISNULL(tc.ThoiGianBatDau,'2009-01-01')) >= @NgayGioiHanTinh
									ORDER BY tc.ThucChayHopDongChiTietID
							))
							BEGIN
								...
								SET @SoHopDong =
								ISNULL((
									SELECT TOP (1) hd.SoHopDong FROM
									(SELECT hdct.HopDongChiTietID, hdct.HopDongFk FROM dbo.HopDongChiTiet hdct WHERE hdct.HopDongChiTietID = @v_HopDongChiTietID)hdct
					...
					CLOSE Record_Cursor_SPChiPhi_1
					DEALLOCATE Record_Cursor_SPChiPhi_1
