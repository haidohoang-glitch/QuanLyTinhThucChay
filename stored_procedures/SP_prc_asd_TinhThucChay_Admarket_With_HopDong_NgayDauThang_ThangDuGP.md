# Stored Procedure: `prc_asd_TinhThucChay_Admarket_With_HopDong_NgayDauThang_ThangDuGP`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2022-09-13 15:40:25.250000
- **Ngày sửa cuối**: 2025-07-10 14:55:26.803000
...
CREATE PROCEDURE [dbo].[prc_asd_TinhThucChay_Admarket_With_HopDong_NgayDauThang_ThangDuGP]
...
BEGIN
	...
	--NEU LA DU LIEU GHI NHAN CHO NGAY 01 HANG THANG THI THUC HIEN GHI NHAN VE NGAY MUON GHI NHAN
	IF(DAY(@NgayThucHien) = 1)
	BEGIN
		--CHO TABLE THUCCHAYDATINHADMARKET
	    UPDATE tcdta
		SET tcdta.NgayThucHien = @NgayGhiNhanThucChay
		, tcdta.GhiChu = N'Xl dl ngay 01 thang, ' + tcdta.GhiChu
		FROM dbo.ThucChayDaTinhAdmarket tcdta
		INNER JOIN dbo.[ThucChay_PerformanceBase_ThayDoi_HopDong] tc
		ON tc.ThucChay_PerformanceBase_ThayDoi_ID = tcdta.SoLuongDotChayBooking
		--AND tc.HopDongID =  tcdta.[HopDongID]
		--AND tc.HopDongChiTietID = tcdta.[HopDongChiTietREF]
		AND tc.DmSanPhamID = tcdta.DmSanPhamREF
		...
		INNER JOIN dbo.[ThucChay_PerformanceBase_ThayDoi_HopDong] tc
		ON tc.ThucChay_PerformanceBase_ThayDoi_ID = tcdta.[ThucChayMuaNgoaiChiTietREF]
		AND tc.HopDongID = tcdta.[HopDongREF]
		AND tc.HopDongChiTietID = tcdta.[HopDongChiTietREF]
		AND tc.DmSanPhamID = tcdta.DmSanPhamREF
		AND tcdta.NgayThucHien = @NgayThucHien
	END
