# Stored Procedure: `ThucChay_InsertThucChayDaTinh_ByHD_Native_Ads`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-07-17 15:02:55.317000
- **Ngày sửa cuối**: 2023-04-10 17:16:08.920000
...
--EXEC [ThucChay_InsertThucChayDaTinh_ByHD_Native_Ads] '2021-02-08','qc8401220',1029371,821,254,569625,'test'


CREATE  PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_ByHD_Native_Ads] 
...
BEGIN
	...
	SELECT  NEWID() AS ThucChayDaTinhID,
		...
		(
			...
			AND HopDongChiTietREF = hd.HopDongChiTietID
			AND DmSanPhamREF = tc.DmSanPhamREF
			AND DmBannerREF = tc.DmBannerID ORDER BY ThucChayHopDongChiTietID
			) AS NhanHang,
			...
			ISNULL((SELECT TOP (1) sp.TenSanPham FROM dbo.DmSanPham sp WHERE sp.DmSanPhamID = tc.DmSanPhamREF ORDER BY sp.DmSanPhamID),'') as TenSanPham,  
			hd.DmNhomWebsiteREF, 
			hd.TenNhomWebsite, 
			hd.DmChuyenMucREF, 
			hd.TenChuyenMuc, 
		...
		(
			...
			FROM
			(
				SELECT tct.SoHopDong, tct.DmSanPhamREF, tct.DmWebsiteID, tct.TenWebsite, tct.DmBannerID, SUM(tct.SoLuongThucChay)SoLuongThucChay
			...
			INNER JOIN 
			(	SELECT distinct HopDongREF, HopDongChiTietREF, DmSanPhamID, DmBannerID, TiLeThucChayHDCTSoVoiBanner FROM dbo.ThucChayHopDongChiTietAndBanner_Native_Ads 
				WHERE HopDongREF = @HopDongID
		...
		(SELECT hd.HopDongID,
			...
			(SELECT * FROM dbo.HopDongChiTiet hdct 
				WHERE 1=1
				AND (hdct.DmSanPhamREF = @DmSanPhamREF OR hdct.DmSanPhamREF = 733)
		...
		AND (hd.DmSanPhamREF = tc.DmSanPhamREF OR hd.DmSanPhamREF = 733)--HAIDH Comment 07/07/2022: Ap dung tinh cho ca nhieu san pham 
	)TD
END
