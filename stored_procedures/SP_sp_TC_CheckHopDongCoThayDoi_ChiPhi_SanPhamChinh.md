# Stored Procedure: `sp_TC_CheckHopDongCoThayDoi_ChiPhi_SanPhamChinh`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-06-12 09:23:44.287000
- **Ngày sửa cuối**: 2017-11-23 10:58:52.920000
...
-- =============================================
--exec [ThucChay_CheckHopDongCoThayDoi_ChiPhi_SanPhamChinh] 25611, 'DT160514', 57511, '2014-05-22'
CREATE PROCEDURE [dbo].[sp_TC_CheckHopDongCoThayDoi_ChiPhi_SanPhamChinh] 
	...
	--NEU HOP DONG CO SU THAY DOI
                ...
                IF ( @HopDongChiTietThayDoiCK <> 0 )
                    BEGIN
                        ...
                        SET @NGUON_LOG = @NGUON_LOG
                            ...
                            + CONVERT(NVARCHAR(30), @HopDongChiTietID) 	
                    END
		
		--GET SO TIEN VA SO LUONG THUCCHAY CUA HOPDONGCHITIET DEN NGAYTHUCHIEN
                SELECT  @GiaTien = SUM(CASE WHEN hdct.DonGia = 0 THEN 0 ELSE ISNULL(ThanhTien / DonGia * DonGia / SoLuong, 0) END)--da co chiet khau
                FROM    HopDongChiTiet hdct
                WHERE   hdct.HopDongChiTietID = @HopDongChiTietID
                        AND ( CASE WHEN hdct.CreatedAt >= hdct.LastModifiedAt
                                   THEN CONVERT(DATE, hdct.CreatedAt)
	    ...
	    --CHECK NEU LA HOPDONGCHITIET KHUYEN MAI
			...
			--1. GET THANHTIENKHUYENMAI DA TINH CUA HOPDONGCHITIET
                        SET @ThanhTienDaTinh = ( SELECT SUM(ISNULL(tcdt.ThanhTienKM,
                                                              0)
	    ...
	    --CHECK NEU LA HOPDONGCHITIET KHONG KHUYEN MAI
			...
			--1. GET THANHTIENSAUTRIETKHAU DA TINH CUA HOPDONGCHITIET					
                        SET @ThanhTienDaTinh = ( SELECT SUM(ISNULL(tcdt.ThanhTienSauTrietKhauThucChay,
                                                              0)
                                                            + ISNULL(tcdt.GiaTriThayDoi,
                                                              0))
                                                 FROM   ThucChayDaTinh tcdt
	    ...
	    --1 IF THANHTIEN DA TINH <>0 VA <> SO TIEN HIEN TAI THI THUC HIEN TINH
                IF ( ( @ThanhTienDaTinh <> 0 )
                     AND ( @ThanhTienDaTinh <> @GiaTienSauCK )
                   )
                    BEGIN
                        SET @GiaTriThayDoiHT = @GiaTienSauCK
                            - @ThanhTienDaTinh
			...
			--1.1 NEU DA TON TAI BAN GHI CUA HOPDONGCHITIET TAI NGAY HIEN TAI THI THUC HIEN UPDATE GIA TRI THAY DOI
				...
				--GET GIA TRI THAY DOI TRUOC DO
                                SET @GiaTriThayDoiBF = ( SELECT
                                                              SUM(ISNULL(tcdt.GiaTriThayDoi,
                                                              0))
                                                         FROM ThucChayDaTinh tcdt
                                                         WHERE
                                                              tcdt.HopDongChiTietREF = @HopDongChiTietID
                                                              AND CONVERT(DATE, tcdt.NgayThucHien) = @NgayThucHien
                                                       )
                                UPDATE  ThucChayDaTinh
                                SET     GiaTriThayDoi = @GiaTriThayDoiHT
                                        - @GiaTriThayDoiBF , -- Cho nay check ky HAIDH 29/10/2013
			...
			--1.2 NEU CHUA TON TAI THI TAO BAN GHI MOI CHO HOPDONGCHITIET TAI NGAY HIEN TAI VOI SO TIEN KHUYEN MAI	
                        IF ( @ISEXIST_HDCT = 0 )
                            BEGIN
                                EXEC sp_TC_InsertThucTreoThayDoi_ChiPhi_SanPhamChinh @HopDongChiTietID,
                                    @NgayThucHien, @GiaTriThayDoiHT
                            END
			--1.3 GHI LOG
                        SET @CONTENT_LOG = @CONTENT_LOG
                            + N' (Có thay đổi thực treo, Giá trị:'
