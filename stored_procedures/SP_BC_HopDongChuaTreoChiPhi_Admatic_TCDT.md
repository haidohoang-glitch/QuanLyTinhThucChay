# Stored Procedure: `BC_HopDongChuaTreoChiPhi_Admatic_TCDT`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-10-29 17:33:21.370000
- **Ngày sửa cuối**: 2025-10-30 09:11:51.087000
...
```sql
--Admatic: Cảnh báo CHƯA treo chi phí QL các hợp đồng có hạng mục Admatic đã chạy xong
--Admatic: Cảnh báo CHƯA TREO ĐỦ chi phí các hợp đồng có hạng mục SP chính đã chạy xong
CREATE PROCEDURE dbo.BC_HopDongChuaTreoChiPhi_Admatic_TCDT
    @Type   INT       = 1,           -- 2: đã chạy ~100% SP chính nhưng CHƯA treo đủ CP
...
BEGIN
    ...
    WHERE hd.TrangThaiHopDong NOT IN (0,3)
      AND hdct.DmLoaiREF IN (42)  -- admatic
	  AND  CAST(
                IIF(hdct.ChietKhau = 100,
                    hdct.SoLuong * hdct.DonGia,
                    hdct.ThanhTien)
            AS DECIMAL(19,2)) <> 0
      AND (
            hdct.DmSanPhamREF IN (535,560)  --Chi phí quản lý; Chi phí sản xuất     
          );	
    ...
    WHERE hd.TrangThaiHopDong NOT IN (0,3)
      AND hdct.DmLoaiREF IN (42)   -- admatic
      AND hdct.DmSanPhamREF not IN (535,560)       ;
   ...
   --         sp.TC_Thieu_SPChinh,
        ...
        JOIN #Agg_SPChinh sp
          ON sp.HopDongID = cp.HopDongID
        WHERE sp.TC_Thieu_SPChinh <= 1000      -- coi như đã chạy ~100% SP chính
          AND cp.ThucChayPB_ChiPhi <10    -- Chưa có TC phân bổ CP
        ORDER BY cp.HopDongID DESC, cp.TC_Thieu_SPChiPhi DESC
        OPTION (RECOMPILE);
    END
