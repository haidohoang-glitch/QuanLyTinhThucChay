# Stored Procedure: `ThucChay_DoiTruVaTinhLai_ThanhTien_GGFB_DEV`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2021-01-07 10:15:10.267000
- **Ngày sửa cuối**: 2022-12-16 17:26:46.247000
...
Exec	[dbo].[ThucChay_DoiTruVaTinhLai_ThanhTien_GGFB] 
	@pSoHopDong ='QC2130720',
	@pHopDongChiTietID =589349,
	@pDmSanPhamREF =772,
	@NgayThucHien=  '2020-10-20'
*/

CREATE PROCEDURE [dbo].[ThucChay_DoiTruVaTinhLai_ThanhTien_GGFB_DEV] 
	@pSoHopDong NVARCHAR(100),
	@pHopDongChiTietID INT,
	@pDmSanPhamREF INT,
	@NgayThucHien DATETIME
...
BEGIN
	...
	,@GhiChuDotChayHopDong NVARCHAR(100) = ''
	,@TongTienThucChaySauChietKhauOder FLOAT = 0

	DECLARE @Table_ThucChay_ggfb TABLE(Id INT,
			Operating_Order_Id INT,	
			Operating_Result_Id INT,	
			Contract_Id INT, 
	...
	SET @GhiChu_DoiTruThucChay = N'Doi tru thuc chay thanhtien_GGFB cho SoHopDong: ' + @SoHopDong
	SET @Ghichu_TinhLaiThucChay = N'Tinh lai thuc chay thanhtien_GGFB cho SoHopDong: ' + @SoHopDong

	--THUC HIEN DOI TRU THUCCHAYDATINH THEO NGAYTHUCHIEN
	EXEC [dbo].[ThucChay_Insert_GTTD_DoiTruGiam_ThucChayDaTinh_ByHD_ThanhTien_GGFB] 
	...
	EXEC [dbo].[ThucChayDaTinh_GTTD_DoiTruGiam_ThucChayDaTinh_MuaNgoai_ThanhTien_GGFB]
	@HopDongID = @HopDongID,
	@HopDongChiTietID = @HopDongChiTietID,
	@DmSanPhamREF = @DmSanPhamREF,
	@NgayGhiNhanThucChay = @NgayThucHien,
	...
	--UPDATE TRANG THAI TINH CUA TABLE [ADS_Operating_Result_Map_Order]
	UPDATE TC
	...
	INNER JOIN 
		(SELECT OD.Contract_Id, OD.Contract_Number
			...
			WHERE OD.Contract_Id = @HopDongID 
			AND OD.Contract_Detail_Id = @HopDongChiTietID
			AND OD.IsDeleted = 0
	...
	--UPDATE TRANG THAI CUA TABLE  [ADS_Operating_Result_Quantity]
	UPDATE TC
	SET TC.IsCalc_Result_Quantity = 0
	...
	INSERT INTO @Table_ThucChay_ggfb (Id ,
			Operating_Order_Id ,	
			Operating_Result_Id ,	
			Contract_Id , 
			Contract_Detail_Id ,
	...
	WHERE hdct.HopDongChiTietID = @HopDongChiTietID AND hdct.DeletedStatus = 0 
	AND hdct.HopDongFK = @HopDongID
	AND (DmSanPhamREF in (306,423,5188) OR DmViTriREF in (100093,100478,100774) ) --AND (DmSanPhamREF in (306,423) OR DmViTriREF in (100093,100478) )
	) hd
	ON hd.HopDongChiTietID = OD.Contract_Detail_Id AND hd.HopDongFK = OD.Contract_Id
	...
	FROM (
		...
		WHERE TC.Status = 2 --DA CHOT
		AND TC.IsDeleted = 0 AND CONVERT(DATE,TC.LastModificationTime) <= @NgayThucHien
	) TC
	INNER JOIN 
		(SELECT OD.Contract_Id, OD.Contract_Number
				, OD.Contract_Detail_Id, OD.Units, OD.Id , OD.Money_Turnover
	...
	INNER JOIN (SELECT * FROM dbo.HopDongChiTiet hdct 
		WHERE hdct.HopDongChiTietID = @HopDongChiTietID AND hdct.DeletedStatus = 0 AND hdct.HopDongFK = @HopDongID
		AND (DmSanPhamREF in (306,423,5188) OR DmViTriREF in (100093,100478,100774) ) --AND (DmSanPhamREF in (306,423) OR DmViTriREF in (100093,100478) )		
	) hd
	ON hd.HopDongChiTietID = OD.Contract_Detail_Id AND hd.HopDongFK = OD.Contract_Id
	

	DECLARE R_Cursor_TinhLai_ggfb CURSOR FOR 
	...
	SELECT Id ,
			...
			UNITS , --don gia
			NganSachOrder , --ngan sach
			IS_SANLUONGCHOT  FROM @Table_ThucChay_ggfb
	ORDER BY Contract_Detail_Id,Operating_Order_Id, Id ASC

	OPEN R_Cursor_TinhLai_ggfb

	-- Perform the first fetch.
	FETCH NEXT FROM R_Cursor_TinhLai_ggfb INTO  @ADS_Operating_Result_Map_OrderId , @Operating_Order_Id, @Operating_Result_Id,
	...
	BEGIN
		...
		BEGIN
			...
			SET @GhiChuDotChayHopDong =  N'ThanhTien_GGFB'
			EXEC [dbo].[ThucChayDaTinh_InsertThucChay_GTTD_ThanhTien_GGFB_DEV]
					...
					@GhiChuDotChayHopDong				= @GhiChuDotChayHopDong,
			...
			IF ( -- nếu tồn tại trên bảng ThucChayDaTinh , thì mới thực hiện tính trên bảng TCDT_MuaNgoai
				 EXISTS(SELECT TOP (1) HopDongID 
						...
						WHERE HopDongID = @HopDongID AND HopDongChiTietREF = @HopDongChiTietID
							  ...
							  AND DmChienDichREF = @Operating_Order_Id --THEO THUC TE SO LUONG PHAT SINH
			...
			BEGIN
				...
				SET @TongTienThucChayMuaSCK = (SELECT TOP (1) TCC.Total_Money_VND FROM DBO.ADS_Operating_Result TCC WHERE TCC.ID = @Operating_Result_Id ORDER BY TCC.ID)
				...
				BEGIN
					...
					BEGIN
						...
						SET @SoLuongThucChay = IIF(@TongTienLaiThucChaySCK <= 0 ,0 ,@TongTienLaiThucChaySCK/@DonGiaTheoDonViTinh)
					END
					ELSE
					BEGIN
						--print 'vao tinh tien'
				...
				--print @TongTienLaiThucChaySCK
				----THUC HIEN INSERT GIA TRI CHENH LECH BAN MUA CUA GGFB
				EXEC [dbo].[ThucChayDaTinh_MuaNgoai_InsertThucChayLai_GTTD_ThanhTien_GGFB]
					@NgayThucHien						= @NgayThucHien,
					@ADS_Operating_Result_Map_OrderId  	= @ADS_Operating_Result_Map_OrderId,
					@HopDongREF							= @HopDongID,
					@HopDongChiTietREF					= @HopDongChiTietID,
				...
				print @ADS_Operating_Result_Map_OrderId
		...
		ELSE
			...
			BEGIN
				...
				SET @GhiChuDotChayHopDong =  N'ThanhTien_GGFB_Chot'
				--1. INSERT VAO ThucChayDaTinh
				EXEC [dbo].[ThucChayDaTinh_InsertThucChaySanLuongChot_GTTD_ThanhTien_GGFB_DEV]
						@NgayThucHien						= @NgayThucHien,
				...
				BEGIN
					...
					BEGIN
						...
						IF(@ThanhTienSauTrietKhauThucChay IS NOT NULL)
						BEGIN
							SET @TongTienThucChayBanSCK = @Sell_Money_VND - @ThanhTienSauTrietKhauThucChay
							SET @TongTienLaiThucChaySCK = @TongTienThucChayBanSCK - @TongTienThucChayMuaSCK
							SET @SoLuongThucChay = IIF(@TongTienLaiThucChaySCK <= 0 ,0 ,@TongTienLaiThucChaySCK/@DonGiaTheoDonViTinh)
						END
						ELSE
					...
					----THUC HIEN INSERT GIA TRI CHENH LECH BAN MUA CUA GGFB
					EXEC [dbo].[ThucChayDaTinh_MuaNgoai_InsertThucChayLai_SanLuongChot_GTTD_ThanhTien_GGFB]
						@NgayThucHien						= @NgayThucHien,
						@ADS_Operating_Result_Map_OrderId  	= @ADS_Operating_Result_Map_OrderId,
						@HopDongREF							= @HopDongID,
						@HopDongChiTietREF					= @HopDongChiTietID,
	...
	CLOSE R_Cursor_TinhLai_ggfb
	DEALLOCATE R_Cursor_TinhLai_ggfb
