# Stored Procedure: `ThucChay_GetSumFilterCondition_bak`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2013-11-26 11:38:21.497000
- **Ngày sửa cuối**: 2014-10-14 10:39:54.363000
...
CREATE PROCEDURE [dbo].[ThucChay_GetSumFilterCondition_bak] 
...
BEGIN
	...
	IF @Count <= 1
	BEGIN	
		SET @PhongID = (SELECT TOP 1 PhongBanREF FROM @QuaTrinhCongTacTemp)
		SET @BoPhanID = (SELECT TOP 1 BoPhanREF FROM @QuaTrinhCongTacTemp)
		SET @NhomLamViecID = (SELECT TOP 1 NhomLamViecREF FROM @QuaTrinhCongTacTemp)
		SET @ChucDanhID = (SELECT TOP 1 ChucDanhREF FROM @QuaTrinhCongTacTemp)										
    ...
    BEGIN
		...
		SELECT 
				T.GroupFieldName,T.GroupFileID,T.DonViTinh,
				T.GiaTriThayDoi,
		...
		SET @Sql ='
		SELECT 
				T.GroupFileName,T.GroupFileID,T.DonViTinh,
				T.SoLuongHopDongNoiBo,T.SoLuongHopDongKhuyenMai,T.SoLuongHopDongThucThu,
				T.SoLuongThucChayNoiBo,T.SoLuongThucChayKhuyenMai,T.SoLuongThucChayThucThu,
				T.ThanhTienNoiBo,T.ThanhTienKhuyenMai,T.ThanhTienThucChaySauChietKhau,T.ThanhTienThucThu,
				T.NgayKyHopDong
			...
			ORDER BY 
				...
				CASE WHEN @GroupFieldName <> ' +@DauNhay + 'SoHopDong' + @DauNhay + ' THEN T.ThanhTienThucThu
					ELSE T.NgayKyHopDong
				END DESC
		'
