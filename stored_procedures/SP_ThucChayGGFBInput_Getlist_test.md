# Stored Procedure: `ThucChayGGFBInput_Getlist_test`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-07-31 11:17:21.523000
- **Ngày sửa cuối**: 2017-08-10 17:17:09.607000
...
```sql
--EXEC [dbo].[ThucChayGGFBInput_Getlist] 1,500,'QC0940617',0
CREATE PROCEDURE [dbo].[ThucChayGGFBInput_Getlist_test] 
...
AS
	...
	-- xác đinh hợp đồng nào đã đươc bình nhập tay
        ...
        SELECT  hd.NgayDanhSoHopDong ,
                ...
                A.ThanhTienHopDong ,
                [NhanHopDong] = (CASE 
									WHEN ISNULL(A.NhanHopDong, '') = ''
									THEN [dbo].[fn_getListNhanHang](hd.SoHopDong,A.DmSanPhamREF,A.DonViTinh,A.DmLoaiBannerREF)
									ELSE A.NhanHopDong
								END) 
        FROM    ThucChayGGFBInput AS A
                INNER JOIN [192.168.23.217].ABM_Data_Release.dbo.HopDong hd ON A.SoHopDong = hd.SoHopDong
        WHERE   hd.DeletedStatus = 0
                AND hd.RecordStatus <> 3
                AND A.NgayThucHien = ( SELECT  MAX(NgayThucHien)
                                            FROM    ThucChayGGFBInput tc2
                                            WHERE   tc2.SoHopDong = A.SoHopDong
                                            );
    ...
    -- Insert statements for procedure here
        INSERT  INTO #tempGF
                ...
                FROM    ( SELECT    t.SoHopDong ,
                                    ...
                                    t.NhanHopDong
                          FROM      ( SELECT    hd.NgayDanhSoHopDong ,
                                                hd.HopDongID ,
                                                hd.SoHopDong ,
                                                ...
                                                [SoLuongThucChay] = ( SELECT    ISNULL(SUM(ISNULL(tcdt.SoLuongThucChay, 0) + ISNULL(tcdt.SoLuongThayDoi, 0)), 0)
																	  FROM      ThucChayDaTinh tcdt
																	  WHERE     tcdt.HopDongID = hd.HopDongID AND tcdt.HopDongChiTietREF = hdct.HopDongChiTietID
																	),
                                                [ThanhTienThucChay] = ( SELECT    ISNULL(SUM(ISNULL(tcdt.ThanhTienSauTrietKhauThucChay, 0) + ISNULL(tcdt.GiaTriThayDoi, 0)), 0)
																		FROM      ThucChayDaTinh tcdt
																		WHERE     tcdt.HopDongID = hd.HopDongID AND tcdt.HopDongChiTietREF = hdct.HopDongChiTietID
																	   ),
                                      ...
                                      FROM      [192.168.23.217].ABM_Data_Release.dbo.HopDong hd
                                                INNER JOIN [192.168.23.217].ABM_Data_Release.dbo.HopDongChiTiet hdct ON hd.HopDongID = hdct.HopDongFK
                                                INNER JOIN [192.168.23.217].ABM_Data_Release.dbo.DmLoaiBanner dlb ON hdct.DmLoaiBannerREF = dlb.DmLoaiBannerID
                                      WHERE     hdct.DmSanPhamREF IN (306, 423)
                                                ...
                                                AND NOT EXISTS ( SELECT #GoogfacebookByNgay.SoHopDong
																 ...
																 WHERE #GoogfacebookByNgay.SoHopDong = hd.SoHopDong
                ...
                IF ( @Review = ' 1=1 ' )
                    BEGIN
                        SET @SQL = 'SELECT 
									...
									FROM #tempGF
									WHERE 1=1 AND ' + @Review + ' AND RowNumber BETWEEN ' + CAST(( ( @PageIndex - 1 ) * @RecordCount + 1 ) AS NVARCHAR(500)) + ' AND ' + CAST(( @PageIndex * @RecordCount ) AS NVARCHAR(500)) + ';'; 
                ...
                ELSE
                    BEGIN
                        SET @SQL = ' INSERT into #tempGF1
									...
									ThanhTienHopDong,
									[dbo].[fn_getListNhanHang](SoHopDong,DmSanPhamREF,DonViTinh,DmLoaiBannerREF) NhanHopDong
									FROM #tempGF where 1=1 and  ' + @Review;
                        PRINT ( @SQL ); 
                        EXEC sp_executesql @SQL;
						...
						SET @SQL = 'SELECT 
									...
									dbo.FormatNumber(ThanhTienHopDong) as ThanhTienHopDong,
								   [dbo].[fn_getListNhanHang](SoHopDong,DmSanPhamREF,DonViTinh,DmLoaiBannerREF) NhanHopDong
									FROM #tempGF1 WHERE ' + ' RowNumber BETWEEN ' + CAST(( ( @PageIndex - 1 ) * @RecordCount + 1 ) AS NVARCHAR(500)) + ' AND ' + CAST(( @PageIndex * @RecordCount ) AS NVARCHAR(500)) + ';'; 
                        PRINT ( @SQL );
                        EXEC sp_executesql @SQL;
                    END;
            END;
        ELSE
            BEGIN
                ...
                FROM    ( SELECT    NgayDanhSoHopDong ,
                                    ...
                                    ChenhLech ,
                                    ThanhTienHopDong ,
                                    [NhanHopDong] = ( SELECT    TenNhanHang
													  FROM      DmNhanHang dnh
													  WHERE     CONVERT(NVARCHAR(500), dnh.DmNhanHangID) = NhanHopDong OR dnh.TenNhanHang = NhanHopDong
													),
                                    CONVERT(NVARCHAR(10), NgayThanhToan, 103) AS NgayThanhToan ,
