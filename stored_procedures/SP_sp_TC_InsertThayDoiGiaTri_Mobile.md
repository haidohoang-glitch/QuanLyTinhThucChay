# Stored Procedure: `sp_TC_InsertThayDoiGiaTri_Mobile`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-06-16 10:50:53.440000
- **Ngày sửa cuối**: 2017-06-23 17:57:49.163000
...
CREATE PROCEDURE [dbo].[sp_TC_InsertThayDoiGiaTri_Mobile]
...
AS
    BEGIN
		...
		--DECLARE @ThanhTienThucChayDaTinh FLOAT
		--DECLARE @SoLuongThucChay INT
		--DECLARE @GiaTriThayDoiHT FLOAT
		--DECLARE @SoLuongThayDoi INT
		
		--SELECT  @ThanhTienThucChayDaTinh = SUM(tcdtm.ThanhTienSauTrietKhauThucChay
  --                                                            + tcdtm.GiaTriThayDoi) ,
  --                                              @SoLuongThucChay = SUM(tcdtm.SoLuongThucChay
  --                                                            + tcdtm.SoLuongThayDoi)
  --                                      FROM    dbo.ThucChayDaTinhMobile tcdtm
  --                                      WHERE   tcdtm.SoHopDong =@SoHopDong
  --                                              AND tcdtm.HopDongChiTietREF = @HopDongChiTietID
  --                                              AND tcdtm.NgayThucHien <= @NgayThucHien
		--										AND tcdtm.TenWebsite = @TenWebsite
  ...
  --                                          - @SoLuongThucChay; 
	...
	--(SELECT TOP 1 DmWebsiteReportingdbID FROM DmWebsiteReportingdb WHERE DmWebsiteReportingdb.TenWebsite = @TenWebsite) DmWebsiteREF,
                                                ...
                                                0 TongSoBaiViet ,
                                                dbo.[fn_TC_GetSoLuongThucChayMobile](@HopDongChiTietID,
											...
											--else @ProductUnitName 
                                                              ...
                                                              THEN 'CPM'
                                                              END )
                                                              END, @DmBannerID) AS SoLuongThucChay ,
                ...
                WHERE   ( TC.SoLuongThucChay IS NOT NULL
