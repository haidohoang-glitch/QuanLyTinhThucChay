# Stored Procedure: `update_thucchayadmarket`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-01-07 16:22:29.377000
- **Ngày sửa cuối**: 2017-01-07 16:25:21.160000
...
```sql
CREATE PROC [dbo].[update_thucchayadmarket]
 @SoHopDong NVARCHAR(50),
@HopDongChiTietID int
AS
BEGIN
DECLARE @NgayThucHien DATETIME, @SoLuongThucChay INT, @ThanhTienTTCK FLOAT , @GiaTriThayDoi INT
...
, @a FLOAT , @b FLOAT, @DmSanPhamREF INT, @a1 FLOAT , @b1 FLOAT
   ...
   SET @b = (SELECT SUM(ThanhTienSauTrietKhauThucChay+GiaTriThayDoi) FROM dbo.ThucChayDaTinhAdmarket WHERE HopDongChiTietREF = @HopDongChiTietID AND NgayThucHien<=@NgayThucHien)
   set @GiaTriThayDoi =    @a- @b

   SET @NhanHang = (SELECT TOP 1 NhanHang FROM ThucChayDaTinhAdmarket WHERE HopDongChiTietREF = @HopDongChiTietID AND NhanHang <> '0'ORDER BY NgayThucHien DESC)
...
SELECT
	...
	0 PrintStatus,
	0 RecordStatus
FROM dbo.ThucChayDaTinhAdmarket tcdt WHERE
