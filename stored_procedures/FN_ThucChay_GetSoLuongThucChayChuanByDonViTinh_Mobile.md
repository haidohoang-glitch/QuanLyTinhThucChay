# Function: `ThucChay_GetSoLuongThucChayChuanByDonViTinh_Mobile`

- **Loại**: SQL_SCALAR_FUNCTION
- **Ngày tạo**: 2014-09-23 14:09:56.097000
- **Ngày sửa cuối**: 2015-04-08 15:47:52.363000
...
-- =============================================
CREATE FUNCTION [dbo].[ThucChay_GetSoLuongThucChayChuanByDonViTinh_Mobile] 
(
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			(
				SELECT SUM(ISNULL(tc.SoLuongThucChay,0)) FROM ThucChayDaTinhMobile tc
		...
		ELSE
			BEGIN
				SET @v_tongviewthucchay =
				(
					SELECT SUM(ISNULL(tc.SoLuongThucChayKM,0)) FROM ThucChayDaTinhMobile tc
					WHERE tc.HopDongChiTietREF = @HopDongChiTietREF
					--AND tc.NgayThucHien BETWEEN @MinDate AND @NgayThucHien
					AND tc.NgayThucHien <= @NgayThucHien
				)	
