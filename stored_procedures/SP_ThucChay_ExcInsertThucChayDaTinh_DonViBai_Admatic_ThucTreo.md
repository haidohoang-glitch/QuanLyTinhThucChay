# Stored Procedure: `ThucChay_ExcInsertThucChayDaTinh_DonViBai_Admatic_ThucTreo`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2021-06-23 11:14:42.590000
- **Ngày sửa cuối**: 2023-08-30 14:38:17.177000
...
CREATE PROCEDURE [dbo].[ThucChay_ExcInsertThucChayDaTinh_DonViBai_Admatic_ThucTreo] 
	@StartDate datetime,
	@EndDate datetime
AS
BEGIN
	...
	AND ((DmSanPhamREF  in (305,5312) ) OR	(DmSanPhamREF  = 598 AND DmVitriREF = 9198))  --29052023 haidh comment them donvibai cho adpage admatic
	--20230821 them san pham 5312 - Brand Protection page
	...
	BEGIN
		...
		SELECT DISTINCT 0 as ThucChayID, hd.SoHopDong,  hd.HopDongID, hd.DmSanPhamREF, hd.TenSanPham,  0 AS DmBannerREF
			, @NgayThucHien AS NgayThucHien, hd.HopDongChiTietID as HopDongChiTietREF, hd.ThucChayHopDongChiTietID
			...
			(
				...
				AND ((hdct.DmSanPhamREF in (305,5312)) OR (hdct.DmSanPhamREF = 598 AND hdct.DmViTriREF = 9198)) --haidh 29052023 comment them adpage admatic donvi bai
			...
			BEGIN
				...
				BEGIN
					...
					IF(NOT EXISTS(SELECT tcdt.HopDongChiTietREF, SUM(tcdt.ThanhTienSauTrietKhauThucChay + tcdt.GiaTriThayDoi)  FROM dbo.ThucChayDaTinh tcdt
						...
						AND tcdt.HopDongChiTietREF = @HopDongChiTietREF
						AND tcdt.SoLuongDotChayHD = @ThucChayHopDongChiTietREF --haidh comment 2021-07-23
