# Stored Procedure: `ThucChay_Admatic_GhiNhanThayDoi_ThucChayDaTinh_bk20251008`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-10-08 14:47:51.710000
- **Ngày sửa cuối**: 2025-10-08 14:47:51.710000
...
--EXEC [dbo].[ThucChay_Admatic_GhiNhanThayDoi_ThucChayDaTinh] @NgayGhiNhan = '2025-05-30',       -- date
--                                                            @SoHopDong = N'QC2470525',                  -- nvarchar(50)
--                                                            @HopDongChiTietID = 757462              -- int




create PROCEDURE [dbo].[ThucChay_Admatic_GhiNhanThayDoi_ThucChayDaTinh_bk20251008]
	@NgayGhiNhan DATE,
...
BEGIN
	...
	BEGIN
		...
		WHERE
			 ...
			 NOT (EXISTS(SELECT HopDongChiTietREF 
								 FROM ABM_Data_ThucChay.dbo.DmThongTinHopDongBanInventory 
        ...
        FROM ABM_Data_ThucChay.dbo.ThucChayHopDongChiTietAndBanner_ThanhTien_Admatic tc
		...
		INNER JOIN (SELECT DISTINCT bn.DmBannerREF
					FROM #DmPBThayDoi dm
					INNER JOIN ABM_Data_ThucChay.dbo.ThucChayHopDongChiTietAndBanner_ThanhTien_Admatic bn
		...
		WHERE	 hd.TrangThaiHopDong NOT IN (0,3) AND
				 ...
				 hdct.DonViTinhREF <> 7 AND
				 hdct.DmSanPhamREF IN (231,238,339,240,598,613,370,680, 733,735,821,342,585,5133,5056,5268, 5367) AND 
				 NOT ( hdct.DmLoaiREF = 13 OR hdct.DmLoaiBannerREF = 18) AND 
				 ...
				 AND tc.DmSanPhamREF IN (231,238,339,240,598,613,370,680, 733,735,821,342,585,5133,5056,5268, 5367) 
				 AND tc.[DeletedStatus] = 0

				 AND (@NgayDanhSoGioiHan IS NULL OR hd.NgayDanhSoHopDong >= @NgayDanhSoGioiHan) 
		----UNION THEM PHAN CUA MKT-FEE
	...
	WHERE   
			...
			AND CAST(tc.NgayThucHien AS DATE) < @NgayGhiNhan
...
SELECT	  NEWID()
		...
		, N'Đối trừ: SP tối ưu [dbo].[ThucChay_Admatic_GhiNhanThayDoi_ThucChayDaTinh] do ' + dm.LyDo AS GhiChu
...
GROUP BY  tcdt.HopDongID
	...
	SELECT	  NEWID()
			...
			, SUM(SoLuongThucChayKM + SoLuongKMThayDoi) AS SoLuongKMThayDoi
			, SUM(ThanhTienKM + GiaTriKMThayDoi) AS GiaTriKMThayDoi
			, N'Tính lại: SP tối ưu [dbo].[ThucChay_Admatic_GhiNhanThayDoi_ThucChayDaTinh] do ' + dm.LyDo AS GhiChu
	FROM    ABM_Data_ThucChay.dbo.[ThucChayDaTinh] tcdt
