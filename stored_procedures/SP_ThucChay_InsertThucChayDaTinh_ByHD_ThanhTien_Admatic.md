# Stored Procedure: `ThucChay_InsertThucChayDaTinh_ByHD_ThanhTien_Admatic`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-06-23 10:40:47.460000
- **Ngày sửa cuối**: 2023-06-21 10:15:32.457000
...
CREATE  PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_ByHD_ThanhTien_Admatic] 
...
BEGIN
	...
	DECLARE @DotChayHopDong NVARCHAR(100) = N'ThanhTien_Admatic'
	...
	SELECT  NEWID() AS ThucChayDaTinhID,
		...
		(
			...
			(CASE WHEN HD.ChietKhau <> 100 THEN ([dbo].[ThucChay_GetSoLuongThucChay_ThanhTien_Admatic] (tc.SoLuongThucChay, tc.ThanhTienThucChaySauCK, tc.SoLuongThucChayKM, tc.ThanhTienThucChayKM,
			...
			(CASE WHEN HD.ChietKhau <> 100 THEN ([dbo].[ThucChay_ThanhTienThucChay_ThanhTien_Admatic] (tc.ThanhTienThucChaySauCK, tc.ThanhTienThucChayKM, @NgayThucHien,
		...
		FROM
		(
			SELECT tc.SoHopDong, tt.HopDongREF, tt.HopDongChiTietREF, tc.DmSanPhamREF, tc.DmBannerID, tc.TenWebsite, tc.DmWebsiteID
			...
			FROM
			(
				SELECT tct.SoHopDong, tct.DmSanPhamREF, tct.DmWebsiteID, tct.TenWebsite, tct.DmBannerID, tct.SoLuongThucChay, tct.SoLuongThucChayKM
				, tct.ThanhTienThucChaySauCK_ChuaVAT as ThanhTienThucChaySauCK, tct.ThanhTienThucChayKM, tct.DonViTinh, tct.NgayThucHien
			...
			INNER JOIN 
			(	SELECT HopDongREF, HopDongChiTietREF, DmSanPhamREF, DmBannerREF, TiLeThucChayHDCTSoVoiBanner 
				FROM dbo.ThucChayHopDongChiTietAndBanner_ThanhTien_Admatic 
				WHERE HopDongREF = @HopDongID
				AND DmSanPhamREF = @DmSanPhamREF AND DmBannerREF = @DmBannerID
				AND DeletedStatus = 0
			)tt ON CONVERT(NVARCHAR(100),tc.DmBannerID) = tt.DmBannerREF
			WHERE 1=1--them vao cho nay de check dieu kien san pham chay va danh so kho giong nhau tru san pham 733 haidhcomment 24/11/2020
			AND EXISTS(SELECT TOP (1) hdct.HopDongChiTietID FROM dbo.HopDongChiTiet hdct 
				WHERE hdct.HopDongChiTietID = tt.HopDongChiTietREF
		...
		(SELECT hd.HopDongID,
			...
			(SELECT * FROM dbo.HopDongChiTiet hdct 
				...
				AND hdct.DmLoaiREF = 42
				--HAIDH COMMENT LẠI DIEU KIEN BEN DUOI KHI TINH THEO DON VI NGAY 21/06/2023
				--AND not hdct.DonViTinhREF in (3,4) --KHONG PHAI DON VI TINH NGAY, TUAN 
				AND hdct.DmLoaiNenTangREF <> 9 --Direct Tag
