# Stored Procedure: `prc_asd_ThucChayDaTinhAdmarket_InsertByPhanBoID_ThayDoiHopDong_doannv_test`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-11-04 11:08:46.190000
- **Ngày sửa cuối**: 2017-11-04 11:12:08.963000
...
-- =============================================
CREATE PROCEDURE [dbo].[prc_asd_ThucChayDaTinhAdmarket_InsertByPhanBoID_ThayDoiHopDong_doannv_test]	
...
BEGIN
	...
	begin
		declare @i int  =1 , @giatrivitri1 money, @giatrivitri2 money, @giatrivitri3 money,
		@giatri1 money ,@giatri2 money , @giatri3 money
		select @giatrivitri1 = sum(ThanhTienSauTrietKhauThucChay + GiaTriThayDoi) from ThucChayDaTinhAdmarket where HopDongChiTietREF = @PhanBoID and DmViTriREF = 1
		select @giatrivitri2 = sum(ThanhTienSauTrietKhauThucChay + GiaTriThayDoi) from ThucChayDaTinhAdmarket where HopDongChiTietREF = @PhanBoID and DmViTriREF = 2
		select @giatrivitri3 = sum(ThanhTienSauTrietKhauThucChay + GiaTriThayDoi) from ThucChayDaTinhAdmarket where HopDongChiTietREF = @PhanBoID and DmViTriREF = 3
		set @giatrivitri1 = isnull(@giatrivitri1,0)
		set @giatrivitri2 = isnull(@giatrivitri2,0)
		set @giatrivitri3 = isnull(@giatrivitri3,0)
		set @giatri1 = @giatrivitri1/(@giatrivitri1 + @giatrivitri2 + @giatrivitri3) * @ThanhTienThucChay
		set @giatri2 = @giatrivitri2/(@giatrivitri1 + @giatrivitri2 + @giatrivitri3) * @ThanhTienThucChay
		set @giatri3 = @ThanhTienThucChay - @giatri1 - @giatri2
		while @i <=3
			begin
			declare @giatrithaydoi money if @i = 1 set @giatrithaydoi = @giatri1
	...
	SELECT DISTINCT
		...
		C.DmLoaiBannerREF DmLoaiBannerREF, 
		C.TenLoaiBanner TenLoaiBanner, 
		case when @i = 1 then 1 when @i= 2 then 2 else 3 end DmViTriREF, 
		case when  @i = 1  then 'AdX'
	         when  @i= 2   then 'AdX Mobile'
			 when  @i =3   then 'AdX Ecommerce' end TenViTri, 
		--ISNULL(dbo.GetDotChayBookingByHopDongChiTiet(C.HopDongChiTietID,'Y'),'') DotChayHopDong,
		@GhiChu DotChayHopDong,
		C.SoLuong AS SoLuongDotChayHD,
		ISNULL(dbo.GetDotChayBookingByHopDongChiTiet(C.HopDongChiTietID,'N'),0) DotChayBooking,
