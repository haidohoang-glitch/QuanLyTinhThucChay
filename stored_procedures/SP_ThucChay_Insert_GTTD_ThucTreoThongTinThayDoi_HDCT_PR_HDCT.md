# Stored Procedure: `ThucChay_Insert_GTTD_ThucTreoThongTinThayDoi_HDCT_PR_HDCT`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-02-05 14:50:56.070000
- **Ngày sửa cuối**: 2024-10-29 10:09:51.527000
...
CREATE PROCEDURE [dbo].[ThucChay_Insert_GTTD_ThucTreoThongTinThayDoi_HDCT_PR_HDCT]
...
AS
    BEGIN
		...
		--1. THUC HIEN DOI TRU THONG TIN DE THONG TIN TREO TRO VE 0
        INSERT  INTO dbo.ThucChayDaTinh
				...
				OUTPUT INSERTED.ThucChayDaTinhID, INSERTED.HopDongID, @ThucChayHopDongChiTietPRID INTO @Table_thucchaydatinhid
                SELECT  NEWID() ,
                        HopDongID ,
                        SoHopDong ,
                        DmMaHopDongREF ,
                ...
                WHERE   HopDongID = @HopDongID
						--AND HopDongChiTietREF = @HopDongChiTietID --Check voi TH HopDongChiTietID thay doi
       ...
       --CAP NHAP TRANG THAI THUC TREO SAU KHI TINH THUC CHAY
		...
		DECLARE @v_ThucchayDatinhID NVARCHAR(100)
		SET @v_ThucchayDatinhID = ISNULL((SELECT top (1) t.ThucChayDaTinhID FROM @Table_thucchaydatinhid t ORDER BY t.ThucChayDaTinhID),'')
       --CAP NHAP TRANG THAI THUC TREO SAU KHI TINH THUC CHAY
	   IF(EXISTS(SELECT Top 1 tcdt.HopDongID FROM dbo.ThucChayDaTinh tcdt 
			WHERE tcdt.ThucChayDaTinhID = @v_ThucchayDatinhID
			AND tcdt.NgayThucHien = @NgayThucHien
			AND tcdt.HopDongID = @HopDongID
			AND tcdt.DotChayBooking = @ThucChayHopDongChiTietPRID ORDER BY tcdt.ThucChayDaTinhID))
		BEGIN
			...
			--xoa dl tabe tam
			DELETE FROM @Table_thucchaydatinhid
		END
		

		--2. THUC HIEN TINH LAI GIA TRI THUC TREO
		--CHECK GIA TRI THANH TIEN VÀ THANHTIENTHUCCHAYDATINH CỦA THỰC TREO
		DECLARE @v_HopDongChiTietREF int, @HopDongREF int, @ThanhTienTCDT FLOAT = 0, @ThanhTien FLOAT = 0, @ChietKhauHDC FLOAT
		,@SoLuongThucChay INT , @ThanhTienThucChaytruocCK FLOAT, @DonGiaChay FLOAT, @ChietKhauChay FLOAT

		SELECT @v_HopDongChiTietREF = hdct.HopDongChiTietID, @ThanhTien = hdct.ThanhTien 
		, @ChietKhauHDC = hdct.chietkhau
		FROM dbo.HopDongChiTiet hdct
		WHERE hdct.HopDongFk = @HopDongID
		AND EXISTS(SELECT TOP (1) ThucChayHopDongChiTietPRID FROM dbo.ThucChayHopDongChiTietPR
					WHERE ThucChayHopDongChiTietPRID = @ThucChayHopDongChiTietPRID AND HopDongREF= @HopDongID
					AND HopDongChiTietREF = hdct.HopDongChiTietID
					 ORDER BY ThucChayHopDongChiTietPRID)
		
		SELECT TOP (1) @SoLuongThucChay = tt.soluong, @DonGiaChay = tt.GiaTien, @ChietKhauChay = tt.ChietKhau  
					FROM dbo.ThucChayHopDongChiTietPR tt
					WHERE tt.ThucChayHopDongChiTietPRID = @ThucChayHopDongChiTietPRID 
					AND tt.HopDongREF= @HopDongID
					 ORDER BY tt.ThucChayHopDongChiTietPRID


		SET @v_HopDongChiTietREF = ISNULL(@v_HopDongChiTietREF,0)
		SET @ThanhTien = ISNULL(@ThanhTien,0)
		SET @ChietKhauHDC = ISNULL(@ChietKhauHDC,0)
		SET @SoLuongThucChay = ISNULL(@SoLuongThucChay,0)		
		SET @DonGiaChay = ISNULL(@DonGiaChay,0)
		SET	@ChietKhauChay = ISNULL(@ChietKhauChay,0)

		IF NOT((@ChietKhauChay = 0 AND @ChietKhauHDC = 100) OR (@ChietKhauChay = 100 AND @ChietKhauHDC = 0))
		BEGIN
			SET @ThanhTienThucChaytruocCK = ISNULL([dbo].[ThucChay_GetThanhTienThucChayTruocChietKhau_PR]
			(
			...
			SELECT  NEWID() , T.HopDongID,
				...
				FROM    ( SELECT  DISTINCT
									...
									0 TongClickThucChay ,
									0 TongSoBaiViet ,
									( CASE WHEN ISNULL(tchpctpr.KhuyenMai,0) = 0 AND tchpctpr.ChietKhau <> 100 THEN ISNULL(tchpctpr.SoLuong, 0)
											ELSE 0
									...
									( CASE WHEN ( ISNULL(tchpctpr.KhuyenMai,0) = 1 OR tchpctpr.ChietKhau = 100 OR tchpctpr.GiaTien = 0 OR tchpctpr.SoLuong = 0 ) THEN 0
									...
									( CASE WHEN ISNULL(tchpctpr.KhuyenMai,0) = 1 OR tchpctpr.ChietKhau = 100 THEN ISNULL(tchpctpr.GiaTien, 0) * ISNULL(tchpctpr.SoLuong, 0)
											ELSE 0
										END ) AS ThanhTienKM ,

									( CASE WHEN ISNULL(tchpctpr.KhuyenMai,0) = 1 OR tchpctpr.ChietKhau = 100 THEN ISNULL(tchpctpr.SoLuong, 0)
											ELSE 0
									...
									0 SoLuongKMThayDoi ,
									0 GiaTriKMThayDoi ,
									N'Tinh lại thực treo trên phân bổ mới' GhiChu
							FROM     (
						...
						--WHERE T.ThanhTienThucChayTruocChietKhau >0  --haidh COMMENT 2021-03-29 bo di vi voi th thay doi phan bo thi bi sai
			END
			SET @v_ThucchayDatinhID = ''
			SET @v_ThucchayDatinhID = ISNULL((SELECT top (1) t.ThucChayDaTinhID FROM @Table_thucchaydatinhid t ORDER BY t.ThucChayDaTinhID),'')
