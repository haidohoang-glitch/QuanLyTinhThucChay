# Stored Procedure: `ThucChay_Update_GTTD_ThucChayDaTinh_HopDongInventory`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2016-12-30 11:06:29.653000
- **Ngày sửa cuối**: 2025-12-22 09:33:13.207000
...
-------------------------------------------------------------
CREATE PROCEDURE [dbo].[ThucChay_Update_GTTD_ThucChayDaTinh_HopDongInventory] 
...
BEGIN
		...
		, @NgayDanhSoGioiHanMKT_FEE_PRO DATETIME = N'2025-10-10'
		...
		WHERE 1=1
		AND NOT (hdcttd.DmLoaiREF in (13) or DmLoaiBannerREF = 18)--Khong update gia tri thay doi cho HTQC Mua Ngoai 
		AND convert(date,hdtd.NgayThayDoi) = @NgayThucHien
		--AND (hdcttd.DmLoaiREF = 42 
		--	AND (hdcttd.DmSanPhamREF NOT IN (733) or ((hdcttd.DmSanPhamREF = 817) AND (hd.NgayDanhSoHopDong >= @NgayDanhSoGioiHanMKT_FEE_PRO)))
		--) --Tinh cho phan bo Admatic , Direct Tag	
		--AND hd.HopDongID IN (SELECT DISTINCT HopDongREF FROM dbo.DmThongTinHopDongBanInventory)
		AND EXISTS (SELECT top 1 v.HopDongChiTietREF FROM dbo.DmThongTinHopDongBanInventory v
		WHERE v.HopDongChiTietREF = hdcttd.HopDongChiTietREF ORDER BY v.HopDongChiTietREF)
