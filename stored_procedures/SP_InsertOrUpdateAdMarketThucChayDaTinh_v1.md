# Stored Procedure: `InsertOrUpdateAdMarketThucChayDaTinh_v1`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-01-27 11:55:16.087000
- **Ngày sửa cuối**: 2017-01-16 17:52:14.287000
...
```sql
CREATE PROCEDURE [dbo].[InsertOrUpdateAdMarketThucChayDaTinh_v1] (
...
AS
BEGIN
	INSERT INTO [ThucChayCPCAdmarket]
	SELECT @NgayThucHien, @siteid, @domain, @ttc, @ttv, @money,@promotion, @TenSanpham, @DmSanPhamREF, @type
	...
	IF @DmSanPhamREFWell = 337
	BEGIN
		SET @DmHinhThucQuangCao = 6
		SET @TenHinhThucQuangCao = 'CPM'
	END
	ELSE
		BEGIN
			SET @DmHinhThucQuangCao = 7
			SET @TenHinhThucQuangCao = 'CPC'
		END
	...
	BEGIN
	    ...
	    SET @siteid = @@IDENTITY
	END	
   ---- delete dữ liệu truoc khi insert
   --DELETE FROM ThucChayDaTinh WHERE CONVERT(date,NgayThucHien) = Convert(date,@NgayThucHien)
   --AND DmSanPhamREF = @DmSanPhamREF AND TenSanPham = @TenSanpham
--CPM Admarket, CPC Admarket, CPCPlus Admarket for Domain
