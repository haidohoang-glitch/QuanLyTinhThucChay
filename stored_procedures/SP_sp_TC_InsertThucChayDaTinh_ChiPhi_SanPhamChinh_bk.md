# Stored Procedure: `sp_TC_InsertThucChayDaTinh_ChiPhi_SanPhamChinh_bk`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2019-07-05 15:02:35.217000
- **Ngày sửa cuối**: 2019-07-05 15:02:35.217000
...
-------------------------------------------------------------
CREATE PROCEDURE [dbo].[sp_TC_InsertThucChayDaTinh_ChiPhi_SanPhamChinh_bk]
    @StartDate DATETIME ,
    @EndDate DATETIME,
...
---------***********DANH SACH CAC SAN PHAM CHINH CO TINH CHI PHI*******-------------
    ...
    --                WHERE   DmSanPhamREF IN ( 140, 228, 549, 564, 375, 231,
    --                                          238, 337, 531, 370, 339, 342,
    --                                          381, 240, 680, 598 , 680, 821)
    --                        AND hdct.DeletedStatus = 0
    --                        AND DmLoaiBannerREF = 17 --Loai banner CHI PHI cua sanpham chinh
    --                        AND ISNULL(dbo.ThucChay_GetSoLuongThucChayChuanByDonViTinh_ChiPhi_SanPhamChinh(@NgayThucHien,
    --                                                          @NgayGioiHanTinh,
    --                                                          hdct.HopDongChiTietID),
    --                                   0) > 0
    --                        AND ( SELECT    COUNT(HopDongChiTietREF)
    --                              FROM      ThucChayDaTinh
    --                              WHERE     ThucChayDaTinh.HopDongChiTietREF = hdct.HopDongChiTietID
    --                            ) > 0
    --                        AND ( SELECT    SUM([ThanhTienSauTrietKhauThucChay]
    --                                            + [GiaTriThayDoi])
    --                              FROM      ThucChayDaTinh
    --                              WHERE     ThucChayDaTinh.HopDongChiTietREF = hdct.HopDongChiTietID
    --                            ) > 0
				--			AND (@HopDongID IS NULL OR hdct.HopDongFK = @HopDongID)
                ...
                INSERT  INTO dbo.ThucChayDaTinh
                        ...
                        FROM    ( SELECT 
	                             ...
	                             FROM      (	
												...
												WHERE DmSanPhamREF IN ( 140, 228, 549, 564, 375, 231, 238, 337, 531, 370, 339, 342, 381, 240, 680, 598, 680, 821)
												AND DeletedStatus = 0
												AND DmLoaiBannerREF = 17 --Loai banner CHI PHI cua sanpham chinh
                                            ) C
                                            INNER JOIN 
