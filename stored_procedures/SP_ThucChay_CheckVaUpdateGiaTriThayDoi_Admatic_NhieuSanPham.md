# Stored Procedure: `ThucChay_CheckVaUpdateGiaTriThayDoi_Admatic_NhieuSanPham`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-02-09 17:12:44.270000
- **Ngày sửa cuối**: 2018-07-23 16:52:12.843000
...
```sql
--EXEC [dbo].[ThucChay_CheckVaUpdateGiaTriThayDoi_Admatic_NhieuSanPham] '2018-06-12'
CREATE  PROCEDURE [dbo].[ThucChay_CheckVaUpdateGiaTriThayDoi_Admatic_NhieuSanPham] 
	@NgayThucHien DATETIME
AS
BEGIN
	DECLARE @HopDongID INT , @NgayThayDoi DATETIME, @HopDongChiTietID INT
	...
	--SET @NgayThucHien = '2017-01-19'
	
	DECLARE Cursor_hopdong_admatic CURSOR FOR

		--XAC DINH DANH SACH CÁC PHAN BO CO THAY DOI THONG TIN THANHTIEN HOAC CHIETKHAU	
		SELECT DISTINCT hdtd.HopDongFK, CONVERT(DATE,hdtd.NgayThayDoi)NgayThayDoi, hdcttd.HopDongChiTietREF
		, hdct.ChietKhau, hdcttd.ChietKhau AS ChietKhau_Bf
		...
		--AND hdtd.HopDongFK = 501207
		--AND hdcttd.HopDongChiTietREF = 503046

	OPEN Cursor_hopdong_admatic
	FETCH NEXT FROM Cursor_hopdong_admatic INTO @HopDongID , @NgayThayDoi , @HopDongChiTietID , @ChietkhauHT, @ChietKhau_Bf
	WHILE @@FETCH_STATUS =0
	BEGIN
		--NEU PHAN BO CO THAY DOI CHIET KHAU THI THUC HIEN DOI TRU DI VA TINH LẠI TOAN BO
		IF(@ChietKhau_Bf <> @ChietkhauHT)
		BEGIN
	...
	FETCH NEXT FROM Cursor_hopdong_admatic INTO @HopDongID , @NgayThayDoi , @HopDongChiTietID , @ChietkhauHT, @ChietKhau_Bf
	END
	CLOSE Cursor_hopdong_admatic;
	DEALLOCATE Cursor_hopdong_admatic;
