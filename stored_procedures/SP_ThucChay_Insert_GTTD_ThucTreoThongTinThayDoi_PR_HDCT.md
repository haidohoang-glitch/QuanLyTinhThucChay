# Stored Procedure: `ThucChay_Insert_GTTD_ThucTreoThongTinThayDoi_PR_HDCT`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2019-12-10 16:54:41.847000
- **Ngày sửa cuối**: 2024-10-26 12:01:06.797000
...
CREATE PROCEDURE [dbo].[ThucChay_Insert_GTTD_ThucTreoThongTinThayDoi_PR_HDCT]
...
AS
    BEGIN
		...
		--THUC HIEN DOI TRU THONG TIN DE THONG TIN TREO TRO VE 0
	   ...
	   IF(EXISTS(SELECT Top 1 tcdt.HopDongID FROM dbo.ThucChayDaTinh tcdt 
			...
			AND tcdt.HopDongChiTietREF = @HopDongChiTietID ORDER BY tcdt.ThucChayDaTinhID))
		BEGIN
		...
		--CHECK DIEU KIEN TINH LAI THUC CHAY
		DECLARE @ThanhTienThucChayTruocCK FLOAT,
		@ChietKhau_HDCT FLOAT,
		@ThanhTien_HDCT FLOAT,
		@SoLuongThucChay INT,
		@DonGiaThucChay FLOAT,
		@ChietKhauThucChay FLOAT

		SELECT TOP (1) @ChietKhau_HDCT = ISNULL(hdct.ChietKhau,0), @ThanhTien_HDCT = ISNULL(hdct.ThanhTien,0) 
					FROM dbo.HopDongChiTiet hdct
					WHERE hdct.HopDongChiTietID = @HopDongChiTietID
					AND hdct.DeletedStatus = 0
					ORDER BY hdct.HopDongChiTietID

		SELECT TOP (1) @ChietKhauThucChay = PR.ChietKhau ,
							@DonGiaThucChay = PR.GiaTien ,
                            @SoLuongThucChay = PR.SoLuong
                    FROM dbo.ThucChayHopDongChiTietPR PR
					WHERE PR.ThucChayHopDongChiTietPRID = @ThucChayHopDongChiTietPRID
					AND PR.HopDongREF= @HopDongID
					AND PR.HopDongChiTietREF = @HopDongChiTietID
		...
		SET @DonGiaThucChay = ISNULL(@DonGiaThucChay,0)
		SET @ChietKhauThucChay = ISNULL(@ChietKhauThucChay,0)
