# Stored Procedure: `BC_HopDongChuaTreoDuChiPhi_PerformanceBase_TCDT`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-10-07 18:06:29.987000
- **Ngày sửa cuối**: 2025-11-27 17:39:29.887000
...
```sql
--Performance Base: Cảnh báo CHƯA treo chi phí các hợp đồng có hạng mục SP chính đã chạy 50%
--Performance Base: Cảnh báo CHƯA TREO ĐỦ chi phí các hợp đồng có hạng mục SP chính đã chạy xong
CREATE PROCEDURE dbo.BC_HopDongChuaTreoDuChiPhi_PerformanceBase_TCDT
    @Type   INT       = 2,           -- 2: đã chạy ~100% SP chính nhưng CHƯA treo đủ CP
                                     -- 1: đã chạy >=50% SP chính nhưng CHƯA treo CP
    @ToDate DATETIME = NULL
AS
BEGIN
    SET NOCOUNT ON;

    IF @Type NOT IN (1,2) SET @Type = 2;
    ...
    /* 1) Phân bổ CHI PHÍ (Performance base) */
    ...
    WHERE hd.TrangThaiHopDong NOT IN (0,3)
      AND hdct.DmLoaiREF IN (26,5010,5038,5000)  -- Performance base
	  AND  CAST(
                IIF(hdct.ChietKhau = 100,
                    hdct.SoLuong * hdct.DonGia,
                    hdct.ThanhTien)
            AS DECIMAL(19,2)) <> 0
      AND (
            hdct.DmSanPhamREF IN (251,253,535,560,630,817,5254,5280,5281,5282,5325)
            OR hdct.TenSanPham LIKE N'%chi phí%'
          );
   ...
   --         sp.TC_Thieu_SPChinh,
        ...
        JOIN #Agg_SPChinh sp
          ON sp.HopDongID = cp.HopDongID
        WHERE (CASE WHEN sp.TongTienSPChinh > 0
                    THEN sp.TongThucChaySPChinh / sp.TongTienSPChinh
                    ELSE 0 END) BETWEEN 0.5  AND 0.99   -- chạy >= 50% SP chính
          AND cp.ThucChayPB_ChiPhi = 0    -- Chưa có TC phân bổ CP
		  AND cp.ChietKhau <> 100
        ORDER BY cp.HopDongID DESC, cp.TC_Thieu_SPChiPhi DESC
        OPTION (RECOMPILE);
    END
