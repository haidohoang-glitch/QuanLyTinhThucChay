# Stored Procedure: `sp_TC_CheckHopDongCoThayDoi_Native_Ads`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-09-26 15:17:31.163000
- **Ngày sửa cuối**: 2023-05-16 14:30:33.703000
...
CREATE  PROCEDURE [dbo].[sp_TC_CheckHopDongCoThayDoi_Native_Ads] 
...
BEGIN
	...
	@EndDate = MAX(TC.NgayThucHien) FROM dbo.ThucChay_Native_Ads TC
	WHERE TC.SoHopDong = @SoHopDong
	AND tc.DmSanPhamREF = @DmSanPhamREF

	SET @SoLuongThucChay = ISNULL(@SoLuongThucChay,0)
	SET @StartDate = convert(date,ISNULL(@StartDate,@NgayThayDoiMax))
	SET @EndDate = convert(date,ISNULL(@EndDate,@NgayThayDoiMax))
