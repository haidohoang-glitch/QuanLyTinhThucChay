# Stored Procedure: `Gen_InsertOrUpdate_ThucChayHopDongChiTiet_ThucTreo`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2019-06-28 09:52:19.423000
- **Ngày sửa cuối**: 2025-08-26 10:12:12.673000
...
*/
CREATE PROCEDURE [dbo].[Gen_InsertOrUpdate_ThucChayHopDongChiTiet_ThucTreo] 	
As 	
BEGIN

DECLARE @NgayThucHien DATETIME, @DmWebsiteREF INT = 265, @TenWebsite NVARCHAR(100) = N'(Blanks)'
...
SET @SQL +=
	'SELECT   tt.Id,
			 tt.Contract_Id,
			 tt.TenNhanHang,
			 IIF(tt.ThoiGianBatDau = '+ @daunhay + '0001-01-01' + @daunhay +',' + @daunhay +'2010-01-01' + @daunhay + ',IIF(tt.ThoiGianBatDau IS NULL, ' + @daunhay + '2010-01-01' + @daunhay + ',tt.ThoiGianBatDau))ThoiGianBatDau,
			 IIF(tt.ThoiGianKetThuc = ' + @daunhay + '0001-01-01' + @daunhay+ ',' + @daunhay + '2010-01-01' + @daunhay + ',IIF(tt.ThoiGianKetThuc IS NULL, ' + @daunhay + '2010-01-01' + @daunhay + ',tt.ThoiGianKetThuc))ThoiGianKetThuc,
			 tt.Link,
			 tt.Banner_Id,
			 tt.TenBanner,
			 tt.Zone_Name,
			 ...
			 ISNULL((SELECT TOP 1 t.TenHinhThucQuangCao FROM dbo.DmHinhThucQuangCao t WHERE t.DmHinhThucQuangCaoID = tt.Product_Formality_Id ORDER BY t.DmHinhThucQuangCaoID), ' + @daunhay +' ' + @daunhay + ') TenHinhThucQuangCao,
			 tt.Product_Id,
			 '''' Product_name,
             ...
             tt.ThanhTien,
			 '' '' LoaiThucTreo,
			 ISNULL(tt.Website_id,265) WebsiteId,
             0 AS [Status] 
			FROM  ' + @server_id + '.' + @database + '.dbo.ThucTreo tt
			WHERE tt.Last_Modified_At >=' + @daunhay + convert(nvarchar(23),@ngaythuchien,121)+ @daunhay+' '
	...
	UPDATE [dbo].[ThucChayHopDongChiTiet]
			SET    [HopDongREF]                = A.HopDongREF,
				   ...
				   [DmWebsiteREF]			   = A.WebsiteId,
				   [TenWebsite]				   = ISNULL((select top (1) w.TenWebsite from dbo.dmwebsite w where w.DmWebsiteID = A.WebsiteId order by w.DmWebsiteID),N'')
	...
	SELECT 
				...
				[dchdct].[LoaiThucTreo],
				[dchdct].WebsiteId,
				ISNULL((select top (1) w.TenWebsite from dbo.dmwebsite w where w.DmWebsiteID = [dchdct].WebsiteId order by w.DmWebsiteID),N'') Tenwebsite
