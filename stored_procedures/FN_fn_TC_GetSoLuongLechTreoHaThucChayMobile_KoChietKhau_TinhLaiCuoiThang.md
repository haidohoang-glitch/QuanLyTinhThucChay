# Function: `fn_TC_GetSoLuongLechTreoHaThucChayMobile_KoChietKhau_TinhLaiCuoiThang`

- **Loại**: SQL_SCALAR_FUNCTION
- **Ngày tạo**: 2017-09-13 11:28:16.530000
- **Ngày sửa cuối**: 2017-09-13 11:28:16.530000
...
CREATE FUNCTION [dbo].[fn_TC_GetSoLuongLechTreoHaThucChayMobile_KoChietKhau_TinhLaiCuoiThang]
    (
...
AS
	...
	-- Declare the return variable here
        ...
        SELECT  @DonGiaSauChietKhau = ( CASE WHEN ( @IsKhuyenMai = 0
                                                    AND @ChietKhau <> 100
                                                  )
                                             THEN @DonGia * ( 100 - @ChietKhau )
                                                  / 100
                                             ELSE @DonGia
                                        END )
        SELECT  @ThanhTienHD = ( CASE WHEN ( @IsKhuyenMai = 0
                                             AND @ChietKhau <> 100
                                           ) THEN @ThanhTienHD
		...
		ELSE
			BEGIN
				...
				IF @DmSanPhamREF = 342
					BEGIN
						SELECT  @ThanhTienTCDT = ( CASE WHEN ( @IsKhuyenMai = 1
															   OR @ChietKhau = 100
															 )
														THEN SUM(ISNULL(tcdt.ThanhTienKM,
																	  0)
																 + ISNULL(tcdt.GiaTriKMThayDoi,
																	  0))
														ELSE SUM(ISNULL(tcdt.ThanhTienSauTrietKhauThucChay,
																	  0)
				...
				ELSE
					BEGIN
						...
						IF ( @ThanhTienHD <= @ThanhTienTCDT )
							...
							--4.2.2 NEU A > B => THANH TIEN LECH TREO HA = (B + C) - A
						ELSE
							SET @ThanhTienLTH = ( @ThanhTienTCDT
												  + @ThanhTienThucChayNgay )
								- @ThanhTienHD
					END
