# Stored Procedure: `ThucChay_ReExcInsertThucChayDaTinh_DonViGoi_ThoiGian_V2`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2022-06-28 16:39:50.967000
- **Ngày sửa cuối**: 2022-06-28 16:57:02.003000
...
CREATE PROCEDURE [dbo].[ThucChay_ReExcInsertThucChayDaTinh_DonViGoi_ThoiGian_V2] 
	@StartDate datetime,
	@EndDate datetime,
...
AS
BEGIN
	DECLARE  @Count INT =0, @NgayGioiHanHopDong DATETIME = '2020-01-01',@DmSanPhamREF INT
	...
	, TC.TenSanPham, tc.ThucChayHopDongChiTietID, tc.NhanHang, tc.DmNhanHangREF
	, tc.DmBannerREF
	...
	(
		...
		, tc.DmSanPhamREF, tc.tensanpham,  tc.NhanHang, tc.DmNhanHangREF
	...
	FROM 
	(
		SELECT 0 AS ID, tc.SoHopDong, tc.DmSanPhamREF, tc.DmBannerREF, tc.DmWebsiteREF, tc.TenWebsite
		, tc.BannerTypeName, SUM(tc.TongViewThucChay) TongViewThucChay, SUM(tc.TongClickThucChay) TongClickThucChay  
		FROM dbo.ThucChay tc
		WHERE tc.DmSanPhamREF IN (339, 240, 598, 342, 5056)
		AND tc.NgayThucHien BETWEEN @StartDate AND @EndDate
		AND tc.SoHopDong = @SoHopDong
		GROUP BY tc.SoHopDong, tc.DmSanPhamREF, tc.DmBannerREF, tc.DmWebsiteREF, tc.TenWebsite
		, tc.BannerTypeName
	)tc
