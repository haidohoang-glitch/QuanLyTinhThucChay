# Stored Procedure: `prc_asd_CheckRunningJob`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-04-13 09:06:52.740000
- **Ngày sửa cuối**: 2017-04-13 09:07:21.203000

## Parameters

| Parameter | Type | Output |
|-----------|------|--------|
| `@NAME` | `nvarchar(200)` | No |
| `@DATE` | `datetime(8)` | No |
| `@STATUS` | `int(4)` | No |

## Definition (Source Code)

```sql
--prc_asd_CheckRunningJob '','02/01/2017',-1
CREATE  PROC [dbo].[prc_asd_CheckRunningJob]
    @NAME NVARCHAR(100) ,
    @DATE DATETIME ,
    @STATUS INT
AS
    BEGIN
        SELECT  [sJOB].[name] AS [JobName] ,
                CAST(( CAST([sJOBH].[run_date] AS CHAR(8)) ) AS DATETIME) RunDate ,
                CASE WHEN [sJOBH].[run_date] IS NULL
                          OR [sJOBH].[run_time] IS NULL THEN NULL
                     ELSE CAST(CAST([sJOBH].[run_date] AS CHAR(8)) + ' '
                          + STUFF(STUFF(RIGHT('000000'
                                              + CAST([sJOBH].[run_time] AS VARCHAR(6)),
                                              6), 3, 0, ':'), 6, 0, ':') AS DATETIME)
                END AS [LastRunDateTime] , 
                  --WHEN 0 THEN 'Failed'
                  --WHEN 1 THEN 'Succeeded'
                  --WHEN 2 THEN 'Retry'
                  --WHEN 3 THEN 'Canceled'
                  --WHEN 4 THEN 'Running' 
                [sJOBH].[run_status] [LastRunStatus] ,
                STUFF(STUFF(RIGHT('000000'
                                  + CAST([sJOBH].[run_duration] AS VARCHAR(6)),
                                  6), 3, 0, ':'), 6, 0, ':') AS [LastRunDuration] ,
                [sJOBH].[message] AS [LastRunStatusMessage] ,
                CASE [sJOBSCH].[NextRunDate]
                  WHEN 0 THEN NULL
                  ELSE CAST(CAST([sJOBSCH].[NextRunDate] AS CHAR(8)) + ' '
                       + STUFF(STUFF(RIGHT('000000'
                                           + CAST([sJOBSCH].[NextRunTime] AS VARCHAR(6)),
                                           6), 3, 0, ':'), 6, 0, ':') AS DATETIME)
                END AS [NextRunDateTime]
        FROM    [msdb].[dbo].[sysjobs] AS [sJOB]
                LEFT JOIN ( SELECT  [job_id] ,
                                    MIN([next_run_date]) AS [NextRunDate] ,
                                    MIN([next_run_time]) AS [NextRunTime]
                            FROM    [msdb].[dbo].[sysjobschedules]
                            GROUP BY [job_id]
                          ) AS [sJOBSCH] ON [sJOB].[job_id] = [sJOBSCH].[job_id]
                LEFT JOIN ( SELECT  [job_id] ,
                                    [run_date] ,
                                    [run_time] ,
                                    [run_status] ,
                                    [run_duration] ,
                                    [message] ,
                                    ROW_NUMBER() OVER ( PARTITION BY [job_id] ORDER BY [run_date] DESC, [run_time] DESC ) AS RowNumber
                            FROM    [msdb].[dbo].[sysjobhistory]
                            WHERE   [step_id] = 0
                          ) AS [sJOBH] ON [sJOB].[job_id] = [sJOBH].[job_id]
                                          AND [sJOBH].[RowNumber] = 1
        WHERE   [enabled] = 1
                --AND sJOB.name IN ( SELECT   C.NAME
                --                   FROM     dbo.CATEGORIES AS C
                --                   WHERE    C.TYPE = 0 )
                AND ( @STATUS = -1
                      OR sJOBH.run_status = @STATUS
                    )
                AND ( ISNULL(@NAME, '') = ''
                      OR sJOB.name LIKE '%' + @NAME + '%'
                    )
				AND DATEDIFF(DAY, @DATE, CAST(( CAST([sJOBH].[run_date] AS CHAR(8)) ) AS DATETIME)) = 0
    END
	  

```
