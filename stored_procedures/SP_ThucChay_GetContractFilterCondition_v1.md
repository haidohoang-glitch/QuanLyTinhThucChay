# Stored Procedure: `ThucChay_GetContractFilterCondition_v1`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2014-02-25 11:29:03.160000
- **Ngày sửa cuối**: 2014-10-14 10:39:54.893000
...
CREATE PROCEDURE [dbo].[ThucChay_GetContractFilterCondition_v1] 
...
BEGIN
	...
	BEGIN
		...
		SET @Sql = '
			...
			(
				SELECT
					T2.ID,T2.SHD AS SoHopDong, T2.DonViTinh,
					...
					ROW_NUMBER() OVER (ORDER BY MAX(T2.NgayKyHopDong) DESC) AS num
				FROM
				(
					SELECT
						T1.'+ @GroupFieldName+','+@GroupByFildID+'T1.DonViTinh,T1.SoHopDong AS SHD,
				...
				GROUP BY T2.'+ @GroupFieldName+',T2.ID,T2.DonViTinh,T2.SHD
			) AS T
		...
		BEGIN
			...
			WHILE @@FETCH_STATUS = 0
			   BEGIN
	   				...
	   				SET @Sql = '
	   					SELECT
								T2.SHD AS SoHopDong,T2.ID, T2.DonViTinh,
								SUM(T2.GiaTriThayDoi) GiaTriThayDoi,
								SUM(T2.SoLuongHopDongNoiBo) SoLuongHopDongNoiBo,
								SUM(T2.SoLuongHopDongKhuyenMai) SoLuongHopDongKhuyenMai,
								SUM(T2.SoLuongHopDongThucThu) SoLuongHopDongThucThu,
			...
			SELECT 
				T.GroupFileName AS SoHopDong,T.GroupFileID,T.DonViTinh,T.GiaTriThayDoi,
