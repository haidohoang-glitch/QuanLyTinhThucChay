# Stored Procedure: `ThucChay_InsertThucChayDaTinh_ByHD_Admatic_Native_Ads`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-12-10 16:02:44.113000
- **Ngày sửa cuối**: 2022-06-02 17:47:47.987000
...
CREATE  PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_ByHD_Admatic_Native_Ads] 
...
AS
BEGIN
	PRINT 'Insert thuc chay Admatic Native Ads'
	...
	SELECT  NEWID() AS ThucChayDaTinhID,
		...
		(
			...
			(
				...
				WHERE HopDongREF = @HopDongID AND HopDongChiTietREF = hd.HopDongChiTietID
				--AND DmSanPhamREF = hd.DmSanPhamREF 
				AND DmBannerREF = tc.DmBannerID 
				AND ISNULL(DmNhanHangREF,'') <>'' 
		...
		(
			...
			INNER JOIN 
			(	SELECT HopDongREF, HopDongChiTietREF, DmSanPhamID, DmBannerID, TiLeThucChayHDCTSoVoiBanner FROM dbo.ThucChayHopDongChiTietAndBanner_Native_Ads 
		...
		(SELECT hd.HopDongID,
			...
			(SELECT * FROM dbo.HopDongChiTiet hdct 
				WHERE 1=1
				AND hdct.DmSanPhamREF IN (821,733, 5133)
