# Stored Procedure: `ThucChay_InsertThucChayDaTinh_ByHD_DonViGoi_V2`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2021-10-29 16:57:56.767000
- **Ngày sửa cuối**: 2023-08-22 16:54:53.860000
...
CREATE  PROCEDURE [dbo].[ThucChay_InsertThucChayDaTinh_ByHD_DonViGoi_V2] 
...
AS
BEGIN
	--PRINT 'Insert thuc chay CPM_DonViGoi'
	...
	SELECT  NEWID() AS ThucChayDaTinhID,
		...
		(
			...
			(CASE WHEN tc.DonViTinh = N'VIEW' OR  tc.DonViTinh = N'TRUE VIEW' THEN tc.SoLuongThucChay
			ELSE 0
			END) AS   TongViewThucChay,
			(CASE WHEN tc.DonViTinh = N'CLICK' THEN tc.SoLuongThucChay
			ELSE 0
			END) AS   TongClickThucChay,
			(CASE WHEN tc.DonViTinh <> N'CLICK' AND TC.DonViTinh <> N'VIEW' AND tc.DonViTinh <> N'TRUE VIEW' THEN tc.SoLuongThucChay
			ELSE 0
