# Stored Procedure: `DongBoDuLieu_GGFB`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-10-03 09:30:21.223000
- **Ngày sửa cuối**: 2022-07-15 19:04:28.670000
...
CREATE PROCEDURE [dbo].[DongBoDuLieu_GGFB]
...
BEGIN
	...
	WHILE (@NgayThucHien <= @EndDate)
		...
		--I. Insert vào [ASDAG2].ABM_Data_Release
		--1. Table ThucChayDaTinh
		--Xoa du lieu truoc insert
		DELETE FROM [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinh 
		WHERE NgayThucHien = @NgayThucHien
		--AND DmChienDichREF = 0 --chi lay mua ngoai
		----Insert du lieu
		INSERT INTO [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinh
		 ([ThucChayDaTinhID]
           ,[HopDongID]
           ,[SoHopDong]
           ,[DmMaHopDongREF]
		...
		--Xoa du lieu truoc insert
		DELETE FROM [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinhAdmarket 
		WHERE NgayThucHien = @NgayThucHien
		----Insert du lieu
		INSERT INTO [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinhAdmarket
		([ThucChayDaTinhID]
           ,[HopDongID]
           ,[SoHopDong]
           ,[DmMaHopDongREF]
		...
		--Xoa du lieu truoc insert
		DELETE FROM [ASDAG2].ABM_Data_Release.dbo.ThucChay_LogNNTinhGiaTriThayDoi 
		WHERE NgayThucHien = @NgayThucHien
		--Insert du lieu
		INSERT INTO [ASDAG2].ABM_Data_Release.dbo.ThucChay_LogNNTinhGiaTriThayDoi
		SELECT * FROM ThucChay_LogNNTinhGiaTriThayDoi tcdt
		WHERE tcdt.NgayThucHien = @NgayThucHien		

		--3. ThucChayDaTinh_MuaNgoai
		DELETE FROM [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinh_MuaNgoai 
		WHERE NgayThucHien = @NgayThucHien
		
		--Insert du lieu
		INSERT INTO [ASDAG2].ABM_Data_Release.dbo.ThucChayDaTinh_MuaNgoai
		( [ID]
		  ,[HopDongREF]
		  ,[SoHopDong]
		  ,[DmMaHopDongREF]
		...
		SELECT [ID]
		  ...
		  ,[GhiChu]
		  ,[CreatedAt]
		  ,[LastModifiedAt] FROM ThucChayDaTinh_MuaNgoai tcdt
		WHERE tcdt.NgayThucHien = @NgayThucHien	
	...
	/*
		--II.Insert vào [ASDAG2].ABM_Data_Partner
		--ThucChayDaTinh
		DELETE FROM [ASDAG2].ABM_Data_Partner.dbo.ThucChayDaTinh
			WHERE NgayThucHien = @NgayThucHien		
		----Insert du lieu
		INSERT INTO  [ASDAG2].ABM_Data_Partner.dbo.ThucChayDaTinh
			SELECT * FROM ABM_Data_Partner.dbo.ThucChayDaTinh tcdt
			WHERE NgayThucHien = @NgayThucHien
		--ThucChayAdmarketPublisher	
		DELETE FROM [ASDAG2].ABM_Data_Partner.dbo.ThucChayAdmarketPublisher
			WHERE NgayThucHien = @NgayThucHien

		INSERT INTO [ASDAG2].ABM_Data_Partner.dbo.ThucChayAdmarketPublisher
			select * from ABM_Data_Partner.dbo.ThucChayAdmarketPublisher tcap
			WHERE NgayThucHien = @NgayThucHien
			*/
		--III. Dong bo bang [DmWebsiteReportingdb]
		DELETE FROM [ASDAG2].ABM_Data_Release.dbo.DmWebsiteReportingdb
		WHERE CONVERT(date,CreatedAt) = @NgayThucHien
		
		INSERT INTO [ASDAG2].ABM_Data_Release.dbo.DmWebsiteReportingdb
		...
		,'duongnguyenthithuy' as NguoiThucHien
		,1 as IsDongBo
		,GETDATE() as ThoiGianHoanThanhDongBo
		
		--Insert into Log [ASDAG2].[ABM_Data_Release]
		INSERT INTO [ASDAG2].[ABM_Data_Release].dbo.SysThuChay_HDCNLog
		(MaHanhDong,IsHanhDong,NgayThucHien,ThoiGianChotSoLieu,NguoiThucHien,IsDongBo,ThoiGianHoanThanhDongBo)		 
		SELECT 
		 1 as MaHanhDong
		,1 as IsHanhDong
