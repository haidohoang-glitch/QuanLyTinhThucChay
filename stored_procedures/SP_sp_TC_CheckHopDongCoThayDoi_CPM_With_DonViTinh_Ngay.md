# Stored Procedure: `sp_TC_CheckHopDongCoThayDoi_CPM_With_DonViTinh_Ngay`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2018-03-01 09:51:59.637000
- **Ngày sửa cuối**: 2020-06-12 14:17:11.527000
...
CREATE  PROCEDURE [dbo].[sp_TC_CheckHopDongCoThayDoi_CPM_With_DonViTinh_Ngay] 
...
BEGIN
	...
	DECLARE @count_HDCT INT, @SoHopDong NVARCHAR(200)
	...
	BEGIN
		...
		(
			...
			FROM dbo.HopDongChiTietThayDoi A
			INNER JOIN dbo.HopDongThayDoi B ON A.HopDongFK = B.HopDongFK
			WHERE A.DeletedStatus <> 1 AND B.DeletedStatus <> 1 AND
			A.HopDongChiTietREF = @HopDongChiTietID AND	convert(date,B.NgayThayDoi) >=  @NgayThucHien
			Order by B.NgayThayDoi asc
		)	
		
		SELECT TOP (1)
			@DonGia = A.DonGia, @ChietKhau = A.ChietKhau, @SoLuongHD = A.SoLuong
			FROM dbo.HopDongChiTietThayDoi A
			INNER JOIN dbo.HopDongThayDoi B ON A.HopDongFK = B.HopDongFK
	...
	SELECT  TOP (1)
	@DonGiaLienKeTruoc  = ISNULL(tcdt.DonGia,0),
	@ChietKhauBF = ISNULL(tcdt.ChietKhau,0),
	@SoLuongHDBF = ISNULL(tcdt.SoLuong,0) ,
	@SoHopDong = ISNULL(tcdt.SoHopDong,'')
	...
	SET @SoLuongThucChay = 	(SELECT ISNULL(sum(tcdt.SoLuongThucChay),0) + SUM(ISNULL(tcdt.SoLuongThayDoi,0)) 
		FROM DBO.ThucChayDaTinh tcdt
		WHERE convert(date,tcdt.NgayThucHien) <= @NgayThucHien
		AND tcdt.HopDongID = @HopDongREF
		AND tcdt.DmSanPhamREF = tcdt.DmSanPhamREF
	...
	BEGIN
		...
		SET @NGUON_LOG = @NGUON_LOG + 'Table:HopDongChiTietThayDoi: ' + CONVERT(NVARCHAR(30),@HopDongChiTietThayDoiCK)
		--THUC HIEN TINH DOI TRU TOAN BO GIA TRI CUA HOPDONGCHITIET
		...
		EXEC [dbo].[ThucChay_ReInsertThucChayDaTinhCPM_With_DonViTinh_Ngay_ByHopDongChiTiet] 
			@pSoHopDong = @SoHopDong,
			@pHopDongChiTietID = @HopDongChiTietID,
			@DmSanPhamREF = @DmSanPhamREF,
			@NgayThucHienGhiNhan = @NgayThucHien
	END
