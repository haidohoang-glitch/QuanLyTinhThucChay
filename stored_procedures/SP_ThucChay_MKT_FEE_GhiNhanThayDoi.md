# Stored Procedure: `ThucChay_MKT_FEE_GhiNhanThayDoi`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2025-06-24 16:41:31.857000
- **Ngày sửa cuối**: 2026-03-18 16:00:37.800000
...
EXEC [dbo].[ThucChay_MKT_FEE_GhiNhanThayDoi]
	@NgayThucHien = '2025-10-15 00:00:00.000',
	@NgayDanhSoGioiHan = '2020-05-01 00:00:00.000'
*/

CREATE PROCEDURE [dbo].[ThucChay_MKT_FEE_GhiNhanThayDoi] 
	@NgayThucHien DATETIME,
	@NgayDanhSoGioiHan DATETIME
AS
BEGIN
	DECLARE @GhiChu NVARCHAR(1000) = N'PP_M doi tru thuc chay thay doi PB-Marketing fee do thay đổi thanhtien'
	...
	*/
	...
	-- CTE 1: Tính tích lũy theo từng dòng
	;WITH TichLuyCTE AS (
		SELECT 
			t.HopDongID,
			t.HopDongChiTietID,
			...
			-- Giá trị tích lũy đến dòng hiện tại
			TichLuy = SUM(t.ThanhTien_DaTinh_HDCT) OVER (
				PARTITION BY t.HopDongChiTietID
	...
	-- CTE 2: Lấy dòng cuối cùng của từng HopDongChiTietID theo RowNumberTCDT DESC
	TichLuyCuoiCTE AS (
		SELECT 
			HopDongChiTietID,
			TichLuy AS ThucChayDaTinhTichLuyCuoi
		FROM (
			SELECT *, 
				   RN = ROW_NUMBER() OVER (
						PARTITION BY HopDongChiTietID
						ORDER BY RowNumberTCDT DESC
				   )
			FROM TichLuyCTE
		) t
		WHERE RN = 1
	...
	-- CTE 3: Gộp lại và tính TrangThaiGhiNhan + đánh số giảm dần
	MarkFlag AS (
		SELECT 
			a.*,
			b.ThucChayDaTinhTichLuyCuoi,

			TrangThaiGhiNhan = CASE 
				WHEN (a.TichLuy > a.ThanhTien_HDCT) AND (b.ThucChayDaTinhTichLuyCuoi > a.ThanhTien_HDCT) AND (a.ThanhTien_DaTinh_HDCT >0) THEN 0
				ELSE 1
			END,

			RowNumberDesc = ROW_NUMBER() OVER (
				PARTITION BY a.HopDongChiTietID
				ORDER BY a.RowNumberTCDT DESC
			)
		FROM TichLuyCTE a
		LEFT JOIN TichLuyCuoiCTE b 
			ON a.HopDongChiTietID = b.HopDongChiTietID
	)

	UPDATE tc
	SET tc.ThanhTienGhiNhanTCDT = m.TichLuy
	...
	SELECT		ThucChayDaTinhID = NEWID(),
				...
				GiaTriThayDoi = -IIF(tcdt.ChietKhau = 100, 0, (tcdt.ThanhTienSauTrietKhauThucChay + tcdt.GiaTriThayDoi)),
				...
				PheDuyetBy = '',
				PheDuyetAt = '',
				SoLuongThayDoi = -(tcdt.SoLuongThucChay + tcdt.SoLuongThayDoi),
				SoLuongKMThayDoi = -(tcdt.SoLuongThucChayKM + tcdt.SoLuongKMThayDoi),
				GiaTriKMThayDoi = -(tcdt.ThanhTienKM + tcdt.GiaTriKMThayDoi),
				GhiChu = @GhiChu + ', SP[ThucChay_MKT_FEE_GhiNhanThayDoi] ; ThucChayDaTinhID = ' + tcdt.ThucChayDaTinhID
	FROM dbo.ThucChayDaTinh tcdt
