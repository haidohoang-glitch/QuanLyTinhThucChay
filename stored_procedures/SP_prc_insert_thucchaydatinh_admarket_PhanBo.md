# Stored Procedure: `prc_insert_thucchaydatinh_admarket_PhanBo`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2024-02-06 11:05:45.853000
- **Ngày sửa cuối**: 2024-12-02 11:42:43.940000
...
-- =============================================
-- exec [dbo].[prc_insert_thucchaydatinh_admarket_PhanBo] '2018-10-08'
CREATE PROCEDURE [dbo].[prc_insert_thucchaydatinh_admarket_PhanBo] 
...
BEGIN
	...
	, @GhiChu NVARCHAR(1000) = N'Insert theo chieu domain vao ThucChayDaTinh'

	/*XU LY THONG TIN WEBSITE TRUOC KHI THUC HIEN INSERT*/
	--[dbo].ThucChayAdmarket_PhanBo
    ...
    IF ( @slsite > 0 )
        BEGIN
            DECLARE Cursor_admarket_site CURSOR FOR
			...
			OPEN Cursor_admarket_site
			FETCH NEXT FROM Cursor_admarket_site INTO @pTenWebiste
			...
			BEGIN
				...
				FETCH NEXT FROM Cursor_admarket_site INTO @pTenWebiste
			END
			CLOSE Cursor_admarket_site
			DEALLOCATE Cursor_admarket_site
        END
	...
	and HopDongID not in (Select HopDongREF from DmThongTinHopDongBanInventory) 

	-- insert các san pham performance base vao table thucchaydatinh tu table  ThucChayAdmarket_PhanBo
	...
	SUM(convert(money,a.domain_tt_money)) ThanhTienSauTrietKhauThucChay, -- dien gia tri  tu bang nhe
	0 GiaTriHoaHongThucChay,
	0 ThanhTienThucThu, -- dien gia tri  tu bang nhe
	SUM(convert(money,a.domain_tt_promotion)) ThanhTienKM, -- dien gia tri  tu bang nhe
	...
	0 GiaTriKMThayDoi,
	@GhiChu GhiChu
	FROM [dbo].ThucChayAdmarket_PhanBo a WHERE (convert(money,a.domain_tt_money) > 0 OR convert(money,a.domain_tt_promotion) > 0)
	...
	(
		...
		@GhiChu GhiChu
		FROM [dbo].ThucChayAdmarket_PhanBo a WHERE (convert(money,a.domain_tt_money) > 0 OR convert(money,a.domain_tt_promotion) > 0)
		AND a.NgayThucHien = @ngaythuchien and ((a.isnoibo = 1) or (a.contract_number like 'NB%' or a.contract_number like 'SH%' or a.contract_number like 'S-NB%'))
