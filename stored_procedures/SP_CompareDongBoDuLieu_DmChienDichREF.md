# Stored Procedure: `CompareDongBoDuLieu_DmChienDichREF`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2020-09-29 14:24:40.080000
- **Ngày sửa cuối**: 2020-10-09 16:04:15.550000
...
CREATE PROCEDURE [dbo].[CompareDongBoDuLieu_DmChienDichREF]
...
BEGIN
	...
	WHERE tcdt.NgayThucHien BETWEEN @StartDate AND @EndDate
	and DmChienDichREF =0 -- chỉ lấy mua ngoài
	GROUP BY tcdt.DmSanPhamREF, tcdt.TenSanPham,SoHopDong,NgayThucHien,HopDongID,HopDongChiTietREF,	DonViTinh, DmHinhThucQuangCao
	)B
	ON (
    ...
    SELECT A.*, B.*
    FROM (
	SELECT tcdt.DmHinhThucQuangCao, tcdt.DmSanPhamREF, tcdt.TenSanPham, tcdt.DmWebsiteREF, tenwebsite, tcdt.NgayThucHien,
	sum(tcdt.ttClick) tc, sum(tcdt.ttView) tv
	FROM [asdag].ABM_Data_Partner.dbo.ThucChayAdmarketPublisher tcdt
	WHERE tcdt.NgayThucHien BETWEEN @StartDate AND @EndDate
	GROUP BY tcdt.DmHinhThucQuangCao, tcdt.DmSanPhamREF, tcdt.TenSanPham, tcdt.DmWebsiteREF, tenwebsite, tcdt.NgayThucHien
	...
	SELECT tcdt.DmHinhThucQuangCao, tcdt.DmSanPhamREF, tcdt.TenSanPham, tcdt.DmWebsiteREF, tenwebsite, tcdt.NgayThucHien,
	sum(tcdt.ttClick) tc, sum(tcdt.ttView) tv
	FROM ABM_Data_Partner.dbo.ThucChayAdmarketPublisher tcdt 	
	WHERE tcdt.NgayThucHien BETWEEN @StartDate AND @EndDate
	GROUP BY tcdt.DmHinhThucQuangCao, tcdt.DmSanPhamREF, tcdt.TenSanPham, tcdt.DmWebsiteREF, tenwebsite, tcdt.NgayThucHien
	)B
	ON ( 
    ...
    AND A.DmWebsiteREF IN (85,3144,--giadinh.net.vn
		134,3134,--nld.com.vn		
		182,3136,--suckhoedoisong.vn
	...
	from [asdag].ABM_Data_Release.dbo.DmWebsiteReportingdb
	 where DeletedStatus = 0
	)A
	full outer join 
	(select DmWebsiteReportingdbId, TenWebsite 
	from dbo.DmWebsiteReportingdb where DeletedStatus = 0
	)B
	on A.DmWebsiteReportingdbId = B.DmWebsiteReportingdbId
	and A.TenWebsite = B.TenWebsite
	
	where A.DmWebsiteReportingdbId is null or B.DmWebsiteReportingdbId is null or
	A.TenWebsite is null or B.TenWebsite is null 
END
--WebsiteMapping_HDCN_Reporting
