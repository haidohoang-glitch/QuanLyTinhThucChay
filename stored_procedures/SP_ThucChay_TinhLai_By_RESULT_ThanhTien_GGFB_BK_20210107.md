# Stored Procedure: `ThucChay_TinhLai_By_RESULT_ThanhTien_GGFB_BK_20210107`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2021-01-07 10:16:07.950000
- **Ngày sửa cuối**: 2021-01-07 10:16:07.950000
...
| `@pOperating_Order_Id` | `int(4)` | No |
| `@ADS_OPERATING_RESULT_MAP_ORDER_OR_QUANTITY_ID` | `int(4)` | No |
| `@NgayThucHien` | `datetime(8)` | No |
| `@Ghichu_TinhLaiThucChay` | `nvarchar(1000)` | No |
...
EXEC [dbo].[ThucChay_TinhLai_By_ORDER_ThanhTien_GGFB] 
	...
	@pOperating_Order_Id INT,
	@NgayThucHien DATETIME,
	@Ghichu_TinhLaiThucChay NVARCHAR(500)

*/

CREATE PROCEDURE [dbo].[ThucChay_TinhLai_By_RESULT_ThanhTien_GGFB_BK_20210107] 
	@pSoHopDong NVARCHAR(100),
	@pHopDongChiTietID INT,
	@pOperating_Order_Id INT,
...
BEGIN
	...
	DECLARE @GhiChu_DoiTruThucChay NVARCHAR(1000)
	DECLARE 
	...
	SET @GhiChu_DoiTruThucChay =  N'%Doi tru thanhtien_GGFB%'
	...
	INNER JOIN 
		(SELECT OD.Contract_Id, OD.Contract_Number
			...
			AND OD.Id = @pOperating_Order_Id
			AND OD.IsDeleted = 0
		) OD ON TC.Operating_Order_Id = OD.Id
	WHERE 1=1
	AND TC.IsDeleted = 0
	AND (ISNULL(TC.Sell_Money_VND,0) <> 0)
	AND TC.ID = @ADS_OPERATING_RESULT_MAP_ORDER_OR_QUANTITY_ID
	...
	BEGIN
		...
		BEGIN
			...
			PRINT '@TongTienThucChaySauChietKhauOder:' + CONVERT(NVARCHAR(50),@TongTienThucChaySauChietKhauOder)
			...
			BEGIN
				...
				BEGIN
					SET @ThanhTienSauTrietKhauThucChay = (SELECT TOP(1) (tcdt.ThanhTienSauTrietKhauThucChay + tcdt.GiaTriThayDoi)  FROM dbo.ThucChayDaTinh tcdt
															...
															AND tcdt.GhiChu not like @GhiChu_DoiTruThucChay
