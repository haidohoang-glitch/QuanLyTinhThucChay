# Stored Procedure: `prc_asd_tinhthucchay_admarket_viewplus_chitiet`

- **Loại**: SQL_STORED_PROCEDURE
- **Ngày tạo**: 2017-09-11 15:02:53
- **Ngày sửa cuối**: 2022-10-06 10:04:10.133000
...
```sql
CREATE PROCEDURE [dbo].[prc_asd_tinhthucchay_admarket_viewplus_chitiet]
...
BEGIN
	...
	declare @nhan_thucchay nvarchar(200), @domain_ref int, @row_count int, @row_index int = 1
	, @nhan_phanbo nvarchar(200)
	...
	begin
		...
		if isnull(@nhan_thucchay,'') =  '' or @nhan_thucchay ='0'
	...
	begin
		set @nhan_phanbo = ISNULL((select top 1 nhan_phanbo from @phanbo),'')
		if isnull(@nhan_phanbo,'') =  '' or @nhan_phanbo ='0'
		begin
			set @nhan_thucchay = STUFF(( SELECT
                                                              ','
                                                              + CONVERT(NVARCHAR(100), nhanhang_id)
	...
	if @row_count = 0  exec [dbo].[prc_asd_insert_ThucChayAdmarket_Error]   @user_id  ,
																			@username  ,
																			@contract_number,
																			@domain_name  ,
																			@domain_tt_click  ,
	...
	from @phanbo where stt = @row_index
		...
		begin
			...
			--print 'gia tri' + convert(nvarchar(50),@thanhtien_phanbo_thucchay)
